...
class BASEClient(metaclass=Singleton):
    ...
    def get_sample_files(self, sampleset_id, batch='batch-0', sensor='all'):

        payload = {
            'sensor': sensor
        }
        url = self.config.url + url_dict.get('get_dataset').format(dataset_id=sampleset_id)
        try:
            dataset_resp = requests.get(url=url,
                                        params=payload,
                                        headers={'apiKey': self.apiKey})
    ...
    def download_labels(self, sampleset_id, save_path, batch='batch-0', sensor='all', ):
        """
        ...
        Returns:
             True if the label files downloaded successfully.
        """
        Utils.makedir(save_path)
        sensors = self.get_sample_files(sampleset_id, batch, sensor)
        if not sensors:
            self.logger.error("No sensors found in the dataset.")
            return False
        ...
        for sensor, files in sensors.items():
            for file in files:
                ...
                dataset_resp = requests.get(url=self.config.url+url_dict.get('get_labels'),
                                            params=payload,
                                            headers={'apiKey': self.apiKey}).json()
                label_info = dataset_resp['data']
                json_file_name = os.path.join(save_path, file['filename'].split('.')[0] + '.json')
