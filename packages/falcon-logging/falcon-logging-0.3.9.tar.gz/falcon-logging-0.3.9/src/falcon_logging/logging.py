...
def set_request_id(request: Request, response: Response):
    cloud_trace_context = request.get_header(_TRACE_CONTEXT_HEADER_NAME, None)

    if cloud_trace_context is None:
    ...
    request.context.x_cloud_trace_context = ctx.x_cloud_trace_context
    request.context.trace_id = ctx.trace_id
    request.context.span_id = ctx.span_id
    request.context.trace = f"projects/{PROJECT_ID}/traces/{ctx.trace_id}"
    ctx.trace = f"projects/{PROJECT_ID}/traces/{ctx.trace_id}"

    response.set_header('X-CLOUD-TRACE-CONTEXT', ctx.x_cloud_trace_context)
    return
