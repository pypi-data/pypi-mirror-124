...
def mongodb_connect(param="core", host=config.mongodb_ip, port=config.mongodb_port,
                    user=config.mongodb_user, pwd=config.mongodb_password):
    try:
...
# def mongodb_old_connect(param="core", host="123.57.158.203", port=27017,
#                         user="root", pwd="Industry@2020"):
#     try:
#         client = MongoClient(host, port, connect=False)
#         db = client.admin  # 先连接系统默认数据库admin
#         db.authenticate(user, pwd, mechanism='SCRAM-SHA-1')
...
def mongodb_collection(db_name, collection):
    try:
        ...
        db = client.admin  # 先连接系统默认数据库admin
        db.authenticate(config.mongodb_user, config.mongodb_password, mechanism='SCRAM-SHA-1')
        db = client[db_name]
...
class MgSession:
    def __init__(self, db="core", host=config.mongodb_ip, user=config.mongodb_user,
