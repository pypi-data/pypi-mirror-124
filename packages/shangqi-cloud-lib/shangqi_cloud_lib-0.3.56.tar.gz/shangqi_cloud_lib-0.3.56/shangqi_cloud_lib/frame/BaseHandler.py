...
from web_frame.frame.DocHandler import insert_params_sql, SqlSession
...
class BaseHandler(tornado.web.RequestHandler, ABC):
    ...
    def insert_request_record(self, func_name, params):
        record_list = [{
            "class_route": self.request.path,
            "interfance_name": func_name,
            "params": json.dumps(params)
        }]
        sql = "INSERT INTO test_request_record ({}) VALUES ({})"
        param_list= ["class_route", "interfance_name", "params_str"]
        value_list = ["%({})s".format(x) for x in param_list]
        sql =  sql.format(",".join(param_list), ",".join(value_list))
