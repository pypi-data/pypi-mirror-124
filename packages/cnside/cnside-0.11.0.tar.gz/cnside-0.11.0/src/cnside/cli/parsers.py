...
from cnside.objects.core import UnifiedPackageID, PYPIPackage, NPMPackage
...
class CLIParser(metaclass=abc.ABCMeta):
    ...
    @property
    @abc.abstractmethod
    def packages(self) -> List[PackageParam]:
...
class PIPInstallParser(CLIParser):
    ...
    def packages(self) -> List[PackageParam]:
        ...
        if not self.args.packages:
            rv.append(PackageParam(data=self.args.requirement, typ=PackageParamType.MANIFEST))
        else:
            for p in self.args.packages:
                rv.append(PackageParam(data=PYPIPackage(p).upid, typ=PackageParamType.UPID))
...
class NPMInstallParser(CLIParser):
    ...
    def packages(self) -> List[PackageParam]:
        ...
        if not self.args.packages:
            rv.append(PackageParam(data=metadata.packages.ManifestNames.NPM, typ=PackageParamType.MANIFEST))
        else:
            for p in self.args.packages:
                rv.append(PackageParam(data=NPMPackage(p).upid, typ=PackageParamType.UPID))
