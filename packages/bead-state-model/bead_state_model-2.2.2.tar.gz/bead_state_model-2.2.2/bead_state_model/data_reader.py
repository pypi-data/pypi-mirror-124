...
class DataReader:
    ...
    def get_filaments(self, frame_idx: int) -> Tuple[List[Filament], LinksArray]:
        """
        ...
        if self.parts:
            ...
            msg += "Use DataReader.get_filaments_all instead."
    ...
    def _read_dataset_from_filaments_group(self, dataset_name: str, frame: int) -> np.ndarray:
        with h5py.File(self.links_file, 'r') as h5f:
            group = h5f['filaments']
            k0 = list(group.keys())[0]
            template = "{:0" + str(len(k0)) + "}"
