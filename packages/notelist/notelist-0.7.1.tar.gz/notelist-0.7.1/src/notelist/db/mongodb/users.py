...
class MongoDbUserManager(UserManager):
    ...
    def get_all(self) -> list[dict]:
        ...
        """
        users = self._db[self._col].find(sort=[("username", ASCENDING)])
    ...
    def get_by_id(self, _id: str) -> Optional[dict]:
        ...
        """
        user = self._db[self._col].find_one({"_id": _id})
    ...
    def get_by_username(self, username: str) -> Optional[dict]:
        ...
        :return: User document if it exists or None otherwise.
        """
        user = self._db[self._col].find_one({"username": username})
    ...
    def put(self, user: dict):
        ...
        user = self._root_dm.switch_id(user)
        f = {"_id": user["_id"]}
        self._db[self._col].replace_one(f, user, upsert=True)
    ...
    def delete(self, _id: str):
        ...
        :param _id: User ID.
        """
        self._db[self._col].delete_one({"_id": _id})
