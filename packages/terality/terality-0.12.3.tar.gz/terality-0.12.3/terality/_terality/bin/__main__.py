...
@click.group(
    help="This is the Terality command line interface. Documentation is available at https://pypi.org/project/terality/.\n\nRun 'terality account create' to get started with a free account.",
...
@account.command(
    ...
    help="Create a free Terality account using the provided email.",
)
@click.option(
...
def account_configure(email: str, api_key: str, overwrite: bool):
    try:
        config: Optional[TeralityConfig] = TeralityConfig.load()
    except Exception:
        config = None
    try:
        credentials: Optional[TeralityCredentials] = TeralityCredentials.load()
    ...
    if (config is not None or credentials is not None) and not overwrite:
        click.confirm("Are you sure you want to overwrite the existing Terality configuration?")
