import random
'''
name generates a random name from the list of adjectives and surnames in this package
formatted as "adjective_surname". For example 'focused_turing'. If retry is `True`, a random
integer between 0 and 10 will be added to the end of the name, e.g `focused_turing3`
If `sep` is something other than `_` it is used intsead of the underscore in separating the
adjective and the surname
'''
def name(sep='_', int_suffix=False):
	left = [
		"admiring",
		"adoring",
		"affectionate",
	...
	left_val = left[random.randint(0,len(left)-1)]
	right_val = right[random.randint(0,len(right)-1)]
	name = f"{left_val}{sep}{right_val}"

	if int_suffix:
		rand_int = random.randint(0, 9)
		name = f"{name}{rand_int}"
	return name
...
def quote(kind='any'):
	...
	negative = [
		...
		'"YOU DIED" Dark Souls',
		'"Can It Wait For A Bit? I\'m In The Middle Of Some Calibrations." Garrus, Mass Effect 2s',
		'"I\'m sorry, Dave. I\'m afraid I can\'t do that." HAL, 2001: A Space Odyssey'
	]

	pool = []
	if kind == 'any':
		pool = positive + neutral + negative
	elif kind == 'positive':
		pool = positive
	elif kind == 'neutral':
		pool = neutral
		...
		raise ValueError("Valid kinds are 'any', 'positive', 'neutral', and negative'")
...
if __name__ == '__main__':
	print(name())
	print(name(sep='-'))
	print(name(sep='~', int_suffix=True))
	print(quote(kind='any'))
	print(quote(kind='positive'))
	print(quote(kind='neutral'))
	print(quote(kind='negative'))