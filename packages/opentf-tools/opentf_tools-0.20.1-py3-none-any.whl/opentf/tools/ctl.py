...
Agent Commands:
  ...
  delete agent {agent_id}      Deregister an agent
...
DELETE_AGENT_HELP = '''Deregister an active agent

Example:
  # Deregister the specified agent
...
def delete_agent(agent_id):
    ...
    try:
        ...
        if what.status_code == 200:
            ...
            logging.error('Cound not delete agent %s: not found.', agent_id)
            sys.exit(1)
        else:
            logging.error(
        ...
        logging.error('Cound not delete agent, %s.', erro)
        sys.exit(2)
    print(what['message'])
