...
def init_series_parser(parser: ArgumentParser):
    ...
    series_cmd_parsers = parser.add_subparsers(
        ...
        **({'required': True} if sys.version_info > (3, 7) else {})  # type: ignore
    )
    init_series_export_parser(series_cmd_parsers.add_parser(
        CMD_EXPORT, help='Export series to a local file',
        formatter_class=RawTextHelpFormatter
    ))
    init_series_import_parser(series_cmd_parsers.add_parser(
        CMD_IMPORT, help='Import series from a local file',
        formatter_class=RawTextHelpFormatter
    ))
...
def handle_series_cmd(client: WaylayClient, args: Namespace):
