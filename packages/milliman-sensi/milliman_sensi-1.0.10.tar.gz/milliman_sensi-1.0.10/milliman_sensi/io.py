...
def read_sensitivities(env_dir):
    ...
    if not set(sensi_config['Stress name']).issubset(set(sensi_param.columns)):
        raise SensiIOError("Columns {} in sensi_param don't match the values in sensi_config".format(sensi_config['Stress name'] - set(sensi_param.columns)))
...
def create_one_sensi_from_base(sensi_name, base_dir, sensi_path=None):
    ...
    if os.path.exists(base_dir) and sensi_path:
        ...
        raise SensiIOError("Can't create a sensi_dir because {} doesn't exist".format(base_dir))
...
class SensiConfig:
    ...
    def create_tables(self, sensi_dirs={}):
        ...
        if self.sensi_list:
            ...
            if sensi_dirs:
                for sensi in sensi_dirs.keys():
                    processed_sensi_dirs[sensi] = SensiIOError("This sensi wasn't found in sensi_config.csv")
                return processed_sensi_dirs
            else:
                raise SensiIOError("The sensi list is empty")

        if sensi_dirs_to_process:
            for sensi in sensi_dirs_to_process.keys():

                try:
                    path_to_sensi = create_one_sensi_from_base(sensi, self.base_dir, sensi_dirs_to_process[sensi])
                except SensiIOError as err:
                    processed_sensi_dirs[sensi] = err
                    continue
                ...
                if os.path.exists(path_to_sensi):
                    try:
                        ...
                        processed_sensi_dirs[sensi] = SensiIOError("couldn't read settings.json inside {}: {}".format(path_to_sensi, err))
                        continue
                    ...
                    processed_sensi_dirs[sensi] = SensiIOError("path_to_sensi doesn't exist, it's {}".format(path_to_sensi))
        else:
            if sensi_dirs:
                for sensi in sensi_dirs.keys():
                    processed_sensi_dirs[sensi] = SensiIOError("{} was not found in the sensi_list extracted from sensi_config.csv".format(sensi))
            else:
                raise SensiIOError("None of the sensi names in sensi_dirs was found in the sensi list")
        ...
        if self.sensi_list:
            if sensi_dirs:
                ...
                if os.path.exists(self.base_dir):
                    ...
                    if sensi_dirs:
                        for sensi in sensi_dirs:
                            processed_sensi_messages[sensi] = SensiIOError("The base_dir wasn't found in {}".format(self.base_dir))
                        return processed_sensi_messages
                    else:
                        raise SensiIOError("base_dir is not found!")
        else:
            if sensi_dirs:
                for sensi in sensi_dirs.keys():
                    processed_sensi_messages[sensi] = SensiIOError("This sensi wasn't found in sensi_config.csv")
                return processed_sensi_messages
            else:
                raise SensiIOError("The sensi list is empty")
                ...
                for sensi_name in sensi_dirs_to_process.keys():
                    ...
                    if os.path.exists(sensi_dirpath):
                        try:
                            ...
                            processed_sensi_messages[sensi] = SensiIOError("couldn't read settings.json inside {}: {}".format(sensi_dirpath, err))
                            continue

                        if settings_json_sensi:

                            # contains the content of settings_modif.csv to be saved
                            settings_modif_dict = {"id": [], "value": []}
                                ...
                                for command in self.param_map[stress_name]:
                                    try:
                                        ...
                                        if syntax.expression.startswith("$"):
                                            ...
                                            # Insert command in settings_modif_dict
                                            settings_modif_dict["id"].append(syntax.expression)
                                            settings_modif_dict["value"].append(syntax.value)
                                            continue

                                        # print(syntax.expression, ";", syntax.col, ";", syntax.condition, ";", syntax.value)
                                    except syn.SensiSyntaxError as err:
                                        processed_sensi_messages[sensi_name] = SensiIOError(err.msg)
                                        break

                                    if path_to_file:
                                        print("Applying command :", command, "\nto file:", path_to_file, end="\n\n")
                                        ...
                                        if applied:
                                            counter += 1
                                            processed_sensi_messages[sensi_name] = "Applied a total of {} stress modification(s)".format(counter)
                                        else:
                                        ...
                                        processed_sensi_messages[sensi_name] = SensiIOError("Couldn't find an input file in {}".format(sensi_dirpath))
                                        break
                                else:
                                    continue

                                # Used to skip to next sensi if a command fails for any stress name
                                break

                            # Saving settings_modif commands to settings_modif.csv
                            if settings_modif_dict["id"] and settings_modif_dict["value"]:
                                settings_modif_pd = pd.DataFrame(settings_modif_dict)
                                settings_modif_pd.to_csv(os.path.join(sensi_dirpath, 'resources/settings_modif.csv').replace("\\", "/"), sep=";", index=False)

                        else:
                            processed_sensi_messages[sensi_name] = SensiIOError("Couldn't find settings.json")

                    else:
                        processed_sensi_messages[sensi_name] = SensiIOError("Sensi_dirpath doesn't exist")

            else:
                if sensi_dirs:
                    for sensi in sensi_dirs.keys():
                        processed_sensi_messages[sensi] = SensiIOError("{} was not found in the sensi_list extracted from sensi_config.csv".format(sensi))
                else:
                    raise SensiIOError("None of the sensi names in sensi_dirs was found in the sensi list")
        else:
            if sensi_dirs:
                for sensi in sensi_dirs.keys():
                    processed_sensi_messages[sensi] = SensiIOError("No commands found in sensi_pram for this sensi")
            else:
                raise SensiIOError("The sensi_param is empty")
