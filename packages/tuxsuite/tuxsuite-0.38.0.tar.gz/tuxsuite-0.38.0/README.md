...
```
wget -O /etc/apt/trusted.gpg.d/tuxsuite.gpg https://tuxsuite.com/packages/signing-key.gpg
```

2) Create /etc/apt/sources.list.d/tuxsuite.list with the following contents:

```
deb https://tuxsuite.com/packages/ ./
```
...
name=tuxsuite
type=rpm-md
baseurl=https://tuxsuite.com/packages/
gpgcheck=1
gpgkey=https://tuxsuite.com/packages/repodata/repomd.xml.key
enabled=1
```
