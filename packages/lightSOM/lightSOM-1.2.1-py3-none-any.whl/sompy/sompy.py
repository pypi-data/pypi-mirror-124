...
class SOM(object):
    ...
    def diff_graph(self, show=False, printout=True, returns=False, path='./'):
        ...
        neighbours = []
        for node in self.codebook.nnodes:
            nodelist = []
            for nodet in self.codebook.nnodes:
                if node != nodet and node.get_nodeDistance(nodet) <= 1.001:
                    nodelist.append(nodet)
            neighbours.append(nodelist)

        diffs = []
        for node, neighbours in zip(self.codebook.nnodes, neighbours):
            diff = 0
            for nb in neighbours:
                diff = diff + node.get_distance(nb.weights)
            diffs.append(diff)

        centers = [[node.pos[0], node.pos[1]] for node in self.codebook.nnodes]
