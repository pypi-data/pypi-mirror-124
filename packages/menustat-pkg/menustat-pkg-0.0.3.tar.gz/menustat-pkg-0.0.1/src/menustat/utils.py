...
def clean_item_name(item_name, string_replace_dict):
    for pat, repl in string_replace_dict.items():
        item_name = item_name.replace(pat, repl)
    # item_name.translate({ord(i): None for i in "'’"})
    return item_name
...
def return_dict_from_list(list, search, field):
    for itemdict in list:
        if itemdict[field] == search:
            returndict = itemdict
    return returndict

def remove_from_dict(idict, badstr, how="key"):
    if how == "key":
        return {k:v for k in idict.items() if not re.match(badstr, k)}
    elif how == "val":
        return {k:v for k, v in idict.items() if not re.match(badstr, v)}

def loop_string_replacement(replacedict, string, regex=True):
...
def return_dict_from_dict(itemdict, search):
...
def return_nutrient_range(value, multiplier=.05):
...
def set_colvals(df, index_name: dict):
    for k, v in index_name.items():
        df.columns.values[k] = v
    return df
...
def return_col_name(df, idx):
    """return name of col with given index"""
    return df.columns.values[idx]
