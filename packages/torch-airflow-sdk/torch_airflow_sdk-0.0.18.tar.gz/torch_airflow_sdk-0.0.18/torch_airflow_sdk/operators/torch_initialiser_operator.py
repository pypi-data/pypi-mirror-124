...
def check_if_pipeline_exists(pipeline_uid: str, client):
    try:
        pipeline = client.get_pipeline(uid=pipeline_uid)
    ...
    def execute(self, context):
        ...
        if pipeline_exists is True:
            ...
            pipeline = CreatePipeline(
                ...
                name=self.pipeline_uid,
                description='Torch pipeline has been created from torch-airflow-sdk',
                meta=PipelineMetadata(
                    owner='sdk/pipeline-user', team='TORCH', codeLocation='...'),
                context={'pipeline': self.pipeline_uid}
            )
            pipeline_res = client.get_torch_client.create_pipeline(pipeline=pipeline)
