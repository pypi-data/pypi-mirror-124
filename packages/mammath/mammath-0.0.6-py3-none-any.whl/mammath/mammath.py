...
def graph(graph_func, lrange=-10, urange=10, graph_points=100, graph_title=None):
    x = np.linspace(lrange, urange, graph_points)
    y = evaluateExpression(graph_func)
...
def exp_graph(*argv):
    ...
    while z < len(argv):
        x = np.linspace(lrange, urange, graph_points)
        y = evaluateExpression(str(argv[z]))
        plt.title(graph_title)
        plt.plot(x,y)
        z += 1
    plt.show()
