...
Implementation and naming conventions are taken from [1]_.
"""
...
def generate_utility_variable(
    ...
    u_var: Matrix[LpVariable],
    g_matrix: Matrix[NumericValue],
) -> Any:
...
def compute_uta_after_normalization(
    performance_table: NumericPerformanceTable,
    criteria_segments: Iterable[int],
    relations: List[Tuple],
    delta: NumericValue = 0.001,
    ...
    g = generate_g_matrix(criteria_segments)
    u_var = generate_u_variables(criteria_segments)
    sigma_var = generate_sigma_variables(len(performance_table))
    ...
    return compute_post_optimality(
        performance_table,
        criteria_segments,
        relations,
        delta,
...
def compute_uta_star_after_normalization(
    performance_table: NumericPerformanceTable,
    criteria_segments: Iterable[int],
    relations: List[Tuple],
    delta: NumericValue = 0.001,
    ...
    # Create all UTA variables
    g = generate_g_matrix(criteria_segments)
    w_var = generate_u_variables(criteria_segments)
    sigma_p_var = generate_sigma_variables(len(performance_table), "sigma_p")
    sigma_n_var = generate_sigma_variables(len(performance_table), "sigma_n")
...
def compute_star_post_optimality(
    performance_table: NumericPerformanceTable,
    criteria_segments: Iterable[int],
    relations: List[Tuple],
    delta: NumericValue = 0.001,
    ...
    for ci in range(len(g)):
        for k, sense in enumerate([LpMinimize, LpMaximize]):
            # Create all UTA variables
            w_var = generate_u_variables(criteria_segments)
            sigma_p_var = generate_sigma_variables(
                len(performance_table), "sigma_p"
            )
            sigma_n_var = generate_sigma_variables(
...
def compute_uta(
    ...
    normalized_functions = compute_uta_after_normalization(
        _performance_table,
        criteria_segments,
        relations,
        delta,
...
def compute_uta_star(
    ...
    normalized_functions = compute_uta_star_after_normalization(
        _performance_table,
        criteria_segments,
        relations,
        delta,
