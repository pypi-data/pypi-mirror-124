...
class LabelmeDataset(Dataset):
    ...
    def __init__(self, rootDir, debug = False, train=True, transforms=None, imgFormat=None, **kwargs):
        ...
        if self.isTrain:
            ...
            for filename in sorted(os.listdir(self._root)):
                if osp.isdir(osp.join(self._root,filename)):
                    continue
            ...
            for filename in sorted(os.listdir(self._root)):
                ...
                suffix = filename[filename.rfind("."):]
                if suffix == ".json":
                    mask_path = osp.join(self._root, filename)

                    with open(mask_path, "r") as f:
                        content = json.load(f)
                    ip = content["imagePath"]
                    if ip in imgs:
                        jsons.append(content["shapes"])
                        json_imgs.append(ip)
        ...
        print("LabelmeDataset Size: ", len(self.imgs))
        # print("Dataset prepared")

    def preprocessLabels(self):
        """将数组转换为 numpy 格式，
        并添加 type 字段表明该区域为 mask 或者 keypoint
        """
        jsons = []
        for cur_label in self._jsons:
            nj = {}
    ...
    def __getitem__(self, idx):
        if not self.isTrain:
            img_path = osp.join(self._root, self.imgs[idx])   
            img = Image.open(img_path)
        ...
        mask = self._jsons[idx]
        print(mask)
        mask = np.array(mask)
        target = self.getTarget(idx, mask)

        img_path = osp.join(self._root, "JPEGImages", self.imgs[idx])
        img = Image.open(img_path).convert('L')
        img = np.array(img)
