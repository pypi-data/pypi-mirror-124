...
class BitmexOrder:
    ...
    def set_position_closed(self, symbol, dir, currentPrice=False):
        ...
        if self.show_slippage and self.props[symbol]['initialSL']:
            ...
            acc_balance_expected = self.entry_acc_balance[symbol] * (
                1 + (pnl_rr * acc_pos_size))
            acc_balance_real = self.entry_acc_balance[symbol] * (
                1 + (acc_balance_diff / self.entry_acc_balance[symbol]))

            # slippage is real acc balance change relative - executed pnl percent
            print("acc_balance_expected", acc_balance_expected, "acc_balance_real", acc_balance_real)
            print("Slippage: {}%".format(
                round((acc_balance_real - acc_balance_expected) / acc_balance_expected * 100, 2)), flush=True)
