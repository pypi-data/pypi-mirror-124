...
class _gencal:
    ...
    * 'tecim' = Time-dep TEC image specified in infile
    ...
    def __caltype_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-10.10s =\x1B[0m %s%-23s%s' % ('caltype',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __infile_inp(self):
        ...
        if self.__infile_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-7.7s =\x1B[0m %s%-23s%s' % ('infile',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __spw_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
    ...
    def __parameter_inp(self):
        ...
        self.__do_inp_output('%-10.10s = %s%-23s%s' % ('parameter',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __uniform_inp(self):
        ...
        if self.__uniform_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-7.7s =\x1B[0m %s%-23s%s' % ('uniform',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
