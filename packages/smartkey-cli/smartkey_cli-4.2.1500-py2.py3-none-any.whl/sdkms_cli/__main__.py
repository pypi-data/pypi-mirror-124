...
def audit_log(**kwargs):
    ...
    from_time = current_time - (int(days) * 3600 * 24)
    batch_size = 25
    if kwargs['batch_size'] is not None:
        if int(kwargs['batch_size'])>1000:
            raise ClientException("Batch-size can't be greater than 1000")
        else:
            batch_size=int(kwargs['batch_size'])
    start = 0
    if not kwargs['account_id']:
        raise ClientException("--account-id is required argument for audit-log command")
    ...
    f = open(filename, 'w')
    f.write('Severity,Type,Groups,IP-Address,Message,Actor,Time\n')

    print('Fetching logs and saving in file: ' + filename)
    while True:
        response = json_request('/sys/v1/logs?size='+str(batch_size)+'&from='+str(start)+'&range_from='+str(from_time)+'&range_to='+str(current_time),
                                exp_status=200, header=None, body=None, method='GET',
                                required_auth=RequiredAuth.USER)
    ...
    print("Fetching logs completed. The log file is " + filename)
    f.close()
...
'''.format(VERSION)
    ...
    parser_audit_log.add_argument("--account-id", help="account id of an account", required=True)
    parser_audit_log.add_argument("--days", help="No. of days", required=True)
    parser_audit_log.add_argument("--batch-size", help="Batch size for audit-log", required=False)
