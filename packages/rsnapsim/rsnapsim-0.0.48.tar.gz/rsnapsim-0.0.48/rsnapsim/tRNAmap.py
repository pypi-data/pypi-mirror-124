...
class tRNAmap():
    ...
    def get_codon_anticodon_dict(self,trna_gene_dict, alt_stopcodons = None, wobble_rules = None):
        bases = ['A','G','C','U']
        all_3mers = [''.join(i) for i in itertools.product(bases, repeat = 3)]
        
        bp_rules = {'A':'U', 'G':'C','C':'G','U':'A'}
        if wobble_rules ==None:
            bp_wobble_rules = {'A':['U','C','G','A'],
                                    'C':['G'],
                                    'G':['C','U'],
                                    'U':['A','G','C','U'],
                                    'I':['A','C','U']}    
        ...
        def generate_matches(self,codon,bp_rules, bp_wobble_rules):
        ...
        for i in range(len(all_3mers)):
            ...
            for anticodon in available_genes:
                if flipped:
                    matches = generate_matches(anticodon[::-1])
                    if codon in matches:
                        match_ids =match_ids + [available_genes.index(anticodon)]
                else:
                    matches =  generate_matches(anticodon)
                    if codon in matches:
                        match_ids =match_ids + [available_genes.index(anticodon)]
                codon_to_species[codon] = match_ids
                
            codon_to_species[codon] = match_ids
        return codon_to_species              
