...
class ContractAddressChangeWizard(models.TransientModel):
    ...
    old_contract_pon = fields.Char(
        compute='_compute_old_contract_pon',
    )
    old_contract_fiber_speed = fields.Char(
        compute='_compute_old_contract_fiber_speed',
    )
    ...
    def _compute_old_contract_pon(self):
        if self.contract_id.vodafone_fiber_service_contract_info_id:
            self.old_contract_pon = self.contract_id.vodafone_fiber_service_contract_info_id.vodafone_id  # noqa
        else:
            self.old_contract_pon = ""

    @api.depends('contract_id')
    def _compute_old_contract_fiber_speed(self):
        if (self.contract_id.vodafone_fiber_service_contract_info_id
            ...
            # (100Mb/600Mb/1Gb)
            self.old_contract_fiber_speed = self.contract_id.current_tariff_contract_line.product_id.attribute_value_ids[0].name  # noqa
        elif self.contract_id.xoln_fiber_service_contract_info_id:
            self.old_contract_fiber_speed = "600Mb"  # Greta fiber contracts have 600Mb
        else:
            self.old_contract_fiber_speed = ""
        ...
        broadband_isp_info = self.env["broadband.isp.info"].create(
            {
                ...
                "old_contract_pon": self.old_contract_pon,
                "old_contract_fiber_speed": self.old_contract_fiber_speed,
            }
        )
        line_params = {
