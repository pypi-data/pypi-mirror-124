...
class FireboltAdapter(SQLAdapter):
    ...
    def convert_agate_date_cols_to_text_cols(cls,agate_table) -> agate.Table:
        """
        Firebolt (or FB JDBC driver?) doesn't like datetime params,
        so we all Date type columns of the agate table to text first.
        """
    ...
    @available.parse_none
    def reformat_view_results(cls, agate_table, schema_relation) -> agate.Table:
        """
    ...
    @available.parse_none
    def stack_tables(cls, tables_list) -> agate.Table:
        """
