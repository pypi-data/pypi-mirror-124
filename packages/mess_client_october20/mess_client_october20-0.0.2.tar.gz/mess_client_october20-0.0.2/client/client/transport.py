...
# from packets.packet_client.client.common.utils import json, send_message, get_message
# упрощаем путь до модулей с "packets.packet_client.client.common.utils"
# на "common.utils" перед сборкой проекта:
...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        logger.debug('Запуск диалога авторизации. Starting auth dialog.')

        # # Посылаем серверу приветственное сообщение и получаем
        # # ответ что всё нормально или ловим исключение.
        # try:
        #     with socket_lock:
            ...
            try:
                ...
                if RESPONSE in ans:
                    if ans[RESPONSE] == 400:
                        ...
                        # авторизации.
                        ans_data = ans[DATA]
                        hashs = hmac.new(passwd_hash_string, ans_data.encode('utf-8'), 'MD5')
                        digest = hashs.digest()
                        my_ans = RESPONSE_511
    ...
    def contacts_list_update(self):
        ...
        """
        # перед обновлением очищаем список контактов
