...
def items_from_events(events: List[dict], pre_ordered=False) -> list:
    # TODO pre_ordered flag is temporary for correct working the orchestrator service.
    exclude = {'id', 'subtype', 'status', 'data', 'create_dt'}
    items_dict = {}
    if not pre_ordered:
        events = sorted(events, key=lambda x: x['update_dt'], reverse=True)
    for event in events:
...
def is_state_service_mock():
    """
    Возвращает значение переменной окружения STATE_SERVICE_MOCK.
    Необходимо для подмены значения STATE_SERVICE_MOCK в тестовых сценариях.
    Из-за инициализации при импорте подменить переменную окружения напрямую нельзя.
    """
    return STATE_SERVICE_MOCK
