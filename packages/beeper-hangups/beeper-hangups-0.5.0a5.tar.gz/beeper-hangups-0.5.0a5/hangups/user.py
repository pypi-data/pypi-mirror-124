...
class User:
    ...
    @staticmethod
    def from_user(user, self_user_id):
        """Construct user from ``Member`` message.

        Args:
            member: ``Member`` message.
            self_user_id (~hangups.user.UserID or None): The ID of the current
                user. If ``None``, assume ``status`` is the current user.

        Returns:
            :class:`~hangups.user.User` object.
        """

        is_self = (self_user_id == user.user_id.id) or (self_user_id is None)

        return User(user.user_id.id, user.name,  user.first_name,
                    user.avatar_url, user.email, [user.email], is_self)
...
class UserList:
    ...
    Args:
        ...
        self_entity: ``Member`` message for the current user.
        entities: List of known ``Member`` messages.
        conv_parts: List of ``ConversationParticipantData`` messages. These are
            used as a fallback in case any users are missing.

    """

    def __init__(self, client, self_id, members):
        self._client = client
        # {str: User}
        self._user_dict = {}
        # Add each entity as a new User.
        for member in members:
            user_ = User.from_user(member.user, self_id)
            self._user_dict[user_.id_] = user_
        self._self_user = self._user_dict[self_id]
    ...
    def get_self(self):
        """Gets the user of hangups.

        Returns:
            :class:`~hangups.user.user` of the connected user.
        """
    ...
    def _on_stream_event(self, event):
        if event.body.HasField('message_posted'):
            creator = event.body.message_posted.message.creator

            user_ = User.from_user(creator, self._self_user.id_)

            existing = self._user_dict.get(creator.user_id.id)
            if existing is None:
