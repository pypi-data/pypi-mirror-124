...
async def build_user_conversation_list(client):
    ...
    # Retrieve the hangups user's status
    get_user_self_info_response = await client.get_self_user_status(
        googlechat_pb2.GetSelfUserStatusRequest(
            request_header=client.get_gc_request_header(),
        )
    )
    self_status = get_user_self_info_response.user_status

    get_members_response = await client.get_members(
        googlechat_pb2.GetMembersRequest(
            request_header=client.get_gc_request_header(),
            member_ids=[googlechat_pb2.MemberId(
                user_id=googlechat_pb2.UserId(id=self_status.user_id.id)
            )]
        )
    )
    self_id = get_members_response.members[0].user.user_id.id

    # Retrieve entities participating in all conversations.
    items = await _sync_all_conversations(client)

    required_user_ids = set()
    for item in items:
        if item.group_id.HasField('dm_id'):
            required_user_ids |= {
                member.id
                for member in item.dm_members.members
            }
        elif item.group_id.HasField('space_id'):
            required_user_ids |= {
                name_user.id
                for name_user in item.name_users.name_user_ids
            }

    required_members = []
    if required_user_ids:
        logger.debug(f'Need to request {len(required_user_ids)} additional members:')
        ...
        try:
            response = await client.get_members(
                googlechat_pb2.GetMembersRequest(
                    request_header=client.get_gc_request_header(),
                    member_ids=[
                        googlechat_pb2.MemberId(
                            user_id=googlechat_pb2.UserId(id=id_)
                        )
                        for id_ in required_user_ids
                    ]
                )
            )
            required_members.extend(response.members)
        except exceptions.NetworkError as e:
            logger.warning('Failed to request missing users: {}'.format(e))

    # Build list of groups (conversations).
    groups = []
    for item in items:
        if item.group_id.HasField('dm_id'):
            # We will get a WorldItemLite for a dm that's been invited but does
            # not yet exist, so we need to check dm_members and make sure we
            # don't have any member with a state of invite.
            invited = [
                membership.membership_state == googlechat_pb2.MembershipState.MEMBER_INVITED
                for membership in item.dm_members.memberships
            ]
            if any(invited):
                logger.debug('dm is pending invite')
                continue

        group_response = await client.get_group(
            googlechat_pb2.GetGroupRequest(
                request_header=client.get_gc_request_header(),
                group_id=item.group_id,
                fetch_options=[
                    googlechat_pb2.GetGroupRequest.FetchOptions.MEMBERS,
                    googlechat_pb2.GetGroupRequest.FetchOptions.INCLUDE_DYNAMIC_GROUP_NAME,
                ],
            )
        )
    ...
    user_list = user.UserList(client, self_id, required_members)
    conversation_list = ConversationList(client, groups,
...
async def _sync_all_conversations(client):
    ...
    request = googlechat_pb2.PaginatedWorldRequest(
        ...
        fetch_options=[
            googlechat_pb2.PaginatedWorldRequest.FetchOptions.EXCLUDE_GROUP_LITE,
        ],
    )
    logger.info('Requesting conversations page')
    response = await client.paginated_world(request)
    items = list(response.world_items)

    logger.info('Synced %s total conversations', len(items))
...
class Conversation:
    ...
    def __init__(self, client, user_list, group, events=[],
                 event_cont_token=None):
        # pylint: disable=dangerous-default-value
        self._client = client  # Client
        self._user_list = user_list  # UserList
        self._conversation = group  # googlechat_pb2.Group
    ...
    def id_(self):
        ...
        return self.id_from_group_id(self._conversation.group_id)

    @property
    def users(self):
        """List of conversation participants (:class:`~hangups.user.User`)."""

        if self._conversation.group_type == googlechat_pb2.Group.GroupType.ROOM:
            members = self._conversation.name_users.name_user_ids
        else:
            members = self._conversation.group_read_state.joined_users

        return [self._user_list.get_user(part.id)
                for part in members]
    ...
    def last_modified(self):
        ...
        timestamp = self._conversation.sort_time
    ...
    def latest_read_timestamp(self):
        ...
        timestamp = self._conversation.group_read_state.last_read_time
        return parsers.from_timestamp(timestamp)
    ...
    def unread_count(self):
        return self._conversation.group_read_state.unread_message_count
    ...
    def is_quiet(self):
        ...
        level = self._conversation.group_read_state.notification_settings.state

        return level == googlechat_pb2.GroupNotificationSettings.GroupNotificationState.MUTED
    ...
    def _wrap_event(event_):
        ...
        if event_.body.HasField('message_posted'):
    ...
    async def send_message(self, text_body, image_file=None, image_id=None,
                           thread_id=None, local_id=None):
        ...
        Args:
            text_body: :str: The contents of the message to send.
            ...
            image_id: (optional) The image metadata from :meth:`upload_image`.
                If you specify both ``image_file`` and ``image_id`` together,
                ``image_file`` takes precedence and ``image_id`` will be ignored.
        ...
        async with self._send_message_lock:
            ...
            if image_file:
                try:
                    image_id = await self._client.upload_image(
                        image_file, self.id_.split(":")[1], filename=image_file_name,
                    )
                except exceptions.NetworkError as e:
            ...
            try:
                ...
                if thread_id:
                    request = googlechat_pb2.CreateMessageRequest(
                        request_header=self._client.get_gc_request_header(),
                        parent_id=googlechat_pb2.MessageParentId(
                            topic_id=googlechat_pb2.TopicId(
                                group_id=self._conversation.group_id,
                                topic_id=thread_id,
                            ),
                        ),
                        local_id=local_id,
                    ...
                    return await self._client.create_message(request)
                else:
                    ...
                    return await self._client.create_topic(request)
            except exceptions.NetworkError as e:
    ...
    async def update_read_timestamp(self, read_timestamp=None):
        ...
        if read_timestamp > self.latest_read_timestamp:
            ...
            timestamp = parsers.to_timestamp(read_timestamp)
            self._conversation.group_read_state.last_read_time = timestamp
            try:
                await self._client.mark_group_read_state(
                    googlechat_pb2.MarkGroupReadstateRequest(
                        request_header=self._client.get_gc_request_header(),
                        id=self._conversation.group_id,
...
class ConversationList:
    ...
    Args:
        ...
        groups: List of ``Group`` messages used to initialize the list of
            conversations.
    ...
    def __init__(self, client, groups, user_list, sync_timestamp):
        self._client = client  # Client
        self._conv_dict = {}  # {group_id: Group}
        ...
        for group in groups:
            self._add_conversation(group)

        self._client.on_stream_event.add_observer(self._on_stream_event)
    ...
    def get(self, conv_id):
        ...
        Args:
            group_id (googlechat_pb2.GroupId): ID of conversation to return.
        ...
        if not isinstance(conv_id, str):
            conv_id = Conversation.id_from_group_id(conv_id)
    ...
    def _add_conversation(self, group):
        """Add new conversation from googlechat_pb2.Group"""
        group = Conversation(self._client, self._user_list, group)
        logger.debug('Adding new conversation: {}'.format(group.id_))
        self._conv_dict[group.id_] = group
        return group

    async def _on_stream_event(self, event):
        if event.body.HasField('message_posted'):
    ...
    async def _get_or_fetch_conversation(self, group_id):
        """Get a cached conversation or fetch a missing conversation.

        Args:
            group_id: googlechat_pb2.GroupId, conversation identifier

        Raises:
            NetworkError: If the request to fetch the conversation fails.

        Returns:
            :class:`.Group` with matching ID.
        """
        group = self._conv_dict.get(Conversation.id_from_group_id(group_id), None)
        if group is None:
            logger.info('Fetching unknown conversation %s', group_id)
            group_response = await self._client.get_group(
                googlechat_pb2.GetGroupRequest(
                    request_header=self._client.get_gc_request_header(),
                    group_id=group_id,
                    fetch_options=[
            ...
            return self._add_conversation(group_response.group)
        else:
            return group

    async def _on_event(self, event_):
        """Receive a googlechat_pb2.Event and fan out to Conversations.

        Args:
            event_: googlechat_pb2.Event instance
        """
        group_id = event_.group_id

        try:
            conv = await self._get_or_fetch_conversation(group_id)
        except exceptions.NetworkError:
            logger.warning(
