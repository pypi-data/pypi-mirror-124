...
def calc_vidya(arr: np.ndarray, period) -> np.ndarray:
    ...
    cmo_arr = abs(np.divide((up_sum - down_sum), tot_sum, np.zeros_like(up_sum), where=tot_sum != 0))
    alpha = 2 / (1 + period)
...
# @jit
def calc_er(arr: np.ndarray, period, array: bool = False):
    if array:
    ...
    er = np.divide(direction, volatility, np.zeros_like(volatility), where=volatility != 0)
    return er
