...
class Episode(object):
    ...
    def rss_entry(self):
        ...
        if self.__alternate_media:
            alternate_enclosure = self.__alternate_media.rss_entry()
    ...
    @property
    def alternate_media(self):
        """Get or set the :class:`~pod2gen.AlternateMedia` object that is attached
        to this episode.
        ...
        commentary tracks or supporting source clips, etc.
            ...
            my_episode.alternate_media = am

        :type: :class:`~pod2gen.AlternateMedia`
        :RSS: ``<podcast:alternateEnclosure>``
        """
        return self.__alternate_media

    @alternate_media.setter
    def alternate_media(self, alternate_media):
        if alternate_media is not None:
            if not isinstance(alternate_media, AlternateMedia):
                raise TypeError(
                    "alternate_media must be an object of type AlternateMedia"
                )

            self.__alternate_media = alternate_media
