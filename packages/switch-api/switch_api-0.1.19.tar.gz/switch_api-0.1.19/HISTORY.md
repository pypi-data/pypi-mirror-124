...
## 0.1.19
## Updated
- To support the for use with ``QueueTask`` class of the ``pipeline`` module:
  - New option ``Signals`` added to the ``````MAPPING_ENTITIES`` literal
  - Update to ``register_task()`` method on the ``Automation`` class of the ``pipeline`` module
