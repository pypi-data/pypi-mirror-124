...
def get_parent_modules_list(api_inputs: ApiInputs, search_filter="*"):
    ...
    url = api_prefix + api_inputs.datacentre + "/" + api_inputs.api_project_id + "/LogicModules/" + search_filter
...
def get_clone_modules_list(api_inputs: ApiInputs, parent_module_id: uuid.UUID, search_filter="*"):
    ...
    url = api_prefix + api_inputs.datacentre + "/" + api_inputs.api_project_id + "/LogicModulesClones/" + \
        str(parent_module_id) + "/" + search_filter
    response = requests.request("GET", url, data=payload, timeout=20, headers=headers)
    response_status = '{} {}'.format(response.status_code, response.reason)
    if response.status_code != 200:
        logger.error("API Call was not successful. Response Status: %s. Reason: %s.", response.status_code,
...
def sensor_has_not_changed(api_inputs: ApiInputs, start_date: datetime.date, end_date: datetime.date,
                           sites: List[str] = None, templates: List[str] = None, hours_unchanged: int = 24):
    ...
    url = api_prefix + api_inputs.datacentre + "/" + api_inputs.db_name + "/" + api_inputs.api_project_id + \
        "/CreateAndRunAdxQuery/SensorHasNotChanged"
    response = requests.post(url, json=payload, headers=headers)
    response_status = '{} {}'.format(response.status_code, response.reason)
    if response.status_code != 200:
        logger.error("API Call was not successful. Response Status: %s. Reason: %s.", response.status_code,
