...
def build_pandoc_cmd(
    ...
    if template in AVAILABLE_TEMPLATES:
        ...
        template_path = os.path.join(cache_dir, f"{template}.tex")
        if ignore_cache or not os.path.exists(template_path):
    ...
    cmd.append(f"--metadata=dotmdc={cache_dir}")
...
def run_compile(
    ...
    if template in TEMPLATE_RESOURCES:
        ...
        for resc in TEMPLATE_RESOURCES[template]:
            resc_path = os.path.join(cache_dir, resc)
            if ignore_cache or not os.path.exists(resc_path):
    ...
    if output_file is None:
        ...
        build_dir = os.path.join(cache_dir, "_build")
        tex_file = os.path.join(build_dir, "main.tex")
        os.makedirs(build_dir, exist_ok=True)

        pandoc_cmd.append(f"--output={tex_file}")
        subprocess.run(pandoc_cmd, check=True)
        ...
        shutil.copyfile(os.path.join(build_dir, "main.pdf"), output_file)
    else:
...
def main():
    ...
    arg_parser.add_argument(
        ...
        metavar="DIRECTORY",
        help="directory for storing dependencies and temporary files",
    )
    arg_parser.add_argument(
        "--ignore-cache",
        action="store_true",
        help="don't use existing contents of the cache directory",
    )
    arg_parser.add_argument(
        "-f",
        "--from",
