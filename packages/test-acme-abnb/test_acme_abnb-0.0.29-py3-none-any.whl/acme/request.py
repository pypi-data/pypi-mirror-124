...
class Request:
    ...
    @http_exception
    def post(self, url: str, files: Optional[TextIO], headers: Dict = {}, data: Dict = {}):
        print('here, post')
        if self.validation is False:
            print('post, false')
            return self._requester_base.post(url, files=files, headers=headers, data=json.dumps(data), verify=False)
        else:
            return self._requester_base.post(url, files=files, headers=headers, data=json.dumps(data))
