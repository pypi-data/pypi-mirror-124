...
if __name__ == "__main__":
    fmt = "%d/%m/%Y %H:%M"
    _fmt = fmt.replace("%", "")
    parser = argparse.ArgumentParser("List zabbix item trend")
    ...
    res = client.get_item_trends(itemid, tini, tend)

    print("item.value; timestamp")
    for item in res:
        print(
            f"{item.value_min}; {item.value_avg}; {item.value_max}; {item.num}; {item.timestamp}"
        )
