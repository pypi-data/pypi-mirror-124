...
def dataframe_apply_patched(*args, **kwargs):
    ...
    with STATSD_CLIENT.timer(f"{pandas_string}df_apply", meta=meta, meta_hooks=meta_hooks):
...
def series_apply_patched(*args, **kwargs):
    ...
    with STATSD_CLIENT.timer(f"{pandas_string}series_apply", meta=meta, meta_hooks=meta_hooks):
