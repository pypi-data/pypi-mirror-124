...
class OidcClient(DBBASE):
    ...
    scopes = Column(
        ...
        info={
            'colanderalchemy': {
                'title': "Données mise à disposition de l'application",
                'description': "Au-delà de l'authentification, l'application "
                "peut demander à accéder à d'autres informations, cela doit "
                "être autorisée ici",
            }
        }
    )
    ...
    admin_email = Column(
        ...
        info={
            'colanderalchemy': {
                ...
                'description': "Les informations de connexion de l'application"
                " seront transmises directement à cette adresse",
            }
        }
    )
    logout_uri = Column(
        ...
        info={
            'colanderalchemy': {
                'title': "Url de déconnexion",
                'description': "Lorsqu'un utilisateur se déconnecte auprès du"
                " service d'authentification centralisée, cette adresse sera "
                "appelée pour assurer que les utilisateurs soient déconnectés "
                "de l'application seront transmises directement à cette "
                "adresse",
            }
        }
    )
    ...
    redirect_uris = relationship(
        ...
        info={
            'colanderalchemy': {
                ...
                "description": "Urls de redirection OpenId Connect utilisée "
                "pour renvoyer l'utilisateur vers l'application après "
                "authentification",
                "widget": deform.widget.SequenceWidget(
                    add_subitem_text_template="Ajouter une url de redirection"
                ),
            }
        }
    )
...
class OidcRedirectUri(DBBASE):
    ...
    uri = Column(
        ...
        nullable=False,
        unique=True,
        info={'colanderalchemy': {"title": "Url"}}
    )
