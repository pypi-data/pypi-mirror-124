...
def generate_api_kubeconfig(cluster, creds, kube_cluster_name):
    """ Generate the kubeconfig used by the api to post up whatever """
    with KubeClusterContext(cluster, creds, kube_cluster_name=kube_cluster_name) as kubeconfig:
        ...
        if cluster["cloud_provider"] == "GCP":
            ...
            parsed_yaml["users"][0]["user"]["auth-provider"] = {
                "name": "gcp-svc",
                "config": {
                    "service-acct": cluster["role_credentials"]["gcp"]["service_account_id"]
                },
            }
...
class KubeClusterContext:
    def __init__(self, cluster, creds, kube_cluster_name=""):
        ...
        self.init_kubeconfig(cluster, creds)

    def __enter__(self):
        self.stored_env = os.environ.get("KUBECONFIG", "")
        os.environ["KUBECONFIG"] = self.kubeconfig.name
    ...
    def init_kubeconfig(self, cluster, creds):
        env = os.environ.copy()
        env["KUBECONFIG"] = self.kubeconfig.name
