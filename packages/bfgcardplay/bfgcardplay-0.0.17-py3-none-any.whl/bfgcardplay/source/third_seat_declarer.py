...
class ThirdSeatDeclarer(ThirdSeat):
    ...
    def selected_card(self) -> Card:
        ...
        if (suit_to_develop and
            ...
            card = self.select_card_in_suit_to_develop(player, suit_to_develop, trick)
            return card
            ...
            if suit == manager.winning_suit:
                if manager.long_player == player.seat:
                    if player.partners_suit_cards[suit.name]:
                        ...
                        partners_suit_cards = player.partners_unplayed_cards[suit]
    ...
    def select_card_in_suit_to_develop(self, player: Player, suit: Suit, trick: Trick) -> Card:
        ...
        missing_queen = Card('Q', suit.name) in player.opponents_unplayed_cards[suit.name]
    ...
    def _get_winning_card(self, player: Player, trick: Trick) -> Union[Card, None]:
        ...
        if cards:
            ...
            if len(cards) > 1:
                ...
                if player.is_winner(our_cards[0]) and player.is_winner(our_cards[1]):
                    log(inspect.stack(), f'{cards[0]}')
                    return cards[0]
    ...
    def _select_card_if_void(self, player: Player, trick: Trick) -> Card:
            ...
            if player.suit_cards[suit_name]:
                ...
                if opponents_cards:
                    if our_cards[0].value < opponents_cards[0].value:  # TODO take into account length
                        log(inspect.stack(), f'{player.suit_cards[suit_name][-1]}')
        ...
        # Only trumps left
        if not suit_length:
            log(inspect.stack(), f'{player.trump_cards[-1]}')
            return player.trump_cards[-1]
        ...
        log(inspect.stack(), f'{player.suit_cards[long_suit][-1]}')
