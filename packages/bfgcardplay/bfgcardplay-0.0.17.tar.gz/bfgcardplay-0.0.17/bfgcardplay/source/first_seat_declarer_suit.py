...
class FirstSeatDeclarerSuit(FirstSeat):
    ...
    def _suits_to_develop(self, manager: object, controls: List[str]) -> List[str]:
        ...
        if entry:
            if suit_to_develop:
                manager.set_suit_to_develop(player.declarer, suit_to_develop)
    ...
    def _can_draw_trumps(self) -> bool:
        ...
        if short_hand.shape[3] <= 2:
            ...
            # if declarer_longer_trumps:
            #     short_suit_cards = player.get_unplayed_cards_by_suit(suit, player.dummy)
            # else:
            #     short_suit_cards = player.get_unplayed_cards_by_suit(suit, player.declarer)
        ...
        for suit_name, suit in SUITS.items():
            if suit != player.trump_suit:
                if controls[suit_name] - len(player.partners_suit_cards[suit_name]) <= 1:
                    ...
                    manager.set_suit_to_develop(player.declarer, Suit(suit_name))
        return [(suit_name, score) for suit_name, score in suit_scores.items()]
