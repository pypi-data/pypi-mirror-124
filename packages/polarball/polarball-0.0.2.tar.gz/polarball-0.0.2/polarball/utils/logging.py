...
polarball_dev_log_handler = handlers.TimedRotatingFileHandler(
...
polarball_dev_log_handler.setFormatter(_dev_formatter)
polarball_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

polarball_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

polarball_usr_log_handler.setLevel(logging.INFO)
...
polarball_usr_log_handler.setFormatter(_usr_formatter)
...
polarball_console_log_handler = logging.StreamHandler(sys.stdout)
polarball_console_log_handler.setFormatter(_console_formatter)
polarball_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    polarball_usr_log_handler.addFilter(warning_filter)
    polarball_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    polarball_usr_log_handler.removeFilter(warning_filter)
    polarball_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    polarball_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(polarball_dev_log_handler)

    log.addHandler(polarball_console_log_handler)

    log.addHandler(polarball_usr_log_handler)
