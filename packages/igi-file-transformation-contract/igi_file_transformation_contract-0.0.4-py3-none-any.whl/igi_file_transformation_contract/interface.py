...
class IFileTransformer(ABC):
    ...
    def get_default_output_path(self, in_path) -> str:
        directory, filename = os.path.split(in_path)
        output_filename = self.get_default_output_filename(input_filename=filename)
        return os.path.join(directory, output_filename)

    def get_default_output_filename(self, input_filename: str) -> str:
        base_name, _ = os.path.split(input_filename)
        output_filename = f"{base_name}_transformed{self.target_file_ext}"
