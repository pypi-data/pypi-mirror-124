...
class Admin(Vault):
    ...
    def __write_signing_key(self, public_key=None, private_key=None):
    ...
    def __generate_signing_key(self):
        ssh_loc = "/Users/%s/.ssh" % getpass.getuser()
        ...
        with open("%s/%s.pub" % (ssh_loc, key_name), 'r') as public_file:
            ...
            public_file.close()
        with open("%s/%s.pub" % (ssh_loc, key_name), 'r') as private_file:
            private_key = private_file.read().replace('\n', '')
            private_file.close()
        return {
