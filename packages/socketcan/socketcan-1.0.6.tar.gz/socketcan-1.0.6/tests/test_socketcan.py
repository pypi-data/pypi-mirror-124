...
def receive_from_can_raw_socket_with_timstamp(interface, q):
    """
    A helper function to receive a can frame from raw socket.
    :param interface: The interface name.
    :param q: The queue where to put the received data into.
...
class TestObjectCreation:
    ...
    def test_can_filter_creation(self):
        ...
        # add some kind of condition here
...
class TestCanRawSocket:
    ...
    def test_raw_message_with_timestamp(self):
        ...
        q = Queue()
        p = Thread(target=receive_from_can_raw_socket_with_timstamp, args=("vcan0", q,))
        p.setDaemon(True)
        p.start()
        time.sleep(1)
        s.send(frame1)
