...
class ClaimLineFocus:
    ...
    def _procedure_code_fallback(self) -> str:
        ...
        Currently, in the new mapping, those fields are:
            ndcCode                 (system ends with "/ndc_code")
            procedureCode           (system ends with "/HC")
            jurisdictionCode        (system ends with "/ER")
            hiecProductCode         (system ends with "/IV")
            abcCode                 (system ends with "/WK")
    ...
    def location_codes(self) -> List[str]:
        """REMOVE THIS METHOD
        Stephanie confirmed: only one place of service per claim line!"""
        try:
            return [
                cast(Coding, c).code
                for c in cast(
