...
class MyWindow(QtWidgets.QMainWindow):
	...
	def __init__(self):
		...
		path = os.path.join(os.path.dirname(__file__)).split(os.sep+'smgenerator')[0]
		uic.loadUi(path+os.sep+'smgenerator'+os.sep+'data'+os.sep+'gui.ui', self)	
		self.show()
	...
	def click_generator_networks(self):
		...
		try:
			...
			comm.send([parameters, NNETS], dest=1, tag=WORKTAG)

			# while True:
			# 	status = MPI.Status()
			# 	comm.recv(source=1, tag=MPI.ANY_TAG, status=status) ###
			# 	if status.Get_tag() == REACTIVATE: ###
			# 		self.generate_networks.setEnabled(True)
				# break

			req = comm.irecv(source=1, tag=REACTIVATE)
			activation = req.wait()
...
def master():
	while True:
		status = MPI.Status()
		im_free = comm.recv(source=0, status=status)
		...
		## if number of slaves are less than the number of models and models > 1
		if size-2>=len(nnets):

			for i in range(2, len(nnets)+2):
				inp = nnets.pop()
				comm.send([pars, inp], dest=i, tag=WORKTAG)

		## if number of slaves are equal or greater than number of models
		else: 
			...
			while len(nnets)>0:
				im_free = comm.recv(source=MPI.ANY_SOURCE, status=status)
				inp = nnets.pop()
				comm.send([pars, inp], dest=im_free, tag=WORKTAG)

		# comm.send(obj="w", dest=0, tag=REACTIVATE) ###
		activation = True
		req = comm.isend(activation, dest=0, tag=REACTIVATE)
		req.wait()
...
def slave():
	while True:
		status = MPI.Status()
		p = comm.recv(source=1, tag=MPI.ANY_TAG, status=status)	
		if status.Get_tag():
			break
		gen = Generator(p[0])
		gen.generateModel(p[1])
		comm.send(rank, dest=1, tag=10)
...
def interface():
	...
	app.exec_()
	try:
		comm.send(obj=None, dest=1, tag=DIETAG)
	except:
		pass
	sys.exit()
