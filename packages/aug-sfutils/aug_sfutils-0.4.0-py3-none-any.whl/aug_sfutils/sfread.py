...
class SFREAD:
    ...
    def getaddrlen(self):
        ...
        sfo = self.sfh['ADDRLEN']
        data = unpack('>H', sfo.val[:2])[0]
        logger.debug('getaddrlen addr:%d, len:%d, data:%d', sfo.address, sfo.length, data)
        logger.debug(sfo.descr)
        return data
    ...
    def getts06(self):
        ...
        for obj in self.parsets:
            if self.sfh[obj].obj_type == 3:
                logger.debug('getts06 Dev:%s n_chan:%d', obj, self.sfh[obj].n_chan)
                if self.sfh[obj].n_chan > 1: # git ???
                    break
