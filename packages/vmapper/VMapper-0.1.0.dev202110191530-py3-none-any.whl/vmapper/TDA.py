...
import sklearn
import TopGene.color
import TopGene.meta_func
...
class mapper():
	def __init__(self,
		...
		color_id_max = len(TopGene.color.color_dict)
	...
	def out_graph_graphviz(
		...
		for i in range(self.num_node):
			...
			G.node(str(i),label='',fillcolor='%s' % TopGene.color.color_dict[self.node_color_id[i_color]][0],width='0.075',shape='circle',style='filled')
		for i in range(self.num_node):
		...
		for i in range(self.num_node):
			...
			for k in range(num_meta):
				if meta_func[0,k] in meta_data[0]:
					...
					node_info += ',\"%s[%s]\":%s' % (meta_func[0,k],meta_func[1,k],TopGene.meta_func.add_meta_info(meta_func[0,k],meta_data_sec,meta_func[1,k],self.node_id_set[i]))
			node_info += '}},'
					...
					for k in range(num_meta):
						if meta_func[0,k] in meta_data[0]:
							...
							if meta_func[1,k][-4:] == 'RECODE':
								meta_func_name += '-RECODE'
							meta_data_sec = meta_data[1:,np.where(meta_data[0]==meta_func_name)[0][0]]
							edge_info += ',\"%s[%s]\":%s' % (meta_func[0,k],meta_func[1,k],TopGene.meta_func.add_meta_info(meta_func[0,k],meta_data_sec,meta_func[1,k],edge_idx_set))
					edge_info += '}},'
			...
			for k in range(num_meta):
				if meta_func[0,k] in meta_data[0]:
					...
					if meta_func[1,k][-4:] == 'RECODE':
						meta_func_name += '-RECODE'
					meta_data_sec = meta_data[1:,np.where(meta_data[0]==meta_func_name)[0][0]]
					meta_data_out_node[i+1,k+n_default] = TopGene.meta_func.add_meta_info(meta_func[0,k],meta_data_sec,meta_func[1,k],self.node_id_set[i],id_light=True)
		i_edge = 0
		for i in range(self.num_node):
			for j in range(i+1,self.num_node):
				if self.adjcy_mat[i,j] > 0:
					...
					for k in range(num_meta):
						if meta_func[0,k] in meta_data[0]:
							...
							if meta_func[1,k][-4:] == 'RECODE':
								meta_func_name += '-RECODE'
							meta_data_sec = meta_data[1:,np.where(meta_data[0]==meta_func_name)[0][0]]
							meta_data_out_edge[i_edge+1,k+n_default] = TopGene.meta_func.add_meta_info(meta_func[0,k],meta_data_sec,meta_func[1,k],edge_idx_set,id_light=True)
					i_edge += 1
	...
	def out_fig_cube_docomp(
		...
		for j in range(len(self.cube_min[i])):
			c = TopGene.color.color_dict[j][0]
	...
	def out_graph_graphviz_V(
		...
		for i in range(self.num_node):
			i_color = i%color_id_max
			G.node(str(i),label='',fillcolor='%s' % TopGene.color.color_dict[self.node_color_id[i_color]][0],width='0.05',shape='circle',style='filled')
		for i in range(self.num_node):
			for j in range(i+1,self.num_node):
				if self.adjcy_mat[i,j] > 0:
...
class smapper():
	...
	def out_png(
		...
		color_id_max = len(TopGene.color.color_dict)
		for i in range(self.num_node):
			G.node(str(i),label='',fillcolor='%s' % TopGene.color.color_dict[self.node_color_id[i]][0],width='0.075',shape='circle',style='filled')
		for i in range(self.num_node):
			for j in range(i+1,self.num_node):
				if self.adjcy_mat[i,j] > 0:
					G.edge(str(j),str(i),arrowhead='none',color='darkslategray')
			...
			for k in range(num_meta):
				if meta_func[0,k] in meta_data[0]:
					...
					if meta_func[1,k][-4:] == 'RECODE':
						meta_func_name += '-RECODE'
					meta_data_sec = meta_data[1:,np.where(meta_data[0]==meta_func_name)[0][0]]
					node_info += TopGene.meta_func.add_meta_info(meta_func[0,k],meta_data_sec,meta_func[1,k],self.node_id_set[i])
			node_info += '}},'
		node_info = node_info[:-1] + ']'
		edge_info = '\"edges\":['
		for i in range(self.num_node):
					...
					for k in range(num_meta):
						if meta_func[0,k] in meta_data[0]:
							...
							if meta_func[1,k][-4:] == 'RECODE':
								meta_func_name += '-RECODE'
							meta_data_sec = meta_data[1:,np.where(meta_data[0]==meta_func_name)[0][0]]
							edge_info += TopGene.meta_func.add_meta_info(meta_func[0,k],meta_data_sec,meta_func[1,k],edge_idx_set)
					edge_info += '}},'
		edge_info = edge_info[:-1] + ']'
		out_cyjs.write('{%s,\"elements\":{%s,%s}}' % (data_info,node_info,edge_info))
...
class dmapper(mapper):
	...
	def out_png(
		...
		for i in range(self.num_node):
			...
			G.node(str(i),label='',fillcolor='%s' % TopGene.color.color_dict[i_color][0],width='0.075',shape='circle',style='filled')
		for i in range(self.num_node):
			for j in range(self.num_node):
				if self.adjcy_mat[i,j] > 0 and i>j:
					i_color = np.where(flow_graph_node==self.node_flow_id[i])[0][0]%color_id_max + 1
					G.edge(str(i),str(j),dir='both',arrowtail='none',arrowhead='none',arrowsize='0.2',color='%s' % TopGene.color.color_dict[i_color][0])
...
def plot_flow(
	...
	for i in range(num_node):
		i_color = i%color_id_max
		G.node(str(i),label='%s' % (flow_data_name[i]),fillcolor='%s' % TopGene.color.color_dict[i+1][0],fontcolor='%s' % TopGene.color.color_dict[i+1][1],width='0.075',shape='square',style='filled')
	for i in range(num_node):
