...
class Marketplace:
    ...
    def get_my_onsale_domains(self, offset=0, options={}):
        if not options:
            options = "sortKey=acquiredAt&sortDirection=desc&limit=" + str(limit)
        url = Endpoint.MY_SALE_DOMAINS + f'{offset}?{options}'
        totalCount =  self.request.get(url)['domains']
        domains = []
        while len(totalCount) > 0:
            url = Endpoint.MY_SALE_DOMAINS + f'{offset}?{options}'
            res = self.request.get(url)
            totalCount = res['domains']
            [domains.append(i) for i in res['domains']]
            offset += 100
        return domains
