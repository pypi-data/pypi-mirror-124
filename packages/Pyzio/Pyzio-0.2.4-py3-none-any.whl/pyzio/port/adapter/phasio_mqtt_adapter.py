...
class PhasioMQTTAdapter(PhasioMQTTPort):
    ...
    def _connect(self, username: str) -> None:
        ...
        self._client_loop_start()

    def _thread_main(self):
        for port in self._mqtt_config.ports():
            self._client.connect_async(self._mqtt_config.host(), port, keepalive=5)
            try:
                self._logger.info(f'Connecting to broker on port {port}')
                self._client.loop_forever(retry_first_connection=False)
                break
            except Exception as e:
                if hasattr(e, 'message'):
                    self._logger.error(e.message)
                self._logger.info(f'Error connecting to broker on port {port}')
                continue

    def _client_loop_start(self):
