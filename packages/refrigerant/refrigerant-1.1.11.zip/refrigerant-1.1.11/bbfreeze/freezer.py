...
dont_copy_as_egg = {"bbfreeze", "PyXML"}
...
class MyModuleGraph(modulegraph.ModuleGraph):
    def _find_single_path(self, name, p, parent=None):
        ...
        m = zi.find_spec(fullname.replace(".", "/"))
        if m:
    ...
    def find_module(self, name, path, parent=None):
        ...
        for p in path:
            try:
                ...
                if found:
                    if res[2][2] == importlib.PKG_DIRECTORY:
                        append_if_uniq(res)
                else:
                    if res[2][2] == importlib.PKG_DIRECTORY:
                        append_if_uniq(res)
    ...
    def load_module(self, fqname, fp, pathname, *args):
        if type == 314:  # TODO we need a way to detect metadata
            ...
            return modulegraph.ModuleGraph.load_module(self, fqname, fp, pathname, *args)
...
"""
    ...
    importlib = __import__("importlib", level=0)
"""
    ...
    for p in sys.path:
        ...
        if not os.path.exists(f):
            continue
        sys.modules[__name__] = importlib.load_dynamic(__name__, f)
        found = True
        break
    if not found:
        try:
...
""" % (modname, modname, attr), name, "exec")
    ...
    def ensureRPath(self, exe):
        ...
        print("RPATH %r of %s needs adjustment. make sure you have the patchelf executable installed." % (
        current_rpath, exe))
    ...
    def _writecode(self, fn, mtime, code):
        ...
        data = importlib.get_magic() + struct.pack("<i", mtime) + marshal.dumps(code)
