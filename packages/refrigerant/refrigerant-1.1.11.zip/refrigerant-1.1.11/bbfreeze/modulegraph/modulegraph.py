...
def moduleInfoForPath(path, suffixes=impmach.all_suffixes()):
    for (ext, readmode, typ) in impmach.all_suffixes():
...
class ModuleGraph(ObjectGraph):
    ...
    def find_all_submodules(self, m):
        ...
        suffixes = [triple[0] for triple in importlib.get_suffixes()]
        for path in m.packagepath:
            try:
                ...
                continue
            for (path, mode, typ) in filter(None, map(moduleInfoForPath, names)):
                if path != '__init__':
                    yield path
    ...
    def load_module(self, fqname, fp, pathname, *args):
        ...
        packagepath = None
        typ, = args
        if typ == importlib.PKG_DIRECTORY:
            m = self.load_package(fqname, pathname)
            self.msgout(2, "load_module ->", m)
            return m
        if typ == importlib.PY_SOURCE:
            co = compile(fp.read() + '\n', pathname, 'exec')
            cls = SourceModule
        elif typ == importlib.PY_COMPILED:
            if fp.read(4) != importlib.get_magic():
                self.msgout(2, "raise ImportError: Bad magic number", pathname)
                raise ImportError("Bad magic number in %s" % pathname)
            fp.read(4)
            co = marshal.load(fp)
            cls = CompiledModule
        elif typ == importlib.C_BUILTIN:
            cls = BuiltinModule
            co = None
    ...
    def find_module(self, name, path, parent=None):
        ...
        if path is None:
            if name in sys.builtin_module_names:
                return (None, None, ("", "", importlib.C_BUILTIN))

            path = self.path

        try:
            stuff = importlib.util.find_spec(name, path)
