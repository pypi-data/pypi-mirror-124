...
def install(service):
    ...
    os.execvp('script',
        ['script', '-ec',
         'salt-call -V; salt-call grains.items; salt-call state.apply {}'.format(service),
          TYPESCRIPT_PATH])
...
@main.command(help='Watching Events')
@click.argument('node')
def events(node):
    """
    Watch the event bus by calling the state.event runner
    :param node: master or minion
    """
    if node not in ['master', 'minion']:
        return click.echo(f'Target master or minion!')

    os.chdir('/srv/odoopbx/salt')
    click.echo(f'Listening to {node}...')

    if node == 'master':
        sock_dir = salt.config.client_config('/srv/odoopbx/salt/master')['sock_dir']
    elif node == 'minion':
        sock_dir = salt.config.minion_config('/srv/odoopbx/salt/minion')['sock_dir']

    os.system(f'salt-run state.event node={node} pretty=True sock_dir={sock_dir}')
