...
from cursedspace import Application, Panel, ScrollPanel, InputLine, ShellContext
    ...
    def do_paint_item(self, y, x, maxwidth, is_selected, taskline):
        ...
        def print_element(y, x, maxwidth, element, align, extra):
            ...
            if cut_off:
                ...
                try:
                    self.win.addstr(y, x+maxwidth+1-len(self.app.overflow_marker[1]),
        ...
        if x < maxwidth:
            try:
                self.win.addstr(y, x, ' '*(maxwidth-x+1), self.app.color(baseattrs, is_selected))
            except:
                pass
...
class Selector(RemappedScrollPanel):
    ...
    def autoresize(self):
        ...
        maxheight, maxwidth = self.app.size()
        w = maxwidth//4 if maxwidth > 11 else maxwidth
        h = min(maxheight//2 if maxheight > 9 else maxheight, len(self.items)+2)
        self.resize(h, w)
    ...
    def do_paint_item(self, y, x, maxwidth, is_selected, item):
        ...
        label = self.indicator(is_selected) + self.number_prefix(item) + item
...
class SourceSelector(Selector):
    ...
    def do_paint_item(self, y, x, maxwidth, is_selected, item):
        attrs = self.app.color(common.SETTING_COL_NORMAL, is_selected)
        label = self.indicator(is_selected) + self.number_prefix(item) + item.displayname
        self.win.addstr(y, x, label[:maxwidth], attrs)
