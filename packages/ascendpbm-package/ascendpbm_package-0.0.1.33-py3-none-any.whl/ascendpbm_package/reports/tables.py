...
def add_rebate_dollarcol(df_claims, df_formulary, claim_ndc, claim_days, form_ndc='ndc',
                         form_id='formulary_id', rebate_flag='rebate_flag', specialty_flag='specialty',
                         days_flag='Days Supply > 84', cutoff=84, positive = 'Y',
                         rebate_amnt="Rebate Amount",
                         form_dict={'H.MNGD': ['REMEDYONE HIGHLY MANAGED FORMULARY', 117.86, 494.26, 1859.00],
                                   'MNGD': ['REMEDYONE MANAGED FORMULARY', 109.66, 475.25, 1836.19],
                                   'OPEN': ['REMEDYONE OPEN FORMULARY', 107.04, 432.57, 1255.54]
                                   }
                         ):
    '''
    Parameters
    ...
    form_id : str, optional
        ...
        The first element of each list in form_dict should match the unique values in this col.
    ...
    rebate_amnt : str, optional
        Highly readable column name for a created rebate $ amount float col rounded to two decimals.
        The default is "Rebate Amount".
    cutoff : int, optional
        Although an int, could be any numeric object comparable with > and <= to df_claims days supply col.
        The default is 84.
    positive : str, optional
        Although a string, could be anything that matches a positive indication in the formulary flags.
        The default is "Y".
    form_dict : dict, optional
        A dictionary for organizing rebate levels and rates.
        Assumes the keys map to the first element of the values and to the rebate logic section in the function itself,
        ...
        The default is {'H.MNGD': ['REMEDYONE HIGHLY MANAGED FORMULARY', 117.86, 494.26, 1859.00],
                        'MNGD': ['REMEDYONE MANAGED FORMULARY', 109.66, 475.25, 1836.19],
                        'OPEN': ['REMEDYONE OPEN FORMULARY', 107.04, 432.57, 1255.54]                                   }.
    ...
    rebates.loc[rebates[rebate_flag] == positive, 'Rebatable'] = 'Y'
    rebates.loc[rebates['Rebatable'].isnull(), 'Rebatable'] = 'N'
    ...
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'N')
                & (rebates[days_flag] == 'Y') & (rebates[form_id] == form_dict['H.MNGD'][0]),
                rebate_amnt] = form_dict['H.MNGD'][2]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'N')
                & (rebates[days_flag] == 'N') & (rebates[form_id] == form_dict['H.MNGD'][0]),
                rebate_amnt] = form_dict['H.MNGD'][1]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'Y')
                & (rebates[form_id] == form_dict['H.MNGD'][0]),
                rebate_amnt] = form_dict['H.MNGD'][3]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'N')
                & (rebates[days_flag] == 'Y') & (rebates[form_id] == form_dict['MNGD'][0]),
                rebate_amnt] = form_dict['MNGD'][2]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'N')
                & (rebates[days_flag] == 'N') & (rebates[form_id] == form_dict['MNGD'][0]),
                rebate_amnt] = form_dict['MNGD'][1]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'Y')
                & (rebates[form_id] == form_dict['MNGD'][0]),
                rebate_amnt] = form_dict['MNGD'][3]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'N')
                & (rebates[days_flag] == 'Y') & (rebates[form_id] == form_dict['OPEN'][0]),
                rebate_amnt] = form_dict['OPEN'][2]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'N')
                & (rebates[days_flag] == 'N') & (rebates[form_id] == form_dict['OPEN'][0]),
                rebate_amnt] = form_dict['OPEN'][1]
    rebates.loc[(rebates[rebate_flag] == 'Y') & (rebates[specialty_flag] == 'Y')
                & (rebates[form_id] == form_dict['OPEN'][0]),
                rebate_amnt] = form_dict['OPEN'][3]
    rebates.loc[rebates[rebate_amnt].isnull(), rebate_amnt] = 0.00
    rebates[rebate_amnt] = (0.95*rebates[rebate_amnt]).round(2)
    
    # Creating summary multiindex df with rebate amount sums and claim counts by above categories
    summary = rebates.groupby([form_id, rebate_flag, days_flag, specialty_flag]).agg(['sum', 'count'])[rebate_amnt]
    summary.rename(columns={'sum':'Projected Rebate Amount', 'count':'Claim Count'}, inplace=True)
    
    # Returning dict of both modified/expanded claims df by formulary and above summary object
    return {'claims': rebates, 'summary': summary}
