...
def _read_prob_img(img_name: str, index: int, cfg):
...
class Stitch:
    ...
    def _save_res(self, im_name, whole_prob, gt):
        ...
        utils.save_prob_map(im_name, whole_prob, self.cfg, self.cfg.TEST.STITCH_SAVE_DIR)
        utils.save_alpha_map(im_name, whole_prob, gt, self.cfg, self.cfg.TEST.STITCH_SAVE_DIR)

    def _sub_func_dilated(self, im_name):
        d = self.cfg.DATASET
        print("stitching: {}{}".format(im_name, d.IMG_EXTENSION))

        rows, cols, patch_size, win, m, n = _load_crop_info(im_name, self.cfg)
        ...
        gt_src = os.path.join(d.TEST_WHOLE_LABEL, im_name + d.IMG_EXTENSION)
        gt = cv2.imread(gt_src, cv2.IMREAD_GRAYSCALE) / 255

        self.evaluator.cal_batch_metric(im_name, whole_prob, gt)

        if self.cfg.TEST.STITCH_SAVE_RES:
    ...
    def run(self):
        if self.cfg.TEST.USE_DILATED_PREDICT:
            self.sub_func(self._sub_func_dilated)
        else:
            self.sub_func(self._sub_func_overlap)
