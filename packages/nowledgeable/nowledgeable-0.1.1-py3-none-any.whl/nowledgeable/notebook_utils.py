...
from .external_exercice_tools import abs_path
...
def prepare_notebook(teacher_path, student_path="", lang="en"):
    ...
    if os.path.exists(student_path):
        raise Exception(
            """Error: Student notebook file '{}' already exists. 
            Please remove the existing file from the folder or rename it
             before creating a new student notebook on this path.""".format(student_path))
    ...
    if any(map(lambda cell: (("image" in "".join(output["data"].keys())) if "data" in output else False for output in cell["outputs"]) if "outputs" in cell else False, teacher_notebook["cells"])):
        ...
        if os.path.isdir(files_path):
            raise Exception(
                """Error: Notebook files folder '{}' already exists. 
                    Please remove it from the path or rename it before creating 
                    a new student notebook in this location.""".format(files_path))
