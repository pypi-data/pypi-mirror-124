...
def zip_dir(start_dir, target_path):
    ...
    for dir_path, dir_names, file_names in os.walk(start_dir):
        ...
        # 禁止上传黑名单
        if f_path.startswith('/venv/'):
            continue
        if f_path.startswith('/.idea/'):
            continue
        if f_path.startswith('/.git/'):
            continue
...
#
#     ignore_file = os.path.join(os.path.abspath(os.curdir), '.teye-spider-ignore')
