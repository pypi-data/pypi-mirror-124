...
class RoughSetSelector(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        self.__reducts = []
        self.__y = y
        self.__n_classes = self.__y.n_classes
        ...
        if self.discrete:
            ...
            self.__nn = WeaklySupervisedKNeighborsClassifier(n_neighbors = self.n_neighbors,
                                      metric = self.metric)
        else:
    ...
    def __find_reducts(self):
        ...
        for k in range(self.__dim, 0, -1):
            for i in itertools.combinations(range(self.__dim), k):
                ...
                accuracy = accuracy_score(self.__nn_all.predict(self.X), self.__nn.predict(temp_X))
