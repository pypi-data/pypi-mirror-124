...
class DELIN(BaseEstimator, TransformerMixin, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        self.__n_classes = len(self.__y[0].n_classes)
        
        self.__y_prime = np.array(self.__X.shape[0], self.__n_classes)
        for i in range(len(self.__y)):
            self.__y_prime[i] = self.__y[i].to_probs
        ...
        for it in range(self.n_iters):
            ...
            for j in range(self.__n_classes):
                self.__Mu[j, :] = np.mean(np.multiply(self.__X, np.transpose([self.__y[:,j]]) ), axis=0)
            ...
            self.__y_prime = self.__clf.fit(X_prime, self.__y_prime).predict_proba(X_prime)
            self.__y_prime = self.__y_prime*self.__y
