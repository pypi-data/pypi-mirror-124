...
class MixinTraces:
    ...
    # TODO: Move `trace_mode` to a enum
    def trace(self, function: typing.Callable, *, trace_mode: typing.Literal['read', 'write', 'unset'] = 'write', trace_name: str = None, **kwargs) -> str:
...
class ContainerWidget(MixinWidget, MixinStateContainer):
    ...
    def init_container(self, *args, layout: typing.Optional[str] = AUTO, expand: bool = True, autogrow: bool = True, **kwargs):
        '''
        ...
        _existing_names = dir(self)
        widgets = self.setup_widgets(*args, **kwargs)
        if widgets is None:
            children = [w for _, w in self.children.items()]
            # logger.debug('tk C: %r', self.children)
            widgets = {}
            dir_names = {id(getattr(self, name, None)): name for name in dir(self) if name not in _existing_names}
        ...
        self.layout_container(layout, expand, autogrow)
        self.setup_defaults()
