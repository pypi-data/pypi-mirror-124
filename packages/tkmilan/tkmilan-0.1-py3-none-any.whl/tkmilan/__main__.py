...
from . import RootWindow, FrameUnlabelled, FrameLabelled, FrameStateful
# from .model import GuiState
from . import Button, Checkbox, Entry, Label, Listbox, Combobox, ComboboxMap, EntryMultiline, Notebook, NotebookUniform, Tree
...
MODULES_VERBOSE = (  # List spammy layouts
    'mixin.layout',
    'model.layout',
)
...
class NB_Child_Simple(FrameUnlabelled):
    def __init__(self, *args, layout='Rx,1', **kwargs):
...
class TextEditor(FrameStateful):
    ...
    def txt_clicked(self, tag, tag_id):
        logger.debug(f'Clicked on {tag} {tag_id}')
...
class RW(RootWindow):
    def setup_widgets(self):
        ...
        self.lbs = LuftBaloons(self, layout='xE')
        self.ubox = UpstreamBool(self, label='Upstream', labelAnchor=CP.S, layout='rx,1',
                                 what_bool=vc)
        ...
        self.txt = TextEditor(self, layout='Rx,1')
        self.nb = NB(self)
    ...
    def debug(self):
        ...
        for line in pformat(self.txt.txt.gstate).splitlines():
            pass  # logging.info('  %s', line)
        # self.set_gui_state(GuiState(enabled=True, valid=True))
        logging.info('=> NB')
