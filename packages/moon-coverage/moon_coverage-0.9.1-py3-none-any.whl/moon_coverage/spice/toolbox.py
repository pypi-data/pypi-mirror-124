...
def sc_state(time, spacecraft, target, abcorr='NONE'):
    ...
    Returns
    -------
    (float, float, float, float, float, float) or np.ndarray
...
def intersect_pt(time, observer, target, frame, ray, limb=False,
    ...
    method: str, optional
        ...
        (See NAIF :func:`sincpt` for more details).
    ...
    --------
    spiceypy.sincpt
    moon_coverage.spice.limb.limb_ip_pt
...
def illum_angles(time, observer, target, pt, abcorr='NONE', method='ELLIPSOID'):
    ...
    if not np.isnan(np.max(pt)):
        *_, p, i, e = sp.ilumin(method, str(target), time,
                                target.frame, abcorr, str(observer), pt)
    else:
