...
from ..esa import ESA_CREMAS
from ..events.event import Event, EventsList, EventWindow
...
class TourConfig:
    ...
    mk: kernels.MetaKernel or str, optional
        Metakernel filename or ESA CReMA key.
        You can provide your own or use the one provided by ESA based
        on the spacecraft selected.

        For example: ``'5.0'`` (default) with the spacecraft ``JUICE``
        will load ``juice_crema_5_0.tm`` metakernel.

        Currently, ESA CReMA keyword are only implemented for ``JUICE``
        but more ESA missions could be added upon request.
        See ``moon_coverage.esa.crema`` for more details.
    ...
    version: str, optional
        ...
        with a key/shortcut and available in ``ESA_CREMAS``.

    kernels_dir: str or pathlib.Path, optional
        Kernels directory location.

        This parameter is only used to subsitute the ``$KERNELS``
        symbol value at runtime in the provided metakernel.
        This parameter has no effect on the :py:attr:`kernels` parameter.
    ...
    exclude: EventWindow or EventsList
        Event window or list of events to exclude from the analysis.

    Raises
    ------
    ValueError
        If the metakernel is provided as a key/shortcut but the associated
        spacecraft is not available in ``ESA_CREMAS``.
    ...
    you could provide a :py:class:`MetaKernel` object with the substituted values
    in the initial call:
    ...
    def __init__(self, mk='5.0', kernels=None,
        ...
        # Kernel setup
        self.mk = mk, version, kernels_dir, download_kernels
        self._add(kernels)
        ...
        return f'<{self.__class__.__name__}> ' + ' | '.join(
            filter(
                ...
                [
                    ...
                    f'Metakernel: {self.mk}' if self.mk else None,
    ...
    def mk(self, args):
        ...
        """
        mk, version, kernels_dir, download_kernels = args

        if not isinstance(mk, (type(None), MetaKernel)):

            # Defined metakernels keyword arguments
            kwargs = {'download': download_kernels}

            if kernels_dir is None:
                kernels_dir = os.environ.get(f'KERNELS_{self.spacecraft}')
            ...
            try:
                ...
                raise KernelRemoteNotFoundError(
                    ...
                    'You can provide directly a `moon_coverage.MetaKernel` object '
    ...
    def _parse(self, times):
        ...
        Method used in :py:func:`__getitem__` to query a trajectory
        on a temporal grid.
        ...
        times: str, int, float, slice, list, Event or EventWindow
            ...
            No change is performed in this method (convertion to ET
            is done in the :py:class:`Trajectory` initialization).
            ...
            If no explicit ``step`` is provided, the :py:attr:`default_time_step`
            (provided in :py:func:`__init__`) will be used; if an ``int`` is
            provided, it will correspond to the total number of points (evenly
            space in time).

            - ``list``, ``tuple``, ``numpy.array`` are only modified
            when an :py:class:`Event` or an :py:class:`EventWindow`
            is provided (converted to ET later).

            - :py:class:`Event` will extract the event time (single value).

            - :py:class:`EvenWindow` will extract the start and stop time of
            the event window (the :py:attr:`default_time_step`)
        ...
        if isinstance(times, (tuple, list, np.ndarray, EventsList)):
    ...
    def _parse_slice(self, t_slice):
        ...
        if start in [None, 'start', 'beg', 'begin', 'begining']:
    ...
    def get_flybys(self, event='all', alt_min=25_000):
        ...
        Parameters
        ----------
        event: events.EventWindow or events.EventsList, optional
    ...
    def flybys(self):
        ...
        See Also
        --------
        :py:func:`get_flybys` if you need a different minimal altitude.
    ...
    def flyby(self, approx_ca_date, *dt, alt_min=25_000):
        ...
        approx_ca_date: float, string or numpy.datetime64
            ...
            :py:class:`Event`, :py:class:`Eventwindow` and
            :py:class:`EventList` can be used as well.
        ...
        *dt: tuple(s), optional
        ...
        Returns
        -------
        moon_coverage.trajectory.SpacecraftFlyby or
        moon_coverage.trajectory.InstrumentFlyby
            If an :py:attr:`instrument` was provided.

            If an :py:class:`Eventwindow` or an
            :py:class:`EventList` is provided, the output
            will be a list of the flybys found in these intervales.
        ...
        elif isinstance(approx_ca_date, (list, EventsList)):
            return [
