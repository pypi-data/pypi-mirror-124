import os, re, logging, subprocess, time, shlex, json
...
try:
  ...
  class docker_images:
    ...
    def list(self):
      ...
      return [docker_image(id) for id in subprocess.check_output(lscmd).decode('ascii').strip('').split()]
    def get(self,id):
...
def __build__(stage, already_tagged, build_arg_dict, items, checked_versions, jobpath=None):
  ...
  if not ok:
    logging.error('Failed to build '+stage['name'])
    return False, []
