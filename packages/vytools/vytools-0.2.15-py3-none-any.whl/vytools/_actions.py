import logging, json, copy, subprocess
...
def image_name_label(hash,images):
  return 'image:'+(','.join(images[hash]['tags']) if hash in images else hash)
  ...
  for hid in [False, True]:
    for k in sorted_image_list:
      ...
      if ok and images[k]['has_image_dependencies'] == hid:
        ...
        nobody_depends_on = ' ' if not len(depended_on) == 0 else '*'
        depends_on_nobody = '-' if not hid else '*' if len(depends_on) == 0 else ' '
        tags = label.replace('image:','')
        details = json.loads(subprocess.check_output(['docker','inspect',k]).decode('utf-8'))
        image_list[label] = {
          'name':tags,
          'thingtype':'image',
          'not_vy': not hid,
        ...
        vytools.printer.print_def(prefx+'|{}|{}|'.format(depends_on_nobody,nobody_depends_on) + tags)
...
def info(original_list=[], items=None, list_dependencies=False, list_private=False, list_images=False, expand=False):
  ...
  if not original_list:
    ...
    if list_images:
      ...
      results['images'] = image_info([],list_dependencies,'    ')
  return results
