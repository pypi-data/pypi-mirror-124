...
class DSLibrary(object):
    ...
    def load_dataframe(self, resource_name: (str, io.RawIOBase, typing.Iterable), **kwargs):
        ...
        def sizer():
            with use_opener(resource_name, mode='rb') as f_r:
                if not hasattr(f_r, "seek") or not f_r.seekable():
                    return
                f_r.seek(0, 2)
                return f_r.tell()
        # FIXME detect non-seekable stream and inhibit sampling/sizing
        # sniff format
        ...
        if read_method:
            ...
            with use_opener(uri, mode='rb', **open_args) as f_r:
                ...
                try:
                    ...
                    raise DSLibraryDataFormatException(f"Invalid character encoding: {err.reason}: encoding={err.encoding}, offset={err.start} - set 'encoding' or 'encoding_errors' options to compensate", offset=err.start)
