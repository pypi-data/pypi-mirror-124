...
class _SARModel(ComplexParamsMixin, JavaMLReadable, JavaMLWritable, JavaModel):
    """
    Args:
        activityTimeFormat (object): Time format for events, default: yyyy/MM/dd'T'h:mm:ss
        ...
        coldStartStrategy (object): strategy for dealing with unknown or new users/items at prediction time. This may be useful in cross-validation or production scenarios, for handling user/item ids the model has not seen in the training data. Supported values: nan,drop.
        finalStorageLevel (object): StorageLevel for ALS model factors.
        implicitPrefs (bool): whether to use implicit preference
        intermediateStorageLevel (object): StorageLevel for intermediate datasets. Cannot be 'NONE'.
        itemCol (object): column name for item ids. Ids must be within the integer value range.
        ...
        ratingCol (object): column name for ratings
        regParam (float): regularization parameter (>= 0)
        seed (long): random seed
        similarityFunction (object): Defines the similarity function to be used by the model. Lift favors serendipity, Co-occurrence favors predictability, and Jaccard is a nice compromise between the two.
        startTime (object): Set time custom now time if using historical data
        startTimeFormat (object): Format for start time
        supportThreshold (int): Minimum number of ratings per item
        timeCol (object): Time of activity
        timeDecayCoeff (int): Use to scale time decay coeff to different half life dur
        userCol (object): column name for user ids. Ids must be within the integer value range.
        userDataFrame (object): Time of activity
    """

    activityTimeFormat = Param(Params._dummy(), "activityTimeFormat", "Time format for events, default: yyyy/MM/dd'T'h:mm:ss")
    ...
    coldStartStrategy = Param(Params._dummy(), "coldStartStrategy", "strategy for dealing with unknown or new users/items at prediction time. This may be useful in cross-validation or production scenarios, for handling user/item ids the model has not seen in the training data. Supported values: nan,drop.")
    
    finalStorageLevel = Param(Params._dummy(), "finalStorageLevel", "StorageLevel for ALS model factors.")
    ...
    intermediateStorageLevel = Param(Params._dummy(), "intermediateStorageLevel", "StorageLevel for intermediate datasets. Cannot be 'NONE'.")
    
    itemCol = Param(Params._dummy(), "itemCol", "column name for item ids. Ids must be within the integer value range.")
    ...
    similarityFunction = Param(Params._dummy(), "similarityFunction", "Defines the similarity function to be used by the model. Lift favors serendipity, Co-occurrence favors predictability, and Jaccard is a nice compromise between the two.")
    
    startTime = Param(Params._dummy(), "startTime", "Set time custom now time if using historical data")
    
    startTimeFormat = Param(Params._dummy(), "startTimeFormat", "Format for start time")
    ...
    timeCol = Param(Params._dummy(), "timeCol", "Time of activity")
    ...
    userCol = Param(Params._dummy(), "userCol", "column name for user ids. Ids must be within the integer value range.")
