...
class StratifiedRepartition(ComplexParamsMixin, JavaMLReadable, JavaMLWritable, JavaTransformer):
    ...
    Args:
        ...
        mode (object): Specify equal to repartition with replacement across all labels, specify original to keep the ratios in the original dataset, or specify mixed to use a heuristic
        seed (long): random seed
    """

    labelCol = Param(Params._dummy(), "labelCol", "The name of the label column")
    
    mode = Param(Params._dummy(), "mode", "Specify equal to repartition with replacement across all labels, specify original to keep the ratios in the original dataset, or specify mixed to use a heuristic")
