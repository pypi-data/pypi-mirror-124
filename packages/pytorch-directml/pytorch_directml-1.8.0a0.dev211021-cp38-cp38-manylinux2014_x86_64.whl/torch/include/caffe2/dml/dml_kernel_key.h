...
struct DmlInputTensorKey {
  ...
  //c10::variant<Tensor, TensorShapeAndType> tensor;
...
struct DmlKernelKey {
  ...
  dml::small_vector<DmlInputTensorKey, 6> input_tensors;

  DmlKernelKey Clone() const;  // Performs a deep copy
  bool operator==(const DmlKernelKey& other) const;
};

uint64_t DmlKernelKeyHash(const DmlKernelKey& k);
}  // namespace dml

// Template specialization of std::hash for DmlKernelKey
template <>
struct std::hash<dml::DmlKernelKey> {
  size_t operator()(const dml::DmlKernelKey& k) const {
    return DmlKernelKeyHash(k);
  }
};