...
def build_CVs(CV_list, parmed_structure, system):
    ...
    #       cv[8] = list of inital distances (CN only)
    ...
    for cv in CV_list:
        ...
        if cv[0] == 'Weighted Distances':
            ...
            group_list = []
            for k, receiver_group in enumerate(cv[1]):
                group_list.append(wd_force.addGroup([int(i) for i in receiver_group]))
            # for donor_group in cv[2]:
                group_list.append(wd_force.addGroup([int(i) for i in cv[2][k]]))
            print('wdist group list',group_list)
        ...
        if cv[0] == 'Coordination Number':
            ...
            # for donor_group in cv[2]:
                group_list.append(cn_force.addGroup([int(i) for i in cv[2][k]]))
