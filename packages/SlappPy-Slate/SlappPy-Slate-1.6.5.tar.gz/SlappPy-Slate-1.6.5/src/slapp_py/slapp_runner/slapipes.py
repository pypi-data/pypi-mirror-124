...
slapp_loop = True
slapp_process: Optional[Process] = None
...
def kill_slapp():
    global slapp_loop, slapp_write_queue, slapp_process
    if slapp_process:
        slapp_loop = False
...
async def _run_slapp(slapp_path: str, mode: str, restart_on_fail: bool = True):
    global slapp_loop, slapp_process

    while True:
        slapp_process = await asyncio.create_subprocess_shell(
        ...
        await asyncio.gather(
            _read_stderr(slapp_process.stderr),
            _read_stdout(slapp_process.stdout),
            _write_stdin(slapp_process.stdin)
        )
        if restart_on_fail:
