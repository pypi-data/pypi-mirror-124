...
class Selector(object):
    ...
    def find_map(self, map, one=True, data=None, target_type=None, **kwargs):
        return self._query_from_map(self.find, map, one=one, data=data, target_type=target_type, **kwargs)
    ...
    def xpath_map(self, query_map, one=True, namespaces=None, **kwargs):
        return self._query_from_map(self.xpath, query_map, one=one, namespaces=namespaces, **kwargs)
    ...
    def css_map(self, query_map, one=True):
        return self._query_from_map(self.css, query_map, one=one)
    ...
    def re_map(self, regex_map, one=True, replace_entities=True):
        return self._query_from_map(self.re, regex_map, one=one, replace_entities=replace_entities)
    ...
    def match_map(self, map: dict, one=True, **kwargs):
        ...
        for key, value in map.items():
            if isinstance(value, str):
                data[key] = self._match(value, **kwargs)
            elif isinstance(value, dict):
                data[key] = self.match_map(value, **kwargs)
            else:
                print(f'Warning ... query not support {type(value)}')

        if not one:
            data_list = self._get_data_list(data)
            return data_list
        else:
            return data

    def _match(self, value, **kwargs):
        try:
            return self.re(value, **kwargs)
        except:
            try:
                return self.css(value)
            except:
                return self.xpath(value, **kwargs)

    def _query_from_map(self, func, map: dict, one=True, **kwargs):
        data = {}
        for key, value in map.items():
            if isinstance(value, str):
                r = func(value, **kwargs)
                if one:
                    ...
                    v = [d for d in r.extract() if d.strip()]
                    if not v: v = []
                data[key] = v.strip()
            elif isinstance(value, dict):
                data[key] = self._query_from_map(func, value, one=one, **kwargs)
            else:
                print(f'Warning ... query not support {type(value)}')
