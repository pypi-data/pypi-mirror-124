...
class HyperdataApi:
    ...
    def create_inference_result(self, table_name: str, object_name: str, columns: List[str]) -> int:
        ...
        table_list = self.hyperdata_client.get_dataobject_tables(
            auth=self.hyperdata_auth, datasource_id=default_datasource_id
        )
        ...
        do_info_list = self.hyperdata_client.get_do_list_by_datasource_id(
            auth=self.hyperdata_auth,
            datasource_id=self.hyperdata_client.get_default_datasource_id(auth=self.hyperdata_auth),
        )
