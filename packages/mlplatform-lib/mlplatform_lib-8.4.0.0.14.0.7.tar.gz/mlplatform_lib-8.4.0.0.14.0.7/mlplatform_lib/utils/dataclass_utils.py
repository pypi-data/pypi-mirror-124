    ...
    for key, val in dict.items():
        ...
        if key in sig.parameters.keys():
            ...
            # regard as typing hint
                ...
                hasattr(sig.parameters[key].annotation, "__origin__") and
                sig.parameters[key].annotation.__origin__ == list
            ):
                if is_dataclass(sig.parameters[key].__args__):
                    init_dict[key] = [
                        from_dict(sig.parameters[key].__args__, elem) for elem in val
                    ]
                else:
                    init_dict[key] = val
