...
class SQLAlchemy:
    ...
    async def _setup(self, instance: SQLAlchemyDBConfig = None, options: Dict = {}):
        (
            ...
            instance.connection_uri_plain_schema,
            instance.engine,
    ...
    async def _create_engine(self, options: Dict = {}) -> Tuple[str, str, str, AsyncEngine, Engine]:
    ...
    async def _setup_engine_logging(
        ...
        if options.get("is_production", False):
            severity1 = logging.INFO if options.get("developer", False) else logging.WARN
            severity2 = logging.DEBUG if options.get("developer", False) else logging.INFO
    ...
    @asynccontextmanager
    async def session_scope(
        self, auto_flush: bool = True, expire_on_commit: bool = True, execution_options: Dict = None
    ) -> AsyncGenerator[AsyncSession, None]:
    ...
    async def _session_scope_generator(
        self, auto_flush=True, expire_on_commit=True, execution_options: Dict = None
    ) -> AsyncGenerator[AsyncSession, None]:
        kwargs = {}
        if execution_options is not None:
            kwargs.update({"execution_options": execution_options})
