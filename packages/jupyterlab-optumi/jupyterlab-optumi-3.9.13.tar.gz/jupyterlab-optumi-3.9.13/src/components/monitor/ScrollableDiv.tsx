...
export class ScrollableDiv extends React.Component<IProps, IState> {
    ...
    private getFormattedContents = (): JSX.Element => {
		...
		if (source.length == 0) {
			return (<span style={{padding: '20px', display: 'inline-block', color: 'var(--jp-ui-font-color1)'}}>No output to display.</span>);
		} else {
            const lines: string[] = [];
            ...
            return (
                <AnsiLink style={{padding: '20px', display: 'inline-block', color: 'var(--jp-ui-font-color1)'}} key={lines[0]} text={ansiText} />
            );
        }
        return (<></>);
		// return content;
    ...
    public render = (): JSX.Element => {
        ...
        return (
            <div style={{overflow: 'auto', width: '100%', height: '100%', whiteSpace: 'pre'}} ref={this.element}>
