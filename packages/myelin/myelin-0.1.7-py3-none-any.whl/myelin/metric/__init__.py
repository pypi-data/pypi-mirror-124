...
def post_hpo_result(config_id, config, budget, loss_metric, loss, info_map):
    __MYELIN_CLIENT__.post_hpo_result(config_id, config, budget, loss_metric, loss, info_map)


def publish_result(loss_value, loss_metric):
    __MYELIN_CLIENT__.post_update(loss_metric, loss_value)
    if myelin.hpo.hpo_enabled():
        myelin.hpo.publish_result(loss_metric, loss_value)
