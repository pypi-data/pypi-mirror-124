...
class Dataset(DataSource):
    ...
    def sample(self, filename, safe=True, fd=None):
        ...
        if filename.lower().endswith('.csv'):
            ...
            df = pd.read_csv(fd, nrows=10, compression='gzip', error_bad_lines=False)
        elif filename.lower().endswith('.tsv.gz'):
            df = pd.read_csv(fd, nrows=10, sep='\t', compression='gzip',
                             error_bad_lines=False)
        elif filename.lower().endswith('.tsv'):
            df = pd.read_csv(fd, nrows=10, sep='\t')
        elif filename.lower().endswith('.pq') or filename.lower().endswith(".pq.sample"):
