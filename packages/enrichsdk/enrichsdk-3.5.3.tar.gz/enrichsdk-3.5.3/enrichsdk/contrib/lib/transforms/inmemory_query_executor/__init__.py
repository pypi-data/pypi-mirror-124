...
class InMemoryQueryExecutorBase(Compute):
    ...
    def update_frame(self, name, engine, sql, df, dependencies=[]):
        ...
        for col in df.columns:
            columns[col] = {
                'touch': self.name,
                'description': self.get_column_description(name, col),
                'datatype': df.get_generic_dtype(col),
            }
    ...
    def generate_paramsets(self, spec, start_date, end_date):
        ...
        if duration == 'day':
            ...
            if diff.days > 90:
                raise Exception("Date range should be < 90 days")
    ...
    def get_specs(self):
        ...
        Specification: A list of dictionaries. Each dict has
          ...
          * categories: String or a list of strings indicating specification groups
             to which the spec belongs
          * segment: How to split the dataframe resulting from query execution. Could
             be none ('complete' as the default name), string (column name) or
             a callback that generates a { name: df } map
          * paramsets_duration: each instance for one 'day' or a window
             of days (defined below)
          * paramsets_window: each instance translates into date range
             for each instance of parameters. The
