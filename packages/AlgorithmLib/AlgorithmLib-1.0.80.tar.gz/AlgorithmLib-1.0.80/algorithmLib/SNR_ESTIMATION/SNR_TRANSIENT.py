...
def cal_snr_transient(refFile=None, noisetFile=None, testFile=None):
    ...
    teststruct,testsamplerate,_ = get_data_of_ctypes_(noisetFile)
    noiseStruct,noisesamplerate,_ = get_data_of_ctypes_(testFile)
    ...
    snr,noise,err = c_double(0.0),c_double(0.0),c_double(0.0)
    mydll.SNR_transient(byref(refstruct),byref(noiseStruct),byref(teststruct),c_double(refsamplerate),byref(snr),byref(noise),byref(err))
