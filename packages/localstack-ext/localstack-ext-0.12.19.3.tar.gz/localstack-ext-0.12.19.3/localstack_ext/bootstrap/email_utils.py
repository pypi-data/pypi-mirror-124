...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DVzmH()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DVzmQ(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DVzmO,from_name=DVzmO,smtp_host=DVzmO,smtp_user=DVzmO,smtp_pass=DVzmO,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DVzmq(recipients,DVzml)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DVzmO):
 ...
 result["From"]=formataddr((DVzmQ(Header(from_name,"utf-8")),from_email))
 return result
