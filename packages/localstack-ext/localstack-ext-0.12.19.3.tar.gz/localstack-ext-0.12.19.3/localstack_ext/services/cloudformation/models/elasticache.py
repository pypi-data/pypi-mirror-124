...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=OKwMg(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not OKwMB
   return result
  except OKwMS:
   props=self.props
   ...
   if parts[0]in candidates and OKwMV(parts)>1:
