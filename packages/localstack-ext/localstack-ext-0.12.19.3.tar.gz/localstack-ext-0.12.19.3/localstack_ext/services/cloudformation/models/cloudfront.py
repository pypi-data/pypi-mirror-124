...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or TcWNA
 @TcWNh
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return TcWNP(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @TcWNE
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if TcWNH(origins,TcWNm)else origins.get("Items")
  origins_to_create=TcWNQ([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if TcWNH(aliases,TcWNm)else aliases.get("Items")
  aliases_to_create=TcWNQ(aliases)
  for distr in distrs:
   origins=TcWNQ([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=TcWNQ(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",TcWNB)
   config["Enabled"]=TcWNf(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if TcWNH(custom_errors,TcWNI)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=TcWNf(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":TcWNB}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=TcWNf(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=TcWNA):
    parent=config if parent is TcWNA else parent
    items=parent.get(key)or[]
    if TcWNH(items,TcWNI)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":TcWNy(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if TcWNH(cache_behavs,TcWNI)else cache_behavs)
