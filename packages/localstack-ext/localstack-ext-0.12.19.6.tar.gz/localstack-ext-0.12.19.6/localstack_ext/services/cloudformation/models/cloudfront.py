...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or yREeX
 @yREeY
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return yREew(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @yREeT
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if yREeU(origins,yREeg)else origins.get("Items")
  origins_to_create=yREet([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if yREeU(aliases,yREeg)else aliases.get("Items")
  aliases_to_create=yREet(aliases)
  for distr in distrs:
   origins=yREet([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=yREet(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",yREed)
   config["Enabled"]=yREea(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if yREeU(custom_errors,yREeh)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=yREea(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":yREed}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=yREea(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=yREeX):
    parent=config if parent is yREeX else parent
    items=parent.get(key)or[]
    if yREeU(items,yREeh)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":yREeu(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if yREeU(cache_behavs,yREeh)else cache_behavs)
