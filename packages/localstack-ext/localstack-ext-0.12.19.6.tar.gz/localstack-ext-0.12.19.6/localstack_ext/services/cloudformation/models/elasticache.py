...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=MXIzV(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not MXIzY
   return result
  except MXIzK:
   props=self.props
   ...
   if parts[0]in candidates and MXIzv(parts)>1:
