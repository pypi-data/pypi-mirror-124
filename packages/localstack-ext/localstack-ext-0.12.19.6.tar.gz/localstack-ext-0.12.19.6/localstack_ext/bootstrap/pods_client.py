...
class PodInfo:
 def __init__(self,name=CsILj,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(CsILe):
 BACKEND="_none_"
 def __init__(self,pod_name=CsILj,config=CsILj):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise CsILl("Not implemented")
 def push(self,comment:CsILf=CsILj)->PodInfo:
  raise CsILl("Not implemented")
 def pull(self):
  raise CsILl("Not implemented")
 def commit(self,message:CsILf=CsILj):
  raise CsILl("Not implemented")
 def inject(self,version:CsILF,reset_state:CsILo):
  raise CsILl("Not implemented")
 def list_versions(self)->List[CsILf]:
  raise CsILl("Not implemented")
 def version_info(self,version:CsILF):
  raise CsILl("Not implemented")
 def set_version(self,version:CsILF,inject_version_state:CsILo,reset_state:CsILo,commit_before:CsILo):
  raise CsILl("Not implemented")
 def list_version_commits(self,version:CsILF)->List[CsILf]:
  raise CsILl("Not implemented")
 def get_commit_diff(self,version:CsILF,commit:CsILF)->CsILf:
  pass
 ...
 def get(cls,pod_name,pre_config=CsILj):
  ...
  raise CsILl('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=CsILp
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=CsILH)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=CsILH
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise CsILl("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @CsILS
 def get_state_zip_from_instance(get_content=CsILp):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise CsILl("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:CsILf=CsILj):
...
class CloudPodManagerCPVCS(CloudPodManager):
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=CsILj)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def init(self):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def inject(self,version:CsILF,reset_state:CsILo):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=CsILH)
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:CsILf=CsILj)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=CsILH)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(CsILB(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=CsILB(zip_data_content)
  return result
 def pull(self):
  ...
  rm_rf(zip_path)
 @CsILS
 def presigned_url(pod_name:CsILf,mode:CsILf)->CsILf:
  ...
  if response.status_code>=400:
   raise CsILl("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  if res.status_code>=400:
   raise CsILl("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:CsILf=CsILj):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=CsILH)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise CsILl('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=CsILt(self,"clone_dir",CsILj)
...
class PodConfigManagerMeta(CsILK):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=CsILj
   for manager in cls.CHAIN:
    try:
     tmp=CsILt(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif CsILQ(tmp,CsILR)and CsILQ(result,CsILR):
       result.extend(tmp)
    except CsILl:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not CsILj:
    return result
   raise CsILl('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(CsILe,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @CsILT
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise CsILl('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(CsILe):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=CsILH).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=CsILi([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=CsILp):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except CsILl:
   if safe:
    return{}
   raise CsILl('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(CsILe):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise CsILl("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise CsILl("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:CsILf,pre_config:Dict[CsILf,CsILf],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def commit_state(pod_name:CsILf,pre_config:Dict[CsILf,CsILf],message:CsILf=CsILj,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:CsILf,version:CsILF,reset_state:CsILo,pre_config:Dict[CsILf,CsILf],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:CsILf,pre_config:Dict[CsILf,CsILf],**kwargs)->List[CsILf]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:CsILF,pod_name:CsILf,pre_config:Dict[CsILf,CsILf],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def set_version(version:CsILF,inject_version_state:CsILo,reset_state:CsILo,commit_before:CsILo,pod_name:CsILf,pre_config:Dict[CsILf,CsILf],**kwargs)->CsILo:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:CsILF,pod_name:CsILf,pre_config:Dict[CsILf,CsILf])->List[CsILf]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:CsILF,commit:CsILF,pod_name:CsILf,pre_config:Dict[CsILf,CsILf])->CsILf:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=CsILj,squash_commits=CsILp,comment=CsILj,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,**kwargs):
 pre_config=kwargs.get("pre_config",CsILj)
 if not pod_name:
  raise CsILl("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def reset_local_state(reset_data_dir=CsILp):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise CsILl("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->CsILf:
 try:
...
def get_persisted_resource_names(data_dir)->List[CsILf]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and CsILB(os.listdir(entry.path))>0:
    names.append(entry.name)
