...
SENT_EMAILS=[]
EMAIL_BLACKLIST=sjVHT()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",sjVHX(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=sjVHM,from_name=sjVHM,smtp_host=sjVHM,smtp_user=sjVHM,smtp_pass=sjVHM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if sjVHp(recipients,sjVHd)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=sjVHM):
 ...
 result["From"]=formataddr((sjVHX(Header(from_name,"utf-8")),from_email))
 return result
