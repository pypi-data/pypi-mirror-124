...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DRqQB()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DRqQj(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DRqQf,from_name=DRqQf,smtp_host=DRqQf,smtp_user=DRqQf,smtp_pass=DRqQf,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DRqQM(recipients,DRqQc)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DRqQf):
 ...
 result["From"]=formataddr((DRqQj(Header(from_name,"utf-8")),from_email))
 return result
