...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or EVxHA
 @EVxHP
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return EVxHg(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @EVxHM
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if EVxHd(origins,EVxHS)else origins.get("Items")
  origins_to_create=EVxHz([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if EVxHd(aliases,EVxHS)else aliases.get("Items")
  aliases_to_create=EVxHz(aliases)
  for distr in distrs:
   origins=EVxHz([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=EVxHz(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",EVxHj)
   config["Enabled"]=EVxHy(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if EVxHd(custom_errors,EVxHm)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=EVxHy(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":EVxHj}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=EVxHy(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=EVxHA):
    parent=config if parent is EVxHA else parent
    items=parent.get(key)or[]
    if EVxHd(items,EVxHm)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":EVxHO(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if EVxHd(cache_behavs,EVxHm)else cache_behavs)
