...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tIpha(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tIphY
   return result
  except tIphm:
   props=self.props
   ...
   if parts[0]in candidates and tIphb(parts)>1:
