...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or yMtlA
 @yMtla
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return yMtlK(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @yMtlc
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if yMtlU(origins,yMtlX)else origins.get("Items")
  origins_to_create=yMtlT([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if yMtlU(aliases,yMtlX)else aliases.get("Items")
  aliases_to_create=yMtlT(aliases)
  for distr in distrs:
   origins=yMtlT([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=yMtlT(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",yMtlq)
   config["Enabled"]=yMtlx(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if yMtlU(custom_errors,yMtlI)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=yMtlx(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":yMtlq}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=yMtlx(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=yMtlA):
    parent=config if parent is yMtlA else parent
    items=parent.get(key)or[]
    if yMtlU(items,yMtlI)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":yMtls(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if yMtlU(cache_behavs,yMtlI)else cache_behavs)
