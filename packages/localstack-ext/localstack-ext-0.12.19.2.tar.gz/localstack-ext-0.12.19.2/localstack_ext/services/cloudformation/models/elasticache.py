...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=SPysj(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not SPysX
   return result
  except SPysr:
   props=self.props
   ...
   if parts[0]in candidates and SPysh(parts)>1:
