...
SENT_EMAILS=[]
EMAIL_BLACKLIST=EYelz()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",EYelc(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=EYeli,from_name=EYeli,smtp_host=EYeli,smtp_user=EYeli,smtp_pass=EYeli,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if EYelH(recipients,EYelp)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=EYeli):
 ...
 result["From"]=formataddr((EYelc(Header(from_name,"utf-8")),from_email))
 return result
