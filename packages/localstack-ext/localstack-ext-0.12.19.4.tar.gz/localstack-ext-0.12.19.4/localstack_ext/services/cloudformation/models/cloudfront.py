...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or EgPqQ
 @EgPqk
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return EgPqV(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @EgPqn
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if EgPqG(origins,EgPqx)else origins.get("Items")
  origins_to_create=EgPqC([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if EgPqG(aliases,EgPqx)else aliases.get("Items")
  aliases_to_create=EgPqC(aliases)
  for distr in distrs:
   origins=EgPqC([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=EgPqC(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",EgPqy)
   config["Enabled"]=EgPqw(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if EgPqG(custom_errors,EgPqY)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=EgPqw(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":EgPqy}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=EgPqw(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=EgPqQ):
    parent=config if parent is EgPqQ else parent
    items=parent.get(key)or[]
    if EgPqG(items,EgPqY)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":EgPqO(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if EgPqG(cache_behavs,EgPqY)else cache_behavs)
