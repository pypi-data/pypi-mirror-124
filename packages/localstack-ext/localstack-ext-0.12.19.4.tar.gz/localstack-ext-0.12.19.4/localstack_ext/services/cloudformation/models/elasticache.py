...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=KXNAp(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not KXNAI
   return result
  except KXNAO:
   props=self.props
   ...
   if parts[0]in candidates and KXNAS(parts)>1:
