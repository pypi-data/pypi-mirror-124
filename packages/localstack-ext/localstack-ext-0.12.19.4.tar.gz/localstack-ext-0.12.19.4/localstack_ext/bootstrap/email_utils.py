...
SENT_EMAILS=[]
EMAIL_BLACKLIST=kljqT()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",kljqY(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=kljqI,from_name=kljqI,smtp_host=kljqI,smtp_user=kljqI,smtp_pass=kljqI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if kljqv(recipients,kljqO)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=kljqI):
 ...
 result["From"]=formataddr((kljqY(Header(from_name,"utf-8")),from_email))
 return result
