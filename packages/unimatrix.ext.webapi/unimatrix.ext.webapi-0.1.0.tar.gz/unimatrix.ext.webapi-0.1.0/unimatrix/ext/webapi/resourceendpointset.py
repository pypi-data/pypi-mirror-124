...
class ResourceEndpointSet:
    ...
    def add_to_router(cls,
        ...
        url_params = []
        if parent:
            url_params.insert(0, parent.path_parameter)
        ...
        for attname, obj in list(cls.__dict__.items()):
            ...
            if obj.action.detail:
                path = f'{base_path}/{{{cls.path_parameter}}}'

                # Check if the path parameter name is not the same as the
                # parent class
                if parent and parent.path_parameter == cls.path_parameter: # pragma: no cover
                    raise ValueError(
            ...
            router.add_api_route(
                ...
                response_model=getattr(obj, 'response_model', None),
            )
        ...
        for action in cls.valid_actions:
            ...
            router.add_api_route(
                path,
                cls._create_annotated_handler(
                    cls, action, handler, parent=parent, detail=is_detail),
                methods=[ ACTION_METHODS[action] ],
                response_model=getattr(handler, 'response_model', None),
            )
        ...
        for subresource_class in cls.subresources:
            subresource_class.add_to_router(
                router, f'{base_path}/{{{cls.path_parameter}}}/{subresource_class.name}', # pylint: disable=line-too-long
                parent=cls,
    ...
    @staticmethod
    def _update_signature(action, handler, protected=False):
    ...
    @staticmethod
    def _create_annotated_handler(view_class, action, handler, parent, detail):
