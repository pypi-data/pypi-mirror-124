...
from ..optimization import Project, Evaluation, Progress, OptimizationProblem
from ..optimization.design import DesignValues, DesignSpace, DesignParameter
from ..optimization.design.continuity import Continuous, Discrete
from ..optimization.goals import Minimum, Maximum
from ..optimization.kpi import KpiValues, KpiSpace, KpiParameter
from ..optimization.parameter import ParameterValue, Parameter
from ..optimization.project_status import Ready, Done
...
class ProjectApiClient(object):
    ...
    def finish_project(
        ...
        self.__session.authenticated_request(
            path='project/finish', version='v2', contains_sensitive_data=False, data=data)

    def analyze_project(self, project_id: str, only_pareto_optimal: bool = False) -> Dict[str, bool]:
        data = {'project_id': project_id, 'onlyParetoOptimal': only_pareto_optimal}
        response = self.__session.authenticated_request(
            path='project/analyze', version='v2', contains_sensitive_data=False,
            data=data)
        return {data['evaluation_id']: data['isParetoOptimal'] for data in response['evaluations']}

    def plan_evaluation(self, project_id: str, desiredQuantity: int) -> List[Evaluation]:
        data = {
            'project_id': project_id,
            'desiredQuantity': desiredQuantity,
        }
        response = self.__session.authenticated_request(
            path='evaluations/plan', version='v2', contains_sensitive_data=False,
            data=data)
    ...
    def show_evaluations(self, evaluation_ids: List[str]) -> List[dict]:
        response = self.__session.authenticated_request(
            path='evaluations/show', version='v2', contains_sensitive_data=False,
            data=evaluation_ids)
    ...
    def submit_evaluation(self, evaluation: Evaluation):
        ...
        self.__session.authenticated_request(
            path='evaluations/submit', version='v2', contains_sensitive_data=False, data=data)

    def analyze_evaluations(self, project_id: str,
                            only_pareto_optimal: bool = False) -> List[Evaluation]:
        analyzation = self.analyze_project(project_id=project_id,
                                           only_pareto_optimal=only_pareto_optimal)
        evaluation_ids = [*analyzation]
        evaluations = self.show_evaluations(evaluation_ids)
        return [self.__map_evaluation(data, analyzation[data['id']]) for data in evaluations]
    ...
    def track_progress(self, project_id):
        ...
        response = self.__session.authenticated_request(
            path='project/progress/track', version='v2', contains_sensitive_data=False,
            data=data)
        return Progress(
    ...
    def __map_project(self, project_data) -> Project:
        ...
        problem = self.__map_problem(design=project_data['design_definition'], target=project_data['target_definition'])
    ...
    def __map_problem(self, design, target) -> OptimizationProblem:
        ...
        for param in design.values():
            ...
            design_params.append(DesignParameter(name=name, uuid=name, minimum=minimum, maximum=maximum, continuity=continuity))

        for param in target.values():
            name = param['name']
            goal = Minimum() if param['goal'] == 'minimize' else Maximum()
            target_params.append(
                KpiParameter(name=name, uuid=name, goal=goal))
    ...
    def __map_evaluation(self, data, is_pareto_optimal: bool = False) -> Evaluation:
        ...
        evaluation = Evaluation(uuid=uuid, design=design_values, kpis=kpis, is_pareto_optimal=is_pareto_optimal)
        return evaluation

    def __map_parameter_values(self, params) -> List[ParameterValue]:
        return [ParameterValue(parameter=Parameter(name=param['name'], uuid=param['name']), value=param['value']) for param in params.values()]
