...
def make_plugin(
    ...
    def process_inbox():
        ...
        if providers:
            category = body['metadata']['labels']['opentestfactory.org/category']

            if category in providers:
                _dispatch_providercommand(plugin, providers[category], body)
            else:
                plugin.logger.warning(f'Ignoring unknown {name} category "{category}".')
        elif provider:
