...
class VirtualSwitchToMachineDisconnectCommand(object):
    def __init__(self, pyvmomi_service, port_group_configurer, resource_model_parser):
        """
    ...
    def disconnect_from_networks(
        ...
        for vm_network_remove_mapping in vm_network_remove_mappings:
            ...
            if vnic is None:
                raise KeyError(
                    "VNIC having MAC address {0} not found on VM having UUID {1}".format(
                        vm_network_remove_mapping.mac_address, vm_uuid
                    )
                )
    ...
    def disconnect(
        self, si, logger, vcenter_data_model, vm_uuid, network_name=None, vm=None
    ):
        """
        ...
        if network:
            ...
            return self.port_group_configurer.disconnect_all_networks(
                vm, default_network, vcenter_data_model.reserved_networks, logger=logger
            )
