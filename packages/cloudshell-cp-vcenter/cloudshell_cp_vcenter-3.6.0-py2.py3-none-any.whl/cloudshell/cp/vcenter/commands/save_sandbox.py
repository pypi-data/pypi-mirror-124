...
class SaveAppCommand:
    ...
    def save_app(
        ...
        logger.info(
            "Save Sandbox command starting on " + vcenter_data_model.default_datacenter
        )

        if not save_app_actions:
            raise Exception("Failed to save app, missing data in request.")

        actions_grouped_by_save_types = groupby(
            save_app_actions, lambda x: x.actionParams.saveDeploymentModel
        )
        ...
        if not error_results:
            ...
            results = self._execute_save_actions_using_pool(
                artifactSaversToActions, cancellation_context, logger, results
            )
    ...
    def _execute_save_actions_using_pool(
        self, artifactSaversToActions, cancellation_context, logger, results
    ):
        save_params = []
        destroy_params = []
        ...
        for artifactSaver in list(artifactSaversToActions.keys()):
            save_params.extend(
                self._get_save_params(
                    artifactSaver, artifactSaversToActions, cancellation_context, logger
                )
            )
            destroy_params.extend(
                self._get_destroy_params(artifactSaver, artifactSaversToActions)
            )
        ...
        if self.cs.check_if_cancelled(cancellation_context):
            logger.info(
                "[{0}] Save sandbox was cancelled, rolling back saved apps".format(
                    thread_id
                )
            )
            results = self._rollback(
                destroy_params, logger, results, results_before_deploy, thread_id
            )

        elif operation_error:
            logger.error(
                "[{0}] Save Sandbox operation failed, rolling backed saved apps. See logs for more information".format(
                    thread_id
                )
            )
            results = self._rollback(
    ...
    def _rollback(
        self, destroy_params, logger, results, results_before_deploy, thread_id
    ):
    ...
    def _get_save_params(
        self, artifactSaver, artifactSaversToActions, cancellation_context, logger
    ):
        return [
            (artifactSaver, a, cancellation_context, logger)
            for a in artifactSaversToActions[artifactSaver]
        ]
    ...
    def _save(self, save_params):
        ...
        try:
            return artifactSaver.save(
                save_action=action, cancellation_context=cancellation_context
            )
