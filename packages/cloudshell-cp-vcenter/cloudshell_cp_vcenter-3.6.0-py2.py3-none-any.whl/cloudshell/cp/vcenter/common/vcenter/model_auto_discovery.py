...
from cloudshell.cp.vcenter.models.QualiDriverModels import (
    AutoLoadAttribute,
    AutoLoadDetails,
)
...
ATTRIBUTE_NAMES_THAT_ARE_SLASH_BACKSLASH_AGNOSTIC = [
    DEFAULT_DVSWITCH,
    DEFAULT_DATACENTER,
    VM_LOCATION,
    VM_STORAGE,
...
class VCenterAutoModelDiscovery(object):
    ...
    def validate_and_discover(self, context):
        ...
        resource.attributes = (
            VCenterAutoModelDiscovery._make_attributes_slash_backslash_agnostic(
                resource.attributes
            )
        )
        ...
        if not si:
            error_message = (
                "Could not connect to the vCenter: {0}, with given credentials".format(
        ...
        try:
            ...
            for key, value in list(resource.attributes.items()):
                ...
                validation_method(
                    si, all_items_in_dc, auto_attr, dc_name, resource.attributes, key
                )
    ...
    def _validate_datacenter(self, si, all_item_in_vc, auto_att, attributes):
        dc = self._validate_attribute(
            si, attributes, vim.Datacenter, DEFAULT_DATACENTER
        )
        if not dc:
    ...
    def _validate_default_dvswitch(
        self, si, all_items_in_vc, auto_att, dc_name, attributes, key
    ):
    ...
    def _check_if_vcenter_user_pass_valid(self, context, session, attributes):
        ...
        connection_details = self._get_connection_details(
            session, attributes[PASSWORD], attributes[USER], context.resource.address
        )
        try:
            si = self.pv_service.connect(
            ...
            raise ValueError(
                "could not connect to the vcenter: {0}, with the given credentials ({1})".format(
                    connection_details.host, connection_details.username
                )
            )
        return si

    def _validate_vm_storage(
        self, si, all_items_in_vc, auto_att, dc_name, attributes, key
    ):
        accepted_types = (vim.Datastore, vim.StoragePod)
        datastore = self._validate_attribute(
            si, attributes, accepted_types, key, dc_name
        )
        if not datastore:
    ...
    def _validate_saved_sandbox_storage(
        self, si, all_items_in_vc, auto_att, dc_name, attributes, key
    ):
        accepted_types = (vim.Datastore, vim.StoragePod)
    ...
    def _validate_vm_resource_pool(
        self, si, all_items_in_vc, auto_att, dc_name, attributes, key
    ):
        if key not in attributes or not attributes[key]:
            return

        cluster = self._validate_vm_cluster(
            si, all_items_in_vc, auto_att, dc_name, attributes, VM_CLUSTER
        )
    ...
    def _validate_holding_network(
        self, si, all_items_in_vc, auto_att, dc_name, attributes, key
    ):
        holding_network = self._validate_attribute(
            si, attributes, vim.Network, key, dc_name
        )
    ...
    def _check_if_attribute_not_empty(attributes, name):
        if not (
            (hasattr(attributes, name) and getattr(attributes, name))
            or (
    ...
    def get_full_name(dc_name, managed_object, name=""):
        ...
        return VCenterAutoModelDiscovery.get_full_name(
            dc_name, managed_object.parent, curr_path
        )
