class VnicToNetworkMapper(object):
    ...
    def map_request_to_vnics(
        self, requests, vnics, existing_network, default_network, reserved_networks
    ):
        """
        ...
        vnics_to_network_mapping = self._map_vnic_to_network(
            vnics, existing_network, default_network, reserved_networks
        )
        for request in requests:
            if request.vnic_name:
                if request.vnic_name not in vnics_to_network_mapping:
                    raise ValueError(
                        'No vNIC by that name "{0}" exist'.format(request.vnic_name)
                    )
                ...
                if self.quali_name_generator.is_generated_name(net_at_requsted_vnic):
                    raise ValueError(
                        'The vNIC: "{0}" is already set with: "{1}"'.format(
    ...
    def _map_vnic_to_network(
        self, vnics, existing_network, default_network, reserved_networks
    ):
        mapping = dict()
        for vnic_name, vnic in list(vnics.items()):
            network_to_map = ""
            if hasattr(vnic, "backing"):
                if hasattr(vnic.backing, "network") and hasattr(
                    vnic.backing.network, "name"
                ):
                    network_to_map = vnic.backing.network.name
                elif hasattr(vnic.backing, "port") and hasattr(
                    vnic.backing.port, "portgroupKey"
                ):
                    network_to_map = self._get_network_name_from_key(
                        vnic.backing.port.portgroupKey,
                        existing_network,
                        default_network,
                    )
            ...
            if (
                not self.quali_name_generator.is_generated_name(network_to_map)
                and network_to_map not in reserved_networks
            ):
