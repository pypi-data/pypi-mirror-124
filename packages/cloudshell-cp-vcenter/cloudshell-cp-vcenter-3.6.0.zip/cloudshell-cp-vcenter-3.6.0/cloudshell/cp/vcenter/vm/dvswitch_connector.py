...
class VirtualSwitchToMachineConnector(object):
    def __init__(self, dv_port_group_creator, virtual_machine_port_group_configurer):
        """
        ...
        self.virtual_machine_port_group_configurer = (
            virtual_machine_port_group_configurer
        )

    def connect_by_mapping(
        self,
        si,
        vm,
        mapping,
        ...
        logger.debug(
            "about to map to the vm: {0}, the following networks".format(
                vm.name if vm.name else vm.config.uuid
            )
        )
        ...
        return self.virtual_machine_port_group_configurer.connect_vnic_to_networks(
            vm, request_mapping, default_network, reserved_networks, logger
        )
