...
class VirtualSwitchConnectCommand:
    ...
    def connect_to_networks(
        ...
        default_network_instance = self.pv_service.get_network_by_full_name(
            si, default_network_name
        )
        ...
        mappings = self._prepare_mappings(
            dv_switch_name=dv_switch_name, vm_network_mappings=vm_network_mappings
        )
    ...
    def _prepare_mappings(self, dv_switch_name, vm_network_mappings):
        ...
        for vm_network_mapping in vm_network_mappings:
            vm_network_mapping.dv_port_name = (
                self.dv_port_group_name_generator.generate_port_group_name(
                    dv_switch_name,
            ...
            vm_network_mapping.vlan_id = self.vlan_id_range_parser.parse_vlan_id(
                vm_network_mapping.vlan_spec, vm_network_mapping.vlan_id
            )

            vm_network_mapping.vlan_spec = self.vlan_spec_factory.get_vlan_spec(
                vm_network_mapping.vlan_spec
            )
