...
class ConnectionCommandOrchestrator(object):
    ...
    def connect_bulk(self, si, logger, vcenter_data_model, request):
        ...
        self.logger.debug(
            "Apply connectivity changes has started with the requet: {0}".format(
                request
            )
        )
        ...
        if vcenter_data_model.reserved_networks:
            self.reserved_networks = [
                name.strip() for name in vcenter_data_model.reserved_networks.split(",")
            ]
        ...
        self.logger.debug(
            "Connectivity actions mappings: {0}".format(
    ...
    def _handle_no_dvswitch_error(self, holder):
        error = ValueError(
            'Please set the attribute "Default DvSwitch" in order to execute any connectivity changes'
        )
        err_res = []
    ...
    def _map_requsets(self, actions):
        ...
        vm_mapping = self._create_mapping_from_groupings(
            grouped_by_vm_by_requset_by_mode
        )
        return vm_mapping
    ...
    def _group_action(self, actions):
        ...
        grouped_by_vm_by_requset_by_mode = self._group_vm_action_type_by_mode(
            grouped_by_vm_by_requset
        )
    ...
    def _group_vm_actions_by_req_type(self, grouped_by_vm):
        ...
        for machine, actions in list(grouped_by_vm.items()):
            ...
            for action in actions:
                self._add_safely_to_dict(
                    dictionary=grouped_by_vm_by_requset[machine],
                    key=action.type,
                    value=action,
                )
    ...
    def _group_action_by_vm(self, actions):
        ...
        for action in actions:
            ...
            self._add_safely_to_dict(
                dictionary=grouped_by_vm, key=vm_uuid, value=action
            )
    ...
    def _run_async_connection_actions(self, si, mappings, pool, logger):
        ...
        for vm, action_mappings in list(mappings.items()):
            async_results.append(
                pool.apply_async(
                    self._apply_connectivity_changes, (si, vm, action_mappings, logger)
                )
            )
    ...
    def _set_vlan(self, action_mappings, si, vm_uuid, logger):
        ...
        try:
            ...
            self.logger.debug(
                "connecting vm({0}) with the mappings".format(
                    vm_uuid, jsonpickle.encode(action_mappings, unpicklable=False)
                )
            )
            ...
            self.logger.exception(
                "Exception raised while connecting vm({})".format(vm_uuid)
            )
    ...
    def _consolidate_duplicate_results(self, results):
        ...
        for result in results:
            self._add_safely_to_dict(
                value=result, dictionary=mapping, key=result.actionId
            )
        for action_id, actions in list(mapping.items()):
            final_act = actions[0]
            for action in actions[1:]:
                final_act.updatedInterface = "{0},{1}".format(
                    final_act.updatedInterface, action.updatedInterface
                )
        ...
        for mode, vlan_to_action in list(set_actions_grouped_by_vlan_id.items()):
            ...
            for vlan_id, actions in list(vlan_to_action.items()):
                ...
                for action in actions:
                    ...
                    for v_name in names:
                        self._add_safely_to_dict(
                            dictionary=set_act_group_by_mode_by_vlan_by_requsted_vnic[
                                mode
                            ][vlan_id],
                            key=v_name,
                            value=action,
    ...
    def _group_action_by_vlan_id(self, set_vlan_actions):
        ...
        for mode, actions in list(set_vlan_actions.items()):
            ...
            for action in actions:
                vlan_id = action.connectionParams.vlanId
                self._add_safely_to_dict(
                    dictionary=set_actions_grouped_by_vlan_id[mode],
                    key=vlan_id,
                    value=action,
                )
    ...
    def _remove_vlan(self, action_mappings, si, vm_uuid, logger):
        ...
        try:
            ...
            self.logger.debug(
                "disconnecting vm({0}) with the mappings".format(
                    vm_uuid, jsonpickle.encode(action_mappings, unpicklable=False)
                )
            )
            connection_results = self.disconnector.disconnect_from_networks(
                si,
                logger,
            ...
            self.logger.error(
                "Exception raised while disconnecting vm({0}) with exception: {1}".format(
    ...
    def _get_vm_uuid(action):
        vm_uuid_values = [
            attr.attributeValue
            for attr in action.customActionAttributes
