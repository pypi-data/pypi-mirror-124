...
Sequence of commands called:
    Installation: prepare, build, compile, install, whichever is not ``null``
...
from pspman.errors import (InstructError, InstructFmtError, InstructTypeError,
                           MissingInstructError)
...
def get_instruct(config: Path = None) -> typing.Dict[str, DefInstruct]:
    ...
    standard_paths: typing.List[Path] = [
        (Path(__file__).parent / 'inst_config').resolve()
    ]
    ...
    for inst_path in standard_paths:
        for node in inst_path.iterdir():
            if all((node.is_file(), node.stem != 'template',
                    node.suffix == '.yml')):
                known_types[node.stem] = DefInstruct(node)
    return known_types


INST_METHODS = get_instruct(config=CONFIG.config_file.parent / 'inst_config')
'''
