...
from pspman import CONFIG, print
from pspman.classes import GitProject, InstallEnv
from pspman.config import MetaConfig
from pspman.queues import (CloneQueue, DeleteQueue, FailQueue, InstallQueue,
                           PSPQueue, PullQueue, SuccessQueue)
from pspman.shell import git_list
...
def find_gits(
...
) -> typing.Tuple[typing.Dict[str, GitProject], typing.Dict[str, GitProject]]:
    '''
    ...
    with open(env.clone_dir / '.pspman.healthy.yml', 'w') as mem_handle:
        for name, project in git_projects.items():
            if project is not None:
                yaml.dump({name: project.__dict__}, mem_handle)
    with open(env.clone_dir / '.pspman.fail.yml', 'w') as mem_handle:
...
def print_projects(
        env: InstallEnv,
        git_projects: typing.Dict[str, GitProject] = None,
        failed_projects: typing.Dict[str, GitProject] = None) -> int:
    '''
...
def print_prefixes(env: InstallEnv, config: MetaConfig = None):
    ...
    for group in config.meta_db_dirs.values():
        if env.verbose:
...
def del_projects(
        ...
        git_projects: typing.Dict[str, GitProject],
        queues: typing.Dict[str, PSPQueue],
        del_list: typing.List[str] = None) -> typing.Dict[str, GitProject]:
    '''
    ...
    for project_name in del_list:
        if project_name not in git_projects:
            ...
            with open(env.clone_dir / '.pspman.fail.yml', 'a') as fail_handle:
...
def _parse_inst(
    inst_input: str
) -> typing.Tuple[str, typing.Optional[str], typing.List[str], typing.Dict[
        str, str], bool]:
    '''
...
def add_projects(env: InstallEnv,
    ...
    queues['clone'] = CloneQueue(env=env,
                                 success=queues['install'],
                                 fail=queues['fail'])
    ...
    for inst_input in to_add_list:
        ...
        new_project = GitProject(url=url,
                                 ...
                                 branch=branch,
                                 pull=pull)
        if (env.clone_dir / new_project.name).is_file():
...
def update_projects(env: InstallEnv, git_projects: typing.Dict[str,
                                                               GitProject],
