...
class RMLEResult:
    ...
    def cov(self):
        ...
        if dim ==2:
            ...
            for i in range(20):
                for j in range(20):
                    E_b0b1 += B0[i] * B1[j] * self.f_shaped[i][j] * b0_step * b1_step
            covar = E_b0b1 - self.ev()[0] * self.ev()[1]
            cov_mat[0][1],cov_mat[1][0] = covar,covar
            return cov_mat
            ...
            for i in range(20):
                for j in range(20):
                    E_b0b1 += B0[i] * B1[j] * f01[i][j] * b0_step * b1_step
            var01 = E_b01 - self.ev()[0] * self.ev()[1]
            for i in range(20):
                for j in range(20):
                    E_b0b2 += B0[i] * B2[j] * f02[i][j] * b0_step * b2_step
            var02 = E_b02 - self.ev()[0] * self.ev()[2]
            for i in range(20):
                for j in range(20):
                    E_b1b2 += B1[i] * B2[j] * f12[i][j] * b1_step * b2_step
            var12 = E_b1b2 - self.ev()[1] * self.ev()[2]
