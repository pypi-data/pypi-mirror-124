...
def parallelize_tranformation(
    mappingfile, mainsheet, stagingdata, outputfile=None, url=None, ionfile=None, n_cores=4
):
    ...
    # Checking for special api calls
    if url is not None and ionfile is not None:
        main_cache = getColumnsApiData(main_cache, url, ionfile)
...
def getColumnsApiData(mapping_cache, url, ionfile):
    """Get additional information from API"""
    cono = 409
    for map in mapping_cache:
        if map["API_FIELD"] == "CONO" and map["FUNC_VAL"] != "": # get company number if exist in mapping
            cono = int(map["FUNC_VAL"])
            
        if map["FUNC_TYPE"] == "api":
            api_values = map["FUNC_ARG"].split("|")
            headers = inforlogin.get_headers_info(ionfile)           
            data = {api_values[2]:api_values[3], "CONO":cono}
            program = api_values[0]
            methode = api_values[1]
            result = fetchApiData(url, headers, program, methode, cono, data)
            if len(result) > 0:
                if "results" in result.keys():
                    if len(result["results"]) > 0:
                        for key in result["results"]:
                            if(result["nrOfSuccessfullTransactions"] == 1 and len(key['records']) > 0):
                                map["FUNC_TYPE"] = "const"
                                map["FUNC_VAL"] = key["records"][0]["MSGN"]
                                map["FUNC_ARG"] = ""
                            else:
                                raise TransformationError(
                                    "Value for the field '{}' mentioned in mapping sheet is not found in M3.".format(
                                        map["API_FIELD"]
                                    )
                                )
            else:
...
def transform_function_column(map_row, map_col, tb_row, func_name, func_args):
    """Reads the value from api against source col, and maps it to map_col in map_row"""
