...
class API:
  ...
  def __init__(self, webview, args, settings, **kwargs):
    ...
    self.commands = Commands(settings)
    self.webadapter = WebAdapter(fail_on_errors=True, verify_tls=args.verify_tls)
    self.config_util = ConfigUtil()
    self.global_username = os.environ.get('GLOBAL_USERNAME') or args.username or self.config_util.get(self.settings,'auth.global.username')
    self.global_password = os.environ.get('GLOBAL_PASSWORD') or args.password or self.config_util.get(self.settings,'auth.global.password')
