...
def run(worker_config_dict: WorkerConfigDict) -> None:
    ...
    def _all_process_wrapper_iterator() -> Iterator[ProcessWrapper]:
        for worker_entry in worker_entries:
            for process_wrapper in worker_entry.process_wraper_list:
                yield process_wrapper

    # Monitor the liveness of all worker processes
    any_done_process_wrapper: ProcessWrapper | None = None
    try:
        while not term_event.is_set():
            for process_wrapper in _all_process_wrapper_iterator():
            ...
            print(
                'Terminate all processes... (cuased by %s)' % (
                    any_done_process_wrapper.name if any_done_process_wrapper else 'unknown'
                ),
                flush=True
            )
    ...
    for process_wrapper in _all_process_wrapper_iterator():
        ...
        _printer(
            ...
            f'{term_signal.name} is requested (PID: {process_wrapper.pid}, PGID: {process_wrapper.pgid})',
        )

        if process_wrapper.parent_worker_config.signal_to_pg:
            os.killpg(process_wrapper.pgid, term_signal.value)
        else:
            os.kill(process_wrapper.pid, term_signal.value)
    ...
    while time.monotonic() < wait_until_ts:
        ...
        for process_wrapper in _all_process_wrapper_iterator():
            if process_wrapper.monitor.is_alive():
                print('- %s' % process_wrapper.name, flush=True)
