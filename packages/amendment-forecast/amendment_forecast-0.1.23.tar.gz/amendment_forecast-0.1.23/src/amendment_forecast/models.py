...
class FBProphet(BaseModel):
    ...
    def run(self, dataframe: pd.DataFrame, train_end: datetime, frequency: str, forecast_period_list: list) -> tuple:
        ...
        Returns:
            dataframes with actual and predicted values
        """
        train_days = (train_end - dataframe[DATE_COLUMN_NAME].min()).days
        period = "30 days" if frequency == "MS" else "1 W"
        horizon_days = (forecast_period_list.max() - pd.to_datetime(dataframe[DATE_COLUMN_NAME]).max()).days

        dataframe = dataframe.rename(columns={DATE_COLUMN_NAME: "ds", VALUE_COLUMN_NAME: "y"})

        model = Prophet()
        model.fit(dataframe, iter=1000)

        df_cv = cross_validation(
            model,
            initial=f'{train_days} days',
            period=period,
            horizon=horizon_days
            ).rename(columns={"ds": DATE_COLUMN_NAME, "y": VALUE_COLUMN_NAME}
            ).set_index(DATE_COLUMN_NAME)
        future = model.make_future_dataframe(periods=len(forecast_period_list))
        forecast = model.predict(future)
        forecast = forecast.rename(columns={"ds": DATE_COLUMN_NAME}).set_index(DATE_COLUMN_NAME)

        return df_cv[VALUE_COLUMN_NAME], df_cv["yhat"], forecast["yhat"]
