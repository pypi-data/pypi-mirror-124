...
def run_forecast_ensemble(
    ...
    for model in ensemble:
        ...
        if model["weight"] > 0:
            ...
            train_df[f"predicted_values_{model['name']}"] = train_df[f"predicted_values_{model['name']}"] * model["weight"]
            return_dataframe = pd.merge(
                left=return_dataframe,
                ...
                left_index=True,
                right_index=True)
    ensemble_prediction_columns = [column for column in return_dataframe.columns if column.startswith("predicted_values")]
...
run_forecast_ensemble(
        ...
        frequency="month",
        period_format=None,
        model_list=["GreyKite", "FBProphet", "Naive", "RandomForest", "SARIMA"])
