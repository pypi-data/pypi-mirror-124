...
if TYPE_CHECKING:
    ...
    from nbplusplus.data_model.code_cell import ExecutedCodeCell
    from nbplusplus.types import TimestampOrCounter
...
def _compute_slice_impl(seeds: List[TimestampOrCounter]) -> Set[TimestampOrCounter]:
    ...
    if nbpp().mut_settings.dynamic_slicing_enabled:
        if isinstance(seeds[0], Timestamp):
            timestamp_to_ts_deps = _graph_union(timestamp_to_ts_deps, nbpp().dynamic_data_deps)
        else:
            timestamp_to_ts_deps = _graph_union(timestamp_to_ts_deps, _coarsen_timestamps(nbpp().dynamic_data_deps))
    if nbpp().mut_settings.static_slicing_enabled:
        if isinstance(seeds[0], Timestamp):
            timestamp_to_ts_deps = _graph_union(timestamp_to_ts_deps, nbpp().static_data_deps)
        else:
            timestamp_to_ts_deps = _graph_union(timestamp_to_ts_deps, _coarsen_timestamps(nbpp().static_data_deps))
