...
from nbplusplus.data_model import sizing
from nbplusplus.data_model.annotation_utils import get_type_annotation, make_annotation_string
from nbplusplus.data_model.code_cell import ExecutedCodeCell, cells
from nbplusplus.data_model.timestamp import Timestamp
from nbplusplus.data_model.update_protocol import UpdateProtocol
from nbplusplus.extra_builtins import EMIT_EVENT
from nbplusplus.run_mode import ExecutionMode, FlowOrder
from nbplusplus.singletons import nbpp, tracer

if TYPE_CHECKING:
    from nbplusplus.types import SupportedIndexType
    from typing import Any, Dict, List, Optional, Set, Tuple

    # avoid circular imports
    from nbplusplus.data_model.scope import Scope
    from nbplusplus.data_model.namespace import Namespace
...
class DataSymbol:
    ...
    def staleness_timestamp(self) -> int:
        return max(self._timestamp.cell_num, nbpp().min_timestamp)
    ...
    def get_top_level(self) -> Optional[DataSymbol]:
        if not self.containing_scope.is_namespace_scope:
            ...
            for alias in nbpp().aliases[containing_scope.obj_id]:
    ...
    @property
    def namespace(self):
        return nbpp().namespaces.get(self.obj_id, None)
    ...
    def collect_self_garbage(self):
        ...
        ns = nbpp().namespaces.get(self.obj_id, None)
        if ns is not None:
            ns._tombstone = True
            ns.obj = None
        self.obj = None
        nbpp().blocked_reactive_timestamps_by_symbol.pop(self, None)
    ...
    def update_obj_ref(self, obj, refresh_cached=True):
        ...
        self._tombstone = False
        self._cached_out_of_sync = True
        if nbpp().settings.mark_typecheck_failures_unsafe and self.cached_obj_type != type(obj):
        ...
        if self.cached_obj_id is not None and self.cached_obj_id != self.obj_id:
            new_ns = nbpp().namespaces.get(self.obj_id, None)
            # don't overwrite existing namespace for this obj
            old_ns = nbpp().namespaces.get(self.cached_obj_id, None)
    ...
    def get_ref_count(self):
        ...
        total -= len(nbpp().aliases[self.obj_id])
        ns = nbpp().namespaces.get(self.obj_id, None)
    ...
    def should_preserve_timestamp(self, prev_obj: Optional[Any]) -> bool:
        if nbpp().mut_settings.exec_mode == ExecutionMode.REACTIVE:
            # always bump timestamps for reactive mode
            return False
        if prev_obj is None:
            return False
        if nbpp().blocked_reactive_timestamps_by_symbol.get(self, -1) == self.timestamp.cell_num:
            return False
    ...
    def _handle_aliases(self):
        old_aliases = nbpp().aliases.get(self.cached_obj_id, None)
        if old_aliases is not None:
            old_aliases.discard(self)
            if len(old_aliases) == 0:
                del nbpp().aliases[self.cached_obj_id]
        nbpp().aliases[self.obj_id].add(self)
    ...
    def update_stmt_node(self, stmt_node):
        ...
        if self.is_function:
            ...
            nbpp().statement_to_func_cell[id(stmt_node)] = self
            self.call_scope = self.containing_scope.make_child_scope(self.name)
        return stmt_node
    ...
    def _get_calling_ast_node(self) -> Optional[ast.AST]:
        ...
        caller_ast_node = nbpp().ast_node_by_id.get(prev_node_id_in_cur_frame_lexical, None)
    ...
    def is_stale_at_position(self, pos: int, deep: bool = True) -> bool:
        ...
        if deep:
            ...
            if not self.is_shallow_stale:
                return False
        if nbpp().mut_settings.flow_order in (FlowOrder.ANY_ORDER, FlowOrder.DAG):
            return True
    ...
    def update_usage_info(self, used_time: Optional[Timestamp] = None, exclude_ns: bool = False) -> None:
        ...
        if ts_to_use.is_initialized and used_time not in self.timestamp_by_used_time and ts_to_use < used_time:
            nbpp().add_dynamic_data_dep(used_time, ts_to_use)
    ...
    def refresh(
        ...
        if bump_version:
            ...
            if ns is not None:
                ...
                for alias in nbpp().aliases[ns.obj_id]:
