...
def train(parameters, cloud_mode, force_download, seed, model):
    ...
    try:
        sys.stdout = ConsoleLogger(name="stdout", console=sys.stdout)
        sys.stderr = ConsoleLogger(name="stderr", console=sys.stderr)
        ...
        with click_spinner.spinner():
            context.save_log_file(sys.stdout)
            context.save_log_file(sys.stderr)
