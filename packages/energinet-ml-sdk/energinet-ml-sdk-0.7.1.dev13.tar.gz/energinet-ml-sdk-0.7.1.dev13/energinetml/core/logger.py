...
class ConsoleLogger:
    ...
    def __init__(self, console: io.TextIOWrapper = sys.stdout):
        ...
        suffix = "out" if console.name == "<stdout>" else "err"
        self.filename = f"{file_prefix}_{suffix}.{ext}"

        self.filepath, self.log = self._init_log_file(self.filename)

    def _check(self) -> None:
        """Performs check of input argument."""
        con_name = self.console.name
        test = (con_name == "<stdout>") | (con_name == "<stderr>")
        if not test:
            raise AssertionError(f"You used a not valid sys.std* > {con_name}")
