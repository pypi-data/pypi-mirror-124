...
from energinetml.azure.datasets import MLDataSet, AzureMLDataStore
...
class TestMLDataSet:
    ...
    def test__path(self):
        with tempfile.TemporaryDirectory() as path:
            ...
            assert uut.path('some', 'folder') == \
                os.path.join(path, 'some', 'folder')
...
class TestAzureMLDataStore:
    ...
    @pytest.mark.parametrize('dataset_version, actual_dataset_version', (
            ('123', '123'),
            (None, 'latest'),
    ))
    def test__load_azureml_dataset__dataset_exists__should_return_dataset(
        ...
        get_by_name_mock.assert_called_once_with(
            workspace=workspace,
            name='datasetname',
            version=actual_dataset_version,
        )
    ...
    def test__load_azureml_dataset__azure_raise_user_error_exception__should_raise_data_set_not_found(  # noqa: E501
        ...
        get_by_name_mock.side_effect = \
            exceptions._azureml_exception.UserErrorException('x')
