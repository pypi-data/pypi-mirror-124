...
from energinetml.cli.cluster.change import change, _update_model_properties
...
def test__change_cluster__provide_a_cluster_that_does_not_exists__should_prompt_for_clister_and_update_model(  # noqa: E501
    ...
    result = CliRunner().invoke(
        ...
        args=[
            '--path', model.path,
            '--cluster-name', 'a-cluster-that-doesnt-exists',
        ],
    )
    ...
    _update_model_properties_mock.assert_called_once_with(
        model=ANY,
        cluster_name='an-existing-cluster',
        vm_size='cluster-vm-size',
    )
