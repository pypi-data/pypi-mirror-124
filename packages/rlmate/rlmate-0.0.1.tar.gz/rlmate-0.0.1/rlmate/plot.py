...
def plot_training_progress(scores, eps_start = None, eps_end = None, eps_decay = None, epsilons = None, window_length = 250, figsize = (12,12), fontsize = 18, path = None, scores_color = 'c', mean_color = 'r', score_range = None, epsilon_range = None, axis_gap = 0.02, mark_best = False, legend = True, ax = None, suppress_single_scores = False):
    ...
    if (eps_start is not None) or (eps_end is not None) or (eps_decay is not None):
        assert all(arg is not None for arg in [eps_start, eps_end, eps_decay]), "You need to provide all of eps_start, eps_end and eps_decay if you want to plot the effect of the epsilon-greedy policy"
        epsilons = []
    ...
    if epsilons is not None and len(epsilons) > 0:
        assert len(epsilons) == len(scores), "The length of the list of epsilon (exploration coefficient) values need to be the same as the amount of scores"
    ...
    if mark_best:
        ...
        other_best = None
        plt.axvline(x = best, linestyle = 'dotted', color = '#A0A0A0', label = 'Highest overall result')
        start_id = 0
...
def plot_training_progress_from_file(file, eps_start = 1, eps_end = 0.001, eps_decay = 0.999, epsilons = [], window_length = 250, figsize = (12,12), fontsize = 18, path = None, scores_color = 'c', mean_color = 'r', score_range = None, epsilon_range = None, axis_gap = 0.02, mark_best = True, legend = True, suppress_singe_scores = False):
    ...
    return plot_training_progress(scores, eps_start, eps_end, eps_decay, epsilons, window_length, figsize,
            fontsize, path, scores_color, mean_color, score_range, epsilon_range, axis_gap, mark_best = mark_best, legend = legend)

# todo: this doesn't feel right. the information is encoded in the experiment and should be read from there. 
def plot_training_progress_from_experiment(exp, eps_start_key='-es', eps_end_key='-ee', eps_decay_key='-ed', window_length = 250, figsize = (12,12), fontsize = 18, path = None, scores_color = 'c', mean_color = 'r', score_range = None, epsilon_range = None, axis_gap = 0.02, mark_best = True, legend = True, suppress_single_scores = False):
    """Read the trainings scores from file and plot the training progress. 
    ...
    return plot_training_progress_from_file(file, eps_start, eps_end, eps_decay, epsilons, window_length, figsize,
            fontsize, path, scores_color, mean_color, score_range, epsilon_range, axis_gap, mark_best = mark_best, legend = legend)
