...
from nbplusplus.analysis.resolved_symbols import ResolvedDataSymbol
from nbplusplus.analysis.live_refs import (
    compute_live_dead_symbol_refs,
    get_symbols_for_references,
    get_live_symbols_and_cells_for_references,
)
from nbplusplus.analysis.slicing import CodeCellSlicingMixin
from nbplusplus.data_model.timestamp import Timestamp
from nbplusplus.ipython_utils import cell_counter as ipy_cell_counter
from nbplusplus.run_mode import FlowOrder
from nbplusplus.singletons import nbpp
...
if TYPE_CHECKING:
    ...
    from nbplusplus.types import CellId, TimestampOrCounter
...
class ExecutedCodeCell(CodeCellSlicingMixin):
    ...
    def __init__(self, cell_id: CellId, cell_ctr: int, content: str, tags: Tuple[str, ...]) -> None:
        ...
        self._cached_typecheck_result: Optional[bool] = None if nbpp().settings.mark_typecheck_failures_unsafe else True
    ...
    def _override_position_index_for_current_flow_semantics(cls) -> Generator[None, None, None]:
        ...
        try:
            if nbpp().mut_settings.flow_order == FlowOrder.ANY_ORDER:
        ...
        with self._override_position_index_for_current_flow_semantics():
            ...
            for sym in live_symbols:
                if filter_to_reactive:
                    if sym.is_blocking:
                        continue
                    if not sym.is_reactive and sym.dsym not in nbpp().updated_reactive_symbols:
                        continue
    ...
    def check_and_resolve_symbols(
        ...
        live_symbol_refs, dead_symbol_refs = compute_live_dead_symbol_refs(self.to_ast(), scope=nbpp().global_scope)
        ...
        dead_symbols, _ = get_symbols_for_references(dead_symbol_refs, nbpp().global_scope)
