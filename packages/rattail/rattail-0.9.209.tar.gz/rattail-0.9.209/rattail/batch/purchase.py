...
class PurchaseBatchHandler(BatchHandler):
    ...
    * ``rattail.enum.PURCHASE_BATCH_MODE_COSTING``
    ...
    def populate_from_invoice_with_po(self, batch, progress=None):
        ...
        # set the invoice total, "original" (per invoice line totals)
        # and "calculated" (which will include all lines from PO)
        batch.invoice_total = sum([row.invoice_total or 0
                                   for row in batch.active_rows()])
        batch.invoice_total_calculated = sum([row.invoice_total
                                              or row.invoice_total_calculated
                                              or row.po_total or row.po_total_calculated
                                              or 0
                                              for row in batch.active_rows()])
