...
def match_feature(prompt_local_datetime_series, df_logs_combined):
    ...
    min_before_list = [60*x for x in list(range(1, 11))]
    ...
    df = pd.DataFrame()
    for min_before in min_before_list:
        col = int(min_before//60 - 1)
        df = pd.concat([df, mims_summary_df[col].rename("mims_summary_" + str(min_before//60) + "min")], axis=1)
        df = pd.concat([df, readings_df[col].rename("num_readings_" + str(min_before//60) + "min")], axis=1)
        df = pd.concat([df, start_time_df[col].rename("start_time_" + str(min_before//60) + "min")], axis=1)
