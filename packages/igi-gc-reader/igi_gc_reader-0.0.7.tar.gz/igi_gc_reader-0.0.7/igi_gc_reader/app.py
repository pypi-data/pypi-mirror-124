...
class GcFileTransformer(IFileTransformer):
    def try_transform_file(
            self, in_path: str, out_path: str = UNASSIGNED_OUT_PATH
    ...
    def transform_file(self, in_path: str, out_path: str = UNASSIGNED_OUT_PATH) -> str:
        """
        Transform GC lab file ready for p:IGI+/Transform import.str
    ...
    def result_disclaimer(self) -> str:
        return ("Please check the output. We have tried to interpret the input file and "
            "asssign data to appropriate indicators and uoms. This is challenging because "
            "the headers do not always use the same names or appear in the same columns. "
            "In the output file headers repeat in groups e.g. one for each indicator "
            "e.g. area, height, concentrations from area etc). If you check the first group "
            "after the context headers for well name etc the same indicators / uoms will be "
            "used for each property after.")
