...
class Analyser(AbstractAnalyser):
    ...
    def get_trajectories_filaments(self, minimum_image: bool) -> np.ndarray:
        """
        Load complete trajectory of all filament beads. This can be very
        memory intensive, when executed for the first time,
    ...
    def get_time(self) -> np.ndarray:
        return self._get_blob_analysis_dataset('time', self._compute_time)
    ...
    def _compute_time(self) -> np.ndarray:
        time_in_steps = self.get_time_in_steps()
        dt = self.data_reader.read_dt()
    ...
    def _check_version(self, data_set: Dataset):
        if "ACTOMYOSIN_ANALYSER_VERSION" in data_set.attrs:
            if data_set.attrs['ACTOMYOSIN_ANALYSER_VERSION'] != _VERSION:
                msg = (f"File: {self.analysis_file}; "
                       f"DataSet: {data_set.name}; "
                       "this dataset was created with version "
                       f"{data_set.attrs['ACTOMYOSIN_ANALYSER_VERSION']}, "
                       f"your version is {_VERSION}. "
                       f"Make sure the data in there is what you expect it to be.")
                logger.warning(msg)
            return
