...
# versions
import os
__version__ = os.environ.get('VERSION_NEW', '0.4.5')
...
for v in libjpeg_versions:
  ...
  cfiles[v] = [f'{clib}/{f}' for f in os.listdir(clib) if re.fullmatch(f'.*\.c|h', f)]
  for excluded_module in ['jmemdos','jmemmac','jmemansi','ansi2knr','ckconfig','jmemname', # platform dependents
    ...
    cfiles[v] = [f for f in cfiles[v] if f[lim:-2] != excluded_module]
...
setuptools.setup(
  ...
  description = 'Low-level loader of JPEG DCT coefficients, based on libjpeg.',
