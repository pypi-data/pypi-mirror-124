...
def send_notifications(email, sender, email_configuration):
    ...
    ATTACHMENT = LOG_PATH
    cover = email_configuration.get('Banner')
    logo = email_configuration.get('Logo')
    paragraph = email_configuration.get('Email_Content')
    # # The HTML body of the email.
    file_name = email_configuration.get('HTML_File')
    # email_html = open(file_name)
    BODY_HTML = open('uploads/HTML_files/' + file_name).read().format(cover=cover,
                                                           logo=logo,
                                                           paragraph=paragraph)
