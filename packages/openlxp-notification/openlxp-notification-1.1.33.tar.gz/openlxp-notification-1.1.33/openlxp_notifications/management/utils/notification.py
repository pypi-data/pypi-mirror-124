...
def send_notifications(email, sender, email_configuration):
    ...
    # Getting path of email cover picture and logo picture
    cover = settings.MEDIA_ROOT + '/' + email_configuration.get('Banner')
    logger.info(cover)
    logo = settings.MEDIA_ROOT + '/' + email_configuration.get('Logo')
    logger.info(logo)
