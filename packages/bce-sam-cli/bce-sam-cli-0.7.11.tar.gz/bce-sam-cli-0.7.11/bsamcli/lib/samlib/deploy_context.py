...
class DeployContext(object):
    ...
    def all_functions(self):
        """
        Return all functions in template.yaml. If function identifier is provided, return the specified function only
        """
        ...
        if self._function_identifier is not None:
            for f in all_functions:
                if f.name == self._function_identifier:
                    return [f]
            raise FunctionNotFound("Unable to find function %s in the template file" % self._function_identifier)
