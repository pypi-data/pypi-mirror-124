...
def create_iir_filter_wc(response_type=None, ftype='butter', order=None, wc=None, rp=None, rs=None,
    ...
    coeffs = signal.iirfilter(N=order, Wn=wc, rp=rp, rs=rs, btype=response_type, analog=False, ftype=ftype, output='ba',
                              fs=sfreq)
    init_state = signal.lfilter_zi(coeffs[0], coeffs[1])
    init_state = np.transpose(np.tile(init_state, (16, 1)), axes=(1, 0))
    ...
    out = Filter(filter_type='FIR - WINDOW', response_type=response_type, cutoff_freqs=wc,
                 bands=bands, filter_coeffs=coeffs, info=info, initial_state=init_state)
...
def apply_filters(data, filters_obj):
    ...
    for f in filters_obj.filters:
        out, final_z = signal.lfilter(f.filter_coeffs[0], f.filter_coeffs[1], out, axis=0, zi=f.init_state)
