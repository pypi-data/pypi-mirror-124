...
class EEGStream:
    ...
    def export_stream(store, file_name, file_ext='txt'):
        ...
        if file_ext == 'npy':
            ...
            df = pd.DataFrame(data=lst, index=None, columns=column_names[:ncols])
            df.to_csv(f"{file_name}.csv", index=False)
            del df
        elif file_ext == 'edf':
            signal_headers = highlevel.make_signal_headers(column_names[:ncols], sample_rate=SAMPLING_FREQUENCY,
            ...
            data = {"data": lst, "cols": column_names[:ncols]}
