    ...
    def _authenticate(self, **kwargs) -> None:
        ...
        if username:
            if token:
                ...
                self._log.warn(
                    "Token or Password is required for authentication.  Starting unathenticated session."
                )
        else:
            self._log.warn(
                "Username is required for authentication.  Starting unathenticated session."
            )
