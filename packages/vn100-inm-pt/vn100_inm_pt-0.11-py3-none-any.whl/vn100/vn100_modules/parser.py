...
class VN100_Parser:
    ...
    def __init__(
        ...
        self.reference_db = VN100_DB(
            logger=self._logger,
            db_connection=reference_db_connection)
        # ////// Binary outputs /////////
    ...
    def _clean_buffer_of_old_and_bad_data(
        ...
        # When the buffer length is greater than X_MESSAGE_MAX_LENGTH, APPLY THIS FUNCTION:
        current_input_buffer_length = len(self.input_buffer)
        if ((reset_flag == False and
             current_input_buffer_length > (2 * self.BIN_MESSAGE_MAX_LENGTH)) or
            (reset_flag == True and
             current_input_buffer_length > (2 * self.RESET_MESSAGE_MAX_LENGTH))):
        ...
        for current_turing_machine_head_position in range(len(self.input_buffer)):
            ...
            if (reset_flag == False):
                try:
                    ...
                    # Normally, when the parser can't find a complete message,
                    # it throws an "list out of range" error:
                    self._logger.debug(
                        "Error during scan for a binary package execution: {}".format(e))
                    ...
                    if (self.input_buffer[asterisk_search_index][0] == int.from_bytes("*".encode("ascii"), "big")):
                        ...
                        if (C8bit_cksm.check_data(bytes(suspected_package)) == True):
                            data_structure = self.reference_db.get_string_data_structure(
                                suspected_package)
                            # Clean the package content from input buffer. +2 is the length of characters \r\n:
                            for index_detected_package in range(len(suspected_package) + 2):
                                self.input_buffer.pop(
                                    current_turing_machine_head_position)
