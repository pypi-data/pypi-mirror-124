...
class Async_Serial:
    def __init__(
        ...
        self.health_watchdog = Health_Watchdog(
            target_class_name="Async_Serial",
            logger=self._logger,
    ...
    async def _reader(self):
        ...
        while (self.availability == True):
            ...
            if (self.serial_port.is_open):
                ...
                _content = [byte[0] for byte in self.buffered_read]
                # self._logger.debug("content: {}".format(bytearray(_content)))
                # self._logger.debug("bp_counter: {}".format(self.counter))
                self.health_watchdog.flash_endpoint_function("reader")
