...
class VN_INM_Interface_Server:
    def __init__(
        ...
        if (performance_settings != None):
            self._FUTURES_LOOP_SLEEP_TIMERS = {
                ...
                "RPC_TO_SET_INM_CHECKER_LOOP_PERIOD": performance_settings[
                    "set_user_inm_configuration"]["dead_times"]["set_inm_checker"]["value"],
                "RPC_TO_READ_INM_CHECKER_LOOP_PERIOD": performance_settings[
                    "set_user_inm_configuration"]["dead_times"]["read_inm_checker"]["value"],
                "NEXT_REQUIREMENT_LOOP_PERIOD": performance_settings[
                    "set_user_inm_configuration"]["dead_times"]["next_requirement"]["value"]
            }
        else:
            self._FUTURES_LOOP_SLEEP_TIMERS = {
        ...
        # Endpoint of the __init__ health:
        self.health_watchdog.flash_endpoint_function("__init__")
        ...
        while (1):
            ...
            if (self._is_autoconfiguring == False):
                ...
                try:
                    ...
                    self.rcvd_data = [0]
                    self.health_watchdog.flash_endpoint_function(
                        "udp_rpc_listener")
                    continue
                # Since the socket is configured as "non-blocking mode", while the clients don't write anything to it,
                # the socket will be unavailable, that behavior is normal:
                except BlockingIOError as e:
                    self._logger.debug("Blocking IO error: {}".format(e))
                    await asyncio.sleep(self._FUTURES_LOOP_SLEEP_TIMERS["REMOTE_CONTROL_SERVER_RETRY_LISTEN_TIME"])
                    self.rcvd_data = [0]
                    self.health_watchdog.flash_endpoint_function(
                        "udp_rpc_listener")
                    ...
                    self._logger.error(
                        'Another kind of exception has been thrown: {}'.format(e))
                    await asyncio.sleep(self._FUTURES_LOOP_SLEEP_TIMERS["REMOTE_CONTROL_SERVER_RETRY_LISTEN_TIME"])
                    self.rcvd_data = [0]
                    self.health_watchdog.flash_endpoint_function(
                        "udp_rpc_listener")
                ...
                # Check the received data size:
                    ...
                    self._logger.warning("Package has an invalid size.")
                    await asyncio.sleep(self._FUTURES_LOOP_SLEEP_TIMERS["REMOTE_CONTROL_SERVER_RETRY_LISTEN_TIME"])
                    self.health_watchdog.flash_endpoint_function(
                        "udp_rpc_listener")
                    continue
        ...
        while (1):
            ...
            if (current_question["is_already_answered"] == True):
                ...
                if (len(self._hmi_questions_queue) > 0):
                    # self._logger.debug(len(self._hmi_questions_queue))
                            ...
                            if (vn100_output_object["content"]["command"]["name"] == self._functions_dict[current_question[
                                ...
                                if (vn100_output_object["content"]["command"]["name"] == "READ_REGISTER"):
                                    ...
                                    self._vn100_controller.output.pop(
                                        current_turing_machine_head_position)
                                    # When the current question has been answered, then:
                                    if (current_question["is_already_answered"] == True):
                                        # If current rpc type is repeated on the self._hmi_questions_queue, delete all of them:
                                        for question_position in range((len(self._hmi_questions_queue) - 1), -1, -1):
                                            if ((current_question["question"]["function"]) == self._hmi_questions_queue[question_position]["function"]):
                                                self._hmi_questions_queue.pop(
                                                    question_position)
                                        # If rpc is from user settings descriptor:
                                        if ("user_settings_order" in current_question["question"].keys()):
                                            _response = {
                                ...
                                # Cases restore factory settings and reset:
                                    ...
                                    # RPC unavailable yet:
                                    self._logger.warning("The RPC named {} is unavailable but it is in the INM. You must develop this feature.".format(
            ...
            self.health_watchdog.flash_endpoint_function("inm_rpc_agent")
            ...
            if (len(self._vn100_controller.output) > 0):
                for current_turing_machine_head_position in range(len(self._vn100_controller.output)):
                    ...
                    if (vn100_output_object["type"] == "string"):
                        ...
                        if (not(command_name == "READ_REGISTER" or
                                ...
                                command_name == "ERROR")):
                            # self._logger.debug(
                            #     "Consumido: {}".format(vn100_output_object))
                            # Remove the INM object from the buffer:
                            self._vn100_controller.output.pop(
                                current_turing_machine_head_position)
                        del command_name
    ...
    async def _set_user_inm_configuration(self):
        ...
        for requirement in self._user_settings_descriptor["content"].items():
            ...
            if (requirement[1]["function"] == "SYSTEM_RESET" or
                ...
                while (this_requirement_is_checked == False):
                    self.health_watchdog.begin_track(
                        "set_user_inm_configuration_rpc_to_set_reset_inm_checker")
                    # Look for answer, with the user settings order:
                    _user_settings_rpc_responses_queue_length = len(
                        self._user_settings_rpc_responses_queue)
                    # self._logger.debug("RESET or RESTORE FACTORY SETTINGS: _user_settings_rpc_responses_queue_length: {}".format(
                    #     _user_settings_rpc_responses_queue_length))
                    if (_user_settings_rpc_responses_queue_length > 0):
                ...
                # When the reset finalizes, end tracking:
                self.health_watchdog.end_track(
                    "set_user_inm_configuration_rpc_to_set_reset_inm_checker")
            else:
                ...
                while (this_requirement_is_checked == False):
                    self.health_watchdog.begin_track(
                        "set_user_inm_configuration_rpc_to_read_inm_checker")
                    # Look for answer, with the user settings order:
                    _user_settings_rpc_responses_queue_length = len(
                        self._user_settings_rpc_responses_queue)
                    # self._logger.debug("_user_settings_rpc_responses_queue_length: {}".format(
                    #     _user_settings_rpc_responses_queue_length))
                    if (_user_settings_rpc_responses_queue_length > 0):
                        for suspected_answer in self._user_settings_rpc_responses_queue:
                            # Search the suspected answer of current requirement:
                            if ((int(suspected_answer["user_settings_order"]) == int(requirement[0])) and
                                ...
                                if (self._user_settings_check_answer_validity_of_rw_register(
                                    ...
                                    while (this_requirement_is_set == False):
                                        self.health_watchdog.begin_track(
                                            "set_user_inm_configuration_rpc_to_set_inm_checker")
                                        # Look for answer, with the user settings order:
        ...
        # End subprocesses health tracking:
        self.health_watchdog.end_track(
            "set_user_inm_configuration_rpc_to_read_inm_checker")
        self.health_watchdog.end_track(
            "set_user_inm_configuration_rpc_to_set_inm_checker")
