...
class VN100_Controller:
    def __init__(self,
        ...
        self.health_watchdog = Health_Watchdog(
            target_class_name="VN100_Controller",
            logger=self._logger,
        ...
        try:
            ...
            self._logger.error("It can't open the serial port '{}' 'because: {}".format(
        ...
        self.MIN_PARSE_INPUT_BUFFER_LENGTH = 12
        self.MIN_PARSE_RESET_LENGTH = 12
    ...
    async def _parse(self):
        ...
        while (self.async_serial_device.availability == True):
            ...
            if (len(self.async_serial_device.buffered_read) > _min_parse_length):
                ...
                # Check output length, only when parse is succeded:
                ...
                #         len(self.output),
                #         out_length))
                # self._logger.debug("Content: {}".format(self.output))
            # Endpoint of the parse health:
            self.health_watchdog.flash_endpoint_function("parse")
