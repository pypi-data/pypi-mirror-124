...
def _debugNodes(nodes: list):
    for node in nodes:
        if node.get('yield'):
            logger.debug('id=%s, yield=%s, organic=%s, irrigated=%s',
                         node.get('@id'), node.get('yield'), node.get('organic'), node.get('irrigated'))
...
def _aggregate(nodes: list, completeness: dict):
    ...
    return {
        ...
        'min': _min(values),
        'sd': _sd(values),
        'observations': len(nodes) + completeness_count_missing
