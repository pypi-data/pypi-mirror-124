...
class TestSum(unittest.TestCase):
    ...
    def test_one_per_second(self):
        def done():
            self.assertAlmostEqual(TestSum.hit_throttle.total_hit_count, 5, delta=1)
            self.assertAlmostEqual(TestSum.hit_throttle.total_limited_count, 45, delta=1)
    ...
    def test_two_per_second(self):
        def done():
            self.assertAlmostEqual(TestSum.hit_throttle.total_hit_count, 10, delta=1)
            self.assertAlmostEqual(TestSum.hit_throttle.total_limited_count, 12, delta=1)
    ...
    def test_four_per_second(self):
        def done():
            self.assertEqual(TestSum.hit_throttle.total_hit_count, 20)
            self.assertEqual(TestSum.hit_throttle.total_limited_count, 3)
