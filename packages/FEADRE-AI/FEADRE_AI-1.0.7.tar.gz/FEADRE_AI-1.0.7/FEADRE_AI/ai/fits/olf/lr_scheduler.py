...
def f_show_scheduler(y_list):
    import matplotlib.pyplot as plt
    plt.plot(y_list, '.-', label='LambdaLR')
    plt.xlabel('epoch')
    plt.ylabel('LR')
    plt.tight_layout()
...
if __name__ == '__main__':
    name = 'multistep'  # cos warmcos yoloxwarmcos yoloxsemiwarmcos multistep
    lr = 1e-3
    iters_per_epoch = 267
    warmup_epochs = 5
    no_aug_epochs = 20
    ...
    # lr_scheduler = LRScheduler(name=name, lr=lr,
    ...
    for i in range(total_epochs):
        update_lr = lr_scheduler.update_lr(i + 1)
        y_list.append(update_lr)
    f_show_scheduler(y_list=y_list)
