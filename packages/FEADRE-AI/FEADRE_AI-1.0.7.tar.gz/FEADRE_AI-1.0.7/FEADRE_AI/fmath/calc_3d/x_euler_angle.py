...
predictor = dlib.shape_predictor(r"M:\AI\datas\kps\face68\shape_predictor_68_face_landmarks.dat")
...
# 获取翻转向量和平移向量
def get_pose_estimation(shape_hwc, image_points):
    '''
    根据图像的尺寸和对应 关键点的二维地址 与3D模型坐标系  摄像机标定
    求出旋转角和 偏移阵
    :param shape_hwc: 图片wh
    :param image_points: (6, 2)
    :return:
        rotation_vector (3, 1), translation_vector (3, 1)
    '''
    # 3D model points.  3D模型
    ...
    # Camera internals 像机参数 这个通过标定
    focal_length = shape_hwc[1]  # hwc
    center_xy = (shape_hwc[1] / 2, shape_hwc[0] / 2)
    camera_matrix = np.array(
        [[focal_length, 0, center_xy[0]],
         [0, focal_length, center_xy[1]],
         [0, 0, 1]], dtype="double"
    )

    print("Camera Matrix :{}".format(camera_matrix))

    # Assuming no lens distortion 透镜畸变
    dist_coeffs = np.zeros((4, 1))

    (success, rotation_vector, translation_vector) = cv2.solvePnP(
        model_points,  # (num_keypoints,3) 世界坐标 人脸 3D模型坐标
        image_points,  # (num_keypoints,2) 图像坐标系中点的坐标 xy
        camera_matrix,  # (3,3) 相机内参矩阵  通过标定
        dist_coeffs,  # (4,1) 畸变系数  通过标定
        flags=cv2.SOLVEPNP_ITERATIVE)
    # 输出的旋转向量。使坐标点从世界坐标系旋转到相机坐标系 输出的平移向量。使坐标点从世界坐标系平移到相机坐标系
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        # 获取翻转向量和平移向量
        ret, rotation_vector, translation_vector, camera_matrix, dist_coeffs \
            = get_pose_estimation(shape_hwc, image_points)
        if ret != True:
            print('get_pose_estimation failed')
            continue
        ...
        pitch, yaw, roll = list(map(lambda k: k[0], euler_angles))

        euler_angle_str = 'Y:{}, X:{}, Z:{}'.format(pitch, yaw, roll)
        print(euler_angle_str)

        # Project a 3D point (0, 0, 1000.0) onto the image plane.
        # We use this to draw a line sticking out of the nose 3D在2D上 在笔子上画一个z轴
