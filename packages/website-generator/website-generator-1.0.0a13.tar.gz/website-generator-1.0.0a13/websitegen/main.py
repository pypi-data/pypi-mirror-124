...
def get_html_from_file(i_path: str) -> str:
    ...
    if i_path.lower().endswith(".md"):
        ...
        """
        # import markdown
        # -documentation: https://python-markdown.github.io/reference/
        # ret_html = markdown.markdown(file_contents, extensions=[
...
def create_html_from_template(i_theme_dir_path: str, i_nav_dict: dict,
    ...
    relative_link: str = ""
    active_nav_file: str = i_public_fd_path.split("/")[-1]
    if i_page_type == PageTypeEnum.blog_post:
        relative_link: str = "../"
        active_nav_file: str = i_public_fd_path.split("/")[-2]
    elif i_page_type == PageTypeEnum.blog_tag:
        relative_link: str = "../../"
        active_nav_file: str = i_public_fd_path.split("/")[-3]
    with open(template_file_path, "r") as template_file:
        template = Template(template_file.read())
        nav_total_str = ""
        for _fd_public_name, _nav_title_str in i_nav_dict.items():
...
def remove_suffix(i_fd_name: str):
    file_name_wo_suffix_str = i_fd_name
    if "." in i_fd_name:
        file_name_wo_suffix_str = i_fd_name.rpartition(".")[0]
    return file_name_wo_suffix_str


def get_text_intro(post_fd_path: str) -> str:
    ret_snippet = get_html_from_file(post_fd_path)

    # root = xml.etree.ElementTree.fromstring(i_html_text)
    # ret_snippet = root.find("main").itertext  # tail
    # pos = i_html_text.find("<main")
    # ret_snippet = i_html_text[pos:]
    # pos = ret_snippet.find(">")
    # ret_snippet = ret_snippet[pos:]
...
def copy_theme_files(i_theme_dir_path: str, i_dest_path: str):
...
def get_page_title_html(i_fd_name: str, i_parent: str = "") -> str:
    title_text: str = get_title_text(i_fd_name)
    ret_string = "<h2>"
    if i_parent:
        ret_string += f'<span class="context">{i_parent}: </span>'
    ...
    for fd_name_str in top_content_fd_list:
        ...
        if is_page(fd_name_str):
            ...
            if public_file_name_str.lower() == INDEX_FILE:
                nav_title_str = "Home"
            ret_nav_dict[public_file_name_str] = nav_title_str
...
def get_tags(i_sub_fd_name_str) -> list:
    regexp: str = r"\[([^]]+)"
    ret_tags = re.findall(regexp, i_sub_fd_name_str)
    return ret_tags
...
def get_output_path(i_argument_dir, i_force_remove_output_bool) -> str:
    ...
    if os.path.isdir(i_argument_dir):
        ...
        if not i_force_remove_output_bool:
        ...
        if remove_output_bool or i_force_remove_output_bool:
...
def generate_post_html(i_init_vals: InitialValues, i_fd_path, i_nav_dict, i_post_content_file_path) -> str:
    post_name = os.path.basename(i_post_content_file_path)
    post_fd_path = os.path.join(i_init_vals.content_dir_path, os.path.basename(i_fd_path), post_name)
    # parent_title = get_public_file_name(i_fd_name)
    # sub_html_content_str = get_page_title_html(i_post_fd_name, i_parent=parent_title)
    sub_html_content_str = get_page_title_html(post_name)
    sub_html_content_str += get_html_from_file(post_fd_path)
    result_str = create_html_from_template(i_init_vals.theme_dir_path, i_nav_dict, post_fd_path,
...
def generate_blog_html(i_init_vals: InitialValues, i_nav_dict, i_content_fd_path) -> dict:
    result_output_dict = {}
    ...
    for post_content_file_path in sorted(blog_content_fd_path_list, key=lambda x: get_datetime(x)):
        ...
        post_html = generate_post_html(i_init_vals, i_content_fd_path, i_nav_dict,
    ...
    for tag_name, post_list in tag_dict.items():
        ...
        for post_public_fn in post_list:
            # post_public_file_name = get_public_file_name(post_public_fn)
            # rel_post_public_path_fd = os.fd_path.join(blog_name, post_public_file_name)
            # ### post_snippet_html: str = generate_post_snippet(i_p)
            inner_tag_page_html += f'<li><a href="../{post_public_fn}">{post_public_fn}</a></li>'
        inner_tag_page_html += "</ul>"
        # tag_page_public_file_path_str = os.fd_path.join(i_output_dir_path, blog_name, TAGS, )
        tag_page_html = create_html_from_template(i_init_vals.theme_dir_path, i_nav_dict,
    ...
    post_html = create_html_from_template(i_init_vals.theme_dir_path,
        i_nav_dict, i_content_fd_path, public_file_path_str, blog_page_html, PageTypeEnum.standard)
...
def get_long_cli_arg(i_cli_arg: str):
    ret_long_cli_arg: str = i_cli_arg.replace('_', '-')
    ret_long_cli_arg = "--" + ret_long_cli_arg
    return ret_long_cli_arg


def str_to_bool(i_string: str) -> bool:
    if i_string == "true":
        return True
    elif i_string == "false":
        return False
    else:
        raise ValueError("str_to_bool only accepts true and false")
...
def get_initial_values():
    ...
    argument_parser = argparse.ArgumentParser(description="A minimalistic static website generator")
    # All dashes (-) are converted to underscores
    argument_parser.add_argument(get_long_cli_arg(CONTENT_DIR), "-d",
        help="Base dir containing the website (input) content")
    argument_parser.add_argument(get_long_cli_arg(THEME_DIR), "-t",
        help="Path to dir containing the theme, or the name of a built-in theme. "
        "These are the built-in themes: " + ", ".join(availalbe_themes))
    argument_parser.add_argument(get_long_cli_arg(OUTPUT_DIR), "-o",
        help="Path to base dir which will recieve the output (html etc) which will make up the "
             "whole website")
    argument_parser.add_argument(get_long_cli_arg(FORCE_REMOVE_OUTPUT), "-f",
        action=argparse.BooleanOptionalAction,
        help="(true/false) No prompt if and when overwriting the output dir.")
    argument_parser.add_argument(get_long_cli_arg(CONTENT_URL), "-c",
        help="Base URL for editing content online. Only used when your content is hosted remotely")
    argument_parser.add_argument(get_long_cli_arg(WEBSITE_URL), "-w",
    ...
    """
    elif fd_name_str == SETTINGS:
    with open(websitegen.globa.SETTINGS_STR, "w+") as file:  # -Please note: relative path
        settings_content_str = file.read()
        pass
    """
    ...
    for k, v in tmp_config.items():
        ...
        if k == FORCE_REMOVE_OUTPUT:
            file_config[k] = False
            file_config[k] = config_parser.getboolean(
                GENERAL_SECTION, FORCE_REMOVE_OUTPUT.replace('_', '-'))
    ...
    ret_initial_values = InitialValues(content_dir_path, theme_dir_path, output_dir_path,
        content_base_url, website_base_url)
