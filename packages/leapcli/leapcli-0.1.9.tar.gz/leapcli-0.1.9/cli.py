...
Usage:
  leap init (--tensorflow|--pytorch) [PROJECT] [ORG] [DATASET]
...
def login_command(arguments: dict):
    ...
    if not project.is_initialized():
        complain('''Tensorleap project not initialized.Did you run `leap init`?''')

    if arguments['API_ID'] is not None and arguments['API_KEY'] is None:
        complain(f'''Either supply both API_ID and API_KEY or neither. {__doc__}''')
    try:
        auth = Authenticator(project)
        auth.cli_login(arguments.get('API_ID', None), arguments.get('API_KEY', None))
    except MalformedKeys:
        complain(f'''Invalid API_ID or API_KEY. {__doc__}''')
    except KeysMixedUp:
        complain(f'''API_ID should come before API_KEY. {__doc__}''')
...
def push_command():
    project = Project(os.getcwd())
    push = Push(project)
    path, content_hash = push.serialize_model()
    push.import_model(content_hash, path)
...
def init_command(arguments: Dict):
    ...
    try:
        ...
        complain(f'''Tensorleap project already initialized. See {expected_conf}''')
