...
class Project:
    ...
    def project_id(self, api: DefaultApi) -> str:
        ...
        matches = [proj.id for proj in api.get_projects().data if proj.name == self.project]
        assert len(matches) == 1
        return matches[0]

    def dataset_id(self, api: DefaultApi) -> str:
        # TODO: cache this result
        matches = [dataset.id for dataset in api.get_datasets().datasets if dataset.name == self.dataset]
        assert len(matches) == 1
        return matches[0]
