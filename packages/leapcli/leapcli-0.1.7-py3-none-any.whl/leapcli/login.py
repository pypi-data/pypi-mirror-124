...
class Authenticator:
    ...
    def key_login(self) -> None:
        host = self.project.detect_backend_url()
        cfg = Configuration(host=host)
        unauthenticated_client = ApiClient(cfg)
        api = DefaultApi(unauthenticated_client)
        params = KeyLoginParams(self.api_id, self.api_key)
        user, status, headers = api.key_login(params, _return_http_data_only=False)
        if status != 200 or not 'Set-Cookie' in headers:
            _log.info('login failed with api_id: %s, api_key: %s', self.api_id,
                      self.redact_key(self.api_key))
            raise LoginFailed()
        self.user = user
        self.cookie = headers['Set-Cookie']

    def load_credentials(self) -> None:
        self.api_id = Authenticator.detect_api_id()
        self.api_key = Authenticator.detect_api_key()
    ...
    def cli_login(self, api_id: str = None, api_key: str = None):
        ...
        print(f'Authenticated as {self.user.local.email}')
        self.write_credentials(self.api_id, self.api_key)
