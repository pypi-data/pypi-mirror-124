...
class Browser():
	...
	def status_bar(self):
		if not self.N > FRAME_WIDTH:
			return None
		return ansi.codes['light'] + f'{self.sel+1} of {self.N}' + ansi.codes['reset']
	...
	def move_down(self):
		self.sel += 1
		self.frame += (self.frame + FRAME_WIDTH == self.sel)  # scroll down if we are off the screen
	
	def move_up(self):
		self.sel -= 1
		self.frame -= (self.frame - 1 == self.sel)  # scroll down if we are off the screen
	...
	def browse(self):
		...
		while True:
			...
			if self.reviewing:
				...
				self.reviewing = self.selected_card.last_grade != 'exit'
			...
			if command := self.cardlist.browser_commands_dict.get(k):
				command()
			
			if command := self.selected_card.browser_commands_dict.get(k):
				command()
