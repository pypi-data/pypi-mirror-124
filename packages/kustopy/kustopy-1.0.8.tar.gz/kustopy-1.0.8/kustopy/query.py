...
def construct_query(client, database, user_input, truncation):
...
def get_pdf(client, database, user_input, truncation):
    response = construct_query(client, database, user_input, truncation)
    return dataframe_from_result_table(response.primary_results[0]).drop(columns=['iris_id', 'iris_metadata'], errors='ignore')
