...
def Start():
    ...
    pres10 = ['6 занятий курса по продаже AI аутсорс проектов (10 000 руб)',
              'Продвинутый курс по Генеративным сетям 4 занятия (10 000 руб)',
              '5 консультаций с куратором в zoom (10 000 руб)',]
    pres15 = ['Курс «Python для анализа данных» 10 занятий (15 000 руб)',
              'Гарантированная стажировка в Университете (15 000 руб)']
    pres20 = ['Сертификат на английском от американской компании (20 000 руб)']
    pres30 = ['1 месяц Terra AI (30 000 руб)']
    variants = [[3,0], [0,3], [2, 0, 0], [0, 2, 0], [0, 0, 2], [1, 1, 0], [1, 0, 1], [0, 1, 1]]
    presents = [pres10.copy(), pres15.copy(), pres20.copy(), pres30.copy()]
    current_variant = random.choice(variants)
    bonuses = []
    for i in range(len(current_variant)):
        current_present = random.choice(presents[current_variant[i]])
        presents[current_variant[i]].remove(current_present)
        for _ in tqdm_(range(60), desc=f'Выбор бонуса №{i+1}', ncols=500):
            time.sleep(0.1)
        print('Бонус: ',current_present)
        time.sleep(2)
        display.clear_output(wait=True)
        bonuses.append(current_present)
        print(colored('Поздравляем!', color='green', attrs=['bold']), end=' ')
        print('Ваш список бонусов:')
        for b in bonuses:    
            print(f'- {b}')
    ...
    pr = colored('Отлично, эти подарки теперь забронированы за вами!', color = 'green', attrs=['bold'])
