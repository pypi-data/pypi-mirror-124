...
class Menus_imagesQuerySet(Model_imagesQuerySet):
    pass


class Menus_imagesManager(Model_imagesManager):
    ...
    def get_queryset(self):
        return Menus_imagesQuerySet(self.model, using=self._db)


class Menus_images(Model_images):
    image = ForeignKeyProtect(Images)
    main_model = ForeignKeyCascade(Menus)
    type = ForeignKeyProtect(Image_types)

    objects = Menus_imagesManager()
