...
def validate_yaml(jobs_data, checks_data, repo_root, prod_name):
    ...
    for query, job_set in bk_query_to_job.items():
        for job_name in job_set:
            if ("up" in job_name and "up" not in query) or (
                "down" in job_name and "down" not in query
            ):
                raise ValueError(
                    f"The magnet polarity specified in the job name ({job_name}) does not match with its input: {query}"
                )
