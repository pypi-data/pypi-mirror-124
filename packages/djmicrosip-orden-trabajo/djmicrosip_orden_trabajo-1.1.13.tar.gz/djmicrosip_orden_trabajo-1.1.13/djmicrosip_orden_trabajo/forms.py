...
class VentasDetalleDocumentoForm(forms.ModelForm):
	#articulo = forms.ModelChoiceField(queryset=Articulo.objects.all().order_by('nombre'), widget=ArticuloWidget, required=True)
...
class PedidoCrmForm(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		if hardware_rows:
			for hw in hardware_rows:
				try:
					...
					hw_option = [hw, hw]
					hws.append(hw_option)
		print(hws)
		self.fields['hardware'].required=False
		#self.fields['hardware'].widget.attrs['class'] = 'form-control'
		self.fields['hardware'].choices=hws
		...
		self.fields['descripcion_solicitud'].widget.attrs['rows'] = 3
		self.fields['preprogramado'].required=False		
		self.fields['descripcion_solicitud'].required=False		
		self.fields['persona_solicito'].required=False		

	class Meta:
		model = Pedidos_Crm
		fields = ('hardware','descripcion_otros','precio_aproximado','descripcion_general','persona_solicito',
				'tipo_servicio','llamada','tipo_llamada','fecha_registro','preprogramado','descripcion_solicitud')
...
class PreferenciasManageForm(forms.Form):
	...
	hardware=forms.CharField(max_length=10000,widget=forms.TextInput(attrs={'class': 'form-control'}),required=False)
	...
	def save(self, *args, **kwargs):
		...
		hardware = Registry.objects.get( nombre = 'SIC_Pedidos_Crm_Hardware')
		hardware.valor = self.cleaned_data['hardware']
		hardware.save()
