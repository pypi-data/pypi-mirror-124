...
def add_pedido(request,folio,template_name='djmicrosip_orden_trabajo/add_pedido.html'):
	...
	if pedido:
		...
		venta_det=VentasDocumentoDetalle.objects.filter(documento=venta)
		initial_detalle=[]
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			if not venta:
				...
				form.instance.condicion_pago=venta_fecha.condicion_pago
			...
			if formset.is_valid():
				...
				if form_crm.is_valid():
					...
					if pedido:
						form_crm.save()
						return redirect("/pedidos/pedido/"+str(form_crm.instance.folio)+"/")
...
def actualiza_base_datos(request):
	...
	try:
		...
		col_names = []
		management.call_command('makemigrations', 'models_base')
		management.call_command('migrate', 'models_base',database=using, interactive=False)
	...
	if not Registry.objects.filter( nombre = 'SIC_Pedidos_Crm_Hardware' ).exists():
			Registry.objects.create(
				nombre = 'SIC_Pedidos_Crm_Hardware',
				tipo = 'V',
				padre = padre,
				valor= '',
			)
	...
	if not Registry.objects.filter( nombre = 'SIC_Pedidos_Crm_Url_Consecutivo_Solicitud' ).exists():
			Registry.objects.create(
				nombre = 'SIC_Pedidos_Crm_Url_Consecutivo_Solicitud',
				tipo = 'V',
				padre = padre,
				valor= '',
			)
			consecutivo = Registry.objects.get( nombre = 'SIC_Pedidos_Crm_Url_Consecutivo_Solicitud')
			consecutivo.valor =1
			consecutivo.save()
...
def preferencias(request):
	...
	hardware= Registry.objects.get( nombre = 'SIC_Pedidos_Crm_Hardware').get_value()
	hardware_rows=hardware.split(',')
	print(hardware_rows)
...
def pedido_solictud(request):
	usuario=request.user
	direccion_cliente=ClienteDireccion.objects.filter(rfc_curp=usuario.username,es_ppal='S').first()
	print(direccion_cliente.cliente)
	cliente_clave=None
	sucursal_id=None
	id_articulo=Registry.objects.get(nombre='SIC_Pedidos_Crm_Articulo_predeterminado').get_value()
	articulo=Articulo.objects.get(id=id_articulo)
	clave_articulo=ArticuloClave.objects.filter(articulo__id=id_articulo).first()
	if direccion_cliente:
		#cliente=direccion_cliente.cliente
		#direcciones_cliente = ClienteDireccion.objects.filter(cliente_id=cliente.id)
		cliente_clave = ClienteClave.objects.filter(cliente__id=direccion_cliente.cliente.id)
		moneda=direccion_cliente.cliente.moneda
		condicion_de_pago=direccion_cliente.cliente.condicion_de_pago
		# print(moneda.id,condicion_de_pago.id)
		# print(direcciones_cliente)
	...
	lista_direccion={}
	direccion={}
	almacen=Almacen.objects.filter(es_predet='S').first()
	form_initial={
		'tipo':'P',
		'fecha':datetime.now(),
		'cliente':direccion_cliente.cliente,
		'cliente_clave':clave_cliente,
		'direccion_consignatario':direccion_cliente,
		'almacen':almacen,
		'moneda':moneda,
		'condicion_pago':condicion_de_pago,
		'vendedor':None,
		'uso_cfdi':None,
		'descuento_porcentaje':None,
		'descuento_importe':None,
		'modalidad_facturacion':None,
		'sistema_origen':'VE',
		'metodo_pago_sat':None,
		'importe_neto':0,
		'tipo_cambio':None,
		'descripcion':'',
		'creacion_usuario':usuario.username,
		'estado':'P',
	}
	form=PedidoAddForm(request.POST or None,initial=form_initial)
	form_orden=PedidoCrmForm(request.POST or None)
	bdatos = request.session["selected_database"]
	conexion = request.session["conexion_activa"]
	...
	if request.method=='POST':
		...
		if form.is_valid():
			if form_orden.is_valid():
				form.instance.tipo='S'
				form.instance.fecha=datetime.now()
				form.instance.cliente=direccion_cliente.cliente
				form.instance.cliente_clave=clave_cliente
				form.instance.direccion_consignatario=direccion_cliente
				form.instance.cliente_direccion=direccion_cliente
				form.instance.almacen=almacen
				form.instance.moneda=moneda
				form.instance.condicion_pago=condicion_de_pago
				form.instance.vendedor=None
				form.instance.uso_cfdi=None
				form.instance.descuento_porcentaje=0
				form.instance.descuento_importe=0
				form.instance.modalidad_facturacion=None
				form.instance.sistema_origen='VE'
				form.instance.metodo_pago_sat=None
				form.instance.importe_neto=0
				form.instance.tipo_cambio=1
				form.instance.descripcion=''
				form.instance.creacion_usuario=usuario.username
				form.instance.estado='P'
				form.instance.aplicado='N'
				print(form.instance.tipo)
				if sucursal_id:
					form.instance.sucursal_id=sucursal_id
				
				descripcion_solicitud=form_orden.cleaned_data['descripcion_solicitud']
				nombre_solicito=form_orden.cleaned_data['persona_solicito']

				object_save=form.save()
				print(object_save)
				print(descripcion_solicitud)
				print(clave_articulo)
				detalle=VentasDocumentoDetalle.objects.create(id=-1,documento=object_save,articulo=articulo,unidades=1,precio_unitario=0,
				descuento_porcentaje=0,precio_total_neto=0,notas=descripcion_solicitud,articulo_clave=clave_articulo.clave)
				
				pedido_crm=Pedidos_Crm.objects.get_or_create(folio=object_save.folio,bdatos=bdatos,conexion=conexion,
				hardware='',descripcion_general='',precio_aproximado=0,tipo_servicio='',persona_solicito=nombre_solicito,
				tipo_llamada='',llamada=False,solicitud=True,descripcion_solicitud=descripcion_solicitud,fecha_registro=datetime.now(),preprogramado=False,venta=form.instance)
...
def lista_solicitudes(request):
	usuario=request.user
	direccion_cliente=ClienteDireccion.objects.filter(rfc_curp=usuario.username,es_ppal='S').first()
	pedidos_all=Pedidos_Crm.objects.filter(solicitud=True,venta__cliente=direccion_cliente.cliente)
