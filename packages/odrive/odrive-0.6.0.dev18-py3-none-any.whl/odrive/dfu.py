...
class FirmwareFile():
    ...
    async def load(self):
        content = await self.loader(self._json['file'])
    ...
    def get_flash_sections(self) -> Iterator[tuple[int, bytes]]:
        ...
        """
        with io.BytesIO(self._content) as stream:
...
class FirmwareIndex():
    @staticmethod
    def from_json(base_path: str, json: dict, loader):
        return FirmwareIndex([FirmwareFile(f, loader) for f in json['files']])
    ...
    async def from_server(session: aiohttp.ClientSession, base_path='https://cdn.odriverobotics.com/firmware'):
        ...
        async def load_file(file: str):
            async with session.get(base_path + '/' + file) as response:
                assert response.status == 200, f"{response.status} {response.reason}"
                return await response.read()

        async with session.get(base_path + '/index.json') as response:
            assert response.status == 200, f"{response.status} {response.reason}"
            return FirmwareIndex.from_json(base_path, await response.json(), load_file)
    ...
    def find(self, app: str, product: str, channel: str) -> FirmwareFile:
        for file in self.files:
            if (file.app, file.product, file.channel) == (app, product, channel):
                return file
        return None
...
async def write_firmware(device, firmware: FirmwareFile, dfu_discoverer: DfuDeviceDiscovery, logger, erase_all: bool = True):
    """
...
async def launch_dfu(serial_number: str, file: str, channel: str, logger):
    """
    ...
    2. If `file` is None, check for the latest firmware, present it to the user
    ...
    file: Path to a .elf file or None to check online.
    ...
    async with DfuDeviceDiscovery() as dfu_discoverer:
        ...
        if file is None:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                firmware_index = await FirmwareIndex.from_server(session)
                file = firmware_index.find('default', product, channel)

                if file is None:
                    raise DfuError(f"No {product} firmware found for on channel {channel}.")
                ...
                if device_commit is None or (file.commit[:8] != device_commit):
                    if not odrive.utils.yes_no_prompt(f"Found new firmware ({device_commit} => {file.commit[:8]}). Install now?", True):
                        return
                else:
                    if not odrive.utils.yes_no_prompt(f"Your current firmware ({file.commit[:8]}) is up to date. Do you want to reinstall this version?", True):
                        return
                logger.info("Downloading firmware...")
                await file.load()
        else:
            assert channel is None
            file = FirmwareFile.from_file(file)
