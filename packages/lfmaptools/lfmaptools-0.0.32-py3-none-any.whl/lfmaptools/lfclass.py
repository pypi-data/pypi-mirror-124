...
class LaharFlowData(object):
    ...
    def _snapshot_nc_files(self):
        nc_files = [f.strip('.nc') for f in self.txt_files if f.strip('.nc').isnumeric()]
        nc_files.sort()
        return nc_files

    def get_snapshot_files(self):
        txt_files = self._snapshot_txt_files()
        nc_files = self._snapshot_nc_files()
    ...
    def _get_data_nc(self, LaharFlowFile):
        rgrp = nc.Dataset(LaharFlowFile, 'r')
    ...
    def _get_nc_max(self):
        ...
        try:
            maxE, maxD = nc_min_max(last_outfile, var='elevation_change')
    ...
    def POI_buffer(self,  lon, lat, buffer, data=None, filename=None, var=None,
        ...
        data_utm = data.rio.reproject(self.utmCode)

        df = pd.DataFrame({'Name':name, 'Longitude':[lon], 'Latitude':[lat]})
        gdf = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.Longitude, df.Latitude), crs=wgs84)

        gdf = gdf.to_crs(self.utmCode)
        gdf['geometry'] = gdf.geometry.buffer(buffer)

        (l, b, r, t) = data_utm.rio.bounds()
        bbox = gpd.GeoDataFrame(geometry=[box(l,b,r,t)], crs=self.utmCode)

        if not gdf.within(bbox)[0]:
            return nodata

        clip_poly = gdf.intersection(bbox)

        if fill_value is not None:
            data_utm = data_utm.fillna(fill_value)

        data_clipped = data_utm.rio.clip(clip_poly)
