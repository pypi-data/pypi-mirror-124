...
@click.option("--color-always", is_flag=True, default=True)
@click.option(
    "--baseline", default=DEFAULT_BASELINE_PATH, help="baseline file (yaml)."
)
def main(url, baseline, verbose=False, color_always=False):
    ...
    with resources.as_file(resources.files("sslcompare")) as sslcompare_path:
        with subprocess.Popen(
            ...
            for line in testssl.stdout:
                ...
                if "Start" in line:
                    click.echo(line, color=color_always)
                    ...
                    if line == " -":
                        if current_protocol in [
                            ...
                            click.echo(
                                f" {'NOT OFFERED':<41}", color=color_always
                            )
