...
class ProtImportDiffractionImages(EdBaseProtocol):
    ...
    def convertInputStep(self, pattern):
        self.loadPatterns()
        self.info(f"Using glob pattern: '{self._globPattern}'")
        self.info(f"Using regex pattern: '{self._regexPattern}'")
    ...
    def _validate(self):
        ...
        if len(glob(self._globPattern)) < 1:
            errors.append("No files found. This might be due to an empty "
                          "directory or using the wrong file format")
    ...
    def _summary(self):
        summary = []
        summary.append(
            f'Data in {self.getFileParents(self.getMatchingFiles())}')
    ...
    def getRotationAxis(self):
        try:
            ...
            self.debug(f"Failed getting rotation axis due to exception:\n{e}")
            axis = None
        return axis

    def getNewDetectorDistance(self):
        newDistance = self.overwriteDetectorDistance.get()
        if newDistance in (None, "None", ""):
            # This, together with the function name, should potentially be
            # changed in the future to use the output from readSmvHeader or
            # equivalent
            return None
        else:
            return newDistance
