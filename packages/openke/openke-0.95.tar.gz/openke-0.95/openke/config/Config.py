...
from openke.utils import generate_type_constraint_specification
...
class Config(object):
    ...
    def __init__(self):
        base_file = "/usr/lib/libmeager.so"
    ...
    def set_in_path(self, path):
        self.in_path = path
        generate_type_constraint_specification(path, exist_ok=True)
        ...
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                if self.test_link_prediction:
                    ...
                    # print(self.lib)
                    self.lib.test_link_prediction(True, False, True)
