...
class UNet3d_Plus(nn.Module):
    def __init__(self, in_channels = 1, out_channels = 2, n_channels = 32, channel_bound = 320):
        ...
        self.classifier2 = nn.Conv3d(n_channels, out_channels, kernel_size = 1)
        self.classifier3 = nn.Conv3d(n_channels, out_channels, kernel_size = 1)
        self.classifier4 = nn.Conv3d(n_channels, out_channels, kernel_size = 1)
...
class UNet3d_MultiModal(nn.Module):
    ...
    def forward(self, CT, PET):
        ...
        l2_output = self.upsample2(self.classifier2(l2_output_CT * l2_output_PET))
        l3_output = self.upsample3(self.classifier3(l3_output_CT * l3_output_PET))
        l4_output = self.upsample4(self.classifier4(l4_output_CT * l4_output_PET))
