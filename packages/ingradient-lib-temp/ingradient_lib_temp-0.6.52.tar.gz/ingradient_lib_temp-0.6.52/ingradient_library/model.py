...
class Deep_Supervision_UNet3d(nn.Module):
    def __init__(self, n_modalities = 4, final_output_channels = 5, patch_size = (128, 128, 128), is_binary = False):
        ...
        conv_kwargs1 = {'kernel_size' : 3, 'padding' : 1,  'stride' : 1}
        conv_kwargs2 = {'kernel_size' : 3, 'stride' : 1, 'padding':1}
        transp_kwargs = {'kernel_size' : 3, 'padding' : 1, 'stride' : 2}
    ...
    def forward(self, x):
        ...
        x4_ = self.decoder1_conv(torch.cat((x4, self.decoder1_trans(x5, output_size = (x4.shape[-3], x4.shape[-2], x4.shape[-1]))), dim = 1))
        x3_ = self.decoder2_conv(torch.cat((x3, self.decoder2_trans(x4_, output_size = (x3.shape[-3], x3.shape[-2], x3.shape[-1]))), dim = 1))
        x2_ = self.decoder3_conv(torch.cat((x2, self.decoder3_trans(x3_, output_size = (x2.shape[-3], x2.shape[-2], x2.shape[-1]))), dim = 1))
        x1_ = self.decoder4_conv(torch.cat((x1, self.decoder4_trans(x2_, output_size = (x1.shape[-3], x1.shape[-2], x1.shape[-1]))), dim = 1))
