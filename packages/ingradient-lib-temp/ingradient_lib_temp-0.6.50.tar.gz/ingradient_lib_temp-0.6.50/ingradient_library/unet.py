...
class UNet_Plus3d(nn.Module):
    ...
    def forward(self, x):
        ...
        l3_output = self.decoder4(torch.cat(x_downsmaple3_up, x_downsample2), dim = 1)
        x_downsmaple2_up = self.conv_upsample3(l3_output)
        l2_output = self.decoder4(torch.cat(x_downsmaple2_up, x_downsample1), dim = 1)
        x_downsmaple1_up = self.conv_upsample4(l2_output)
        l1_output = self.decoder4(torch.cat(x_downsmaple1_up, x), dim = 1)
