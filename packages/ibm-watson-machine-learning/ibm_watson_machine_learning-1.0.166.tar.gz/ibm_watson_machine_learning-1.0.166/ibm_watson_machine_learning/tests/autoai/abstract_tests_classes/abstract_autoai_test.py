...
class AbstractTestAutoAIAsync(abc.ABC):
    ...
    def test_05b_create_model_from_training(self):
        self.wml_client.software_specifications.list()
        software_spec_uid = self.wml_client.software_specifications.get_id_by_name('autoai-kb_3.1-py3.7')

        meta_props = {
            self.wml_client.repository.ModelMetaNames.NAME: 'test model',
        ...
        # Save model from the training
        trained_model_details = self.wml_client.repository.store_model(model=self.run_id, meta_props=meta_props)

        self.wml_client.repository.delete(trained_model_details['metadata']['id'])
