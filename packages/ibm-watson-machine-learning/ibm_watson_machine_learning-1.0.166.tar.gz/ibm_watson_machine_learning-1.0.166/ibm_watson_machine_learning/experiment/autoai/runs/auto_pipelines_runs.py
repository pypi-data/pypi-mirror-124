...
class AutoPipelinesRuns(BaseAutoPipelinesRuns):
    ...
    def get_params(self, run_id: str = None) -> dict:
        ...
        if kb_node_number is not None:
            ...
            if kb_parameters.get('text_processing_options') and kb_parameters['text_processing_options'].get('word2vec'):
                params['word2vec_feature_number'] = kb_parameters['text_processing_options']['word2vec'].get('output_dim')
            if kb_parameters['learning_type'] == 'timeseries':
                params['backtest_num'] = kb_parameters.get('backtest_num')
                params['lookback_window'] = kb_parameters.get('lookback_window')
                params['forecast_window'] = kb_parameters.get('forecast_window')
                params['backtest_gap_length'] = kb_parameters.get('backtest_gap_length')
        else:
            params = {
                'name': name,
                'desc': description,
