...
class BaseModelMixin:
    ...
    @classmethod
    def to_dict_field_filter(cls, field, *args, **kwargs):
        """
        to_dict callback.
    ...
    @classmethod
    def get_columns_json(cls, json_data):
        """
        ...
        for col in cls.get_columns():
            ...
            if field in json_data and field not in auto_fields and not col.info.get('auto', False):
    ...
    def check_add_data(cls, json_data):
        ...
        return cls._check_unique(json_data)

    @classmethod
    def add_db(cls, json_data):
        """
    ...
    @classmethod
    def bulk_add(cls, items, with_relationship):
        """
    ...
    # -------------------------------------------update-------------------------------------------
    @classmethod
    def check_update_data(cls, json_data):
        """
        ...
        exist = cls._check_exist(json_data)
        if exist is not True:
            return exist
        return cls._check_unique(json_data)

    @classmethod
    def update_db(cls, json_data):
        """
        ...
        :return:
        """
        pk_value = cls._get_pk_value(json_data)
        instance = cls.query_by_pk(pk_value)
        with db_session() as session:
            # Only the fields in json will be updated
            for field in cls.get_columns_json(json_data):
                setattr(instance, field, json_data.get(field))
            relationships = create_relationships(cls, json_data)
            for field in relationships:
                setattr(instance, field, relationships[field])
        return instance

    @classmethod
    def batch_update(cls, items, with_relationship):
        """
    ...
    def check_delete_data(cls, pk_value):
        ...
        json_data = {primary_field: pk_value}

        return cls._check_exist(json_data)
    ...
    @classmethod
    def query_by_unique_key(cls, json_data):
        """
    ...
    # -------------------------------------------check-------------------------------------------
    @classmethod
    def _get_pk_value(cls, json_data):
        """
        ...
        primary_field = cls.get_primary_field()
        if primary_field:
            return json_data.get(primary_field)

    @classmethod
    def _check_exist(cls, json_data):
        """
