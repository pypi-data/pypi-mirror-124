...
def create_instance(model_cls, json_data, create_relationship=True):
    """
    Create an instance of the specified model class with the data
    :param model_cls: The specified model class.
    :param json_data: The data used to create instance.
    ...
    col_value_dict = model_cls.get_columns_json(json_data)
    instance = model_cls(**col_value_dict)

    if create_relationship is True:
        relationships = create_relationships(model_cls, json_data)
        for key in relationships:
            setattr(instance, key, relationships[key])
    return instance


def create_relationships(model_cls, json_data):
    """
    ...
    for key in relationships.keys():
        ...
        relationship_kwargs = json_data.get(key)
...
def model_to_dict(data, option=None):
    """
    ...
    if isinstance(data, list):
        ...
        return data_list
    elif isinstance(data, ModelMixin):
        return data.to_dict(option)
    else:
        return data
