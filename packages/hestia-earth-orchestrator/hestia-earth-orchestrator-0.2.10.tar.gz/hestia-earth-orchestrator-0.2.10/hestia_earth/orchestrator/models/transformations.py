...
def _previous_transformation(cycle: dict, transformation: dict):
    term_id = transformation.get('previousTransformationTerm', {}).get('@id')
    return next(
        (v for v in cycle.get('transformations', []) if v.get('term', {}).get('@id') == term_id),
        cycle
    )
...
def _run_transformation(cycle: dict, models: list):
    def run(transformation: dict):
        previous = _previous_transformation(cycle, transformation)
        transformation = _apply_transformation_share(previous, transformation)
        return _run_models(cycle, transformation, models)
    return run


def run(models: list, cycle: dict):
    transformations = cycle.get('transformations', [])
    result = list(map(_run_transformation(cycle, models), transformations))
    return result
