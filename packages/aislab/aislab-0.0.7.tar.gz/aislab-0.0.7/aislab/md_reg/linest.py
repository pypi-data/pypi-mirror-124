...
def elspm(U, Y, na=0, nb=0, nc=0, pm0=0, maxiter=50, axcnv=None, xcnv=1e-8, afcnv=None, fcnv=1e-8, gcnv=None,
    ...
    return Pm, E
##############################################################################
def elspm_apl(U, Y, E, Pm, na=0, nb=0, nc=0, pm0=0):
...
def elspv(U, Y, na=0, nb=0, nc=0, pm0=0, maxiter=50, axcnv=None, xcnv=1e-8, afcnv=None, fcnv=1e-8, gcnv=None,
    ...
    Ymab = lspv_apl(U[nn:], Y[nn:], arx, na=na, nb=nb, pm0=pm0)
    if nc.size == 0:
        E = np.empty((0, 0))
        return arx, E
    ...
    f = np.sum(np.diag(E.T@E) / N)
    pm = np.vstack((arx, np.zeros((int(np.sum(nc)), 1))))
    ...
    return pm, E
##############################################################################
def elspv_apl(U, Y, E, pm, na=0, nb=0, nc=0, pm0=0):
    F = dmpm(U, Y, E, na, nb, nc, pm0)
    Pm = pv2m(pm, na, nb, nc, pm0)
    Ym = F@Pm
    return Ym
##############################################################################
# glspm():
...
##############################################################################
def lspm_apl(U, Y, Pm, na=0, nb=0, pm0=0):
    if max(na, nb) == 0:
        if pm0: U = np.hstack((np.ones((U.shape[0], 1)), U))
        Ym = U@Pm
    else:
...
def lspv(U=np.empty((0, 0)),
    ...
    return c_(pm)
##############################################################################
def lspv_apl(U, Y, pm, na=None, nb=None, pm0=0):
