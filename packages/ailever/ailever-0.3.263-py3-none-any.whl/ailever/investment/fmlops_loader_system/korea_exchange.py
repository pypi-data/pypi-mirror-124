...
def download(n=30, queue=queue):
    ...
    for KI in tqdm(KIs):
        ...
        korea_index.to_csv(os.path.join(CORE_MS1.path,f'{KI}.csv'))

    krx = fdr.StockListing('KRX')
    if not os.path.isfile(os.path.join(CORE_MS1.path, f'KRX.csv')):
        krx.to_csv(os.path.join(CORE_MS1.path,'KRX.csv'))
...
def korea_ex(date='2010-01-01', mode='Close', cut=None, baskets=None):
    ...
    """
    stock_list = pd.read_csv(os.path.join(CORE_MS1.path, 'KRX.csv')).drop('Unnamed: 0', axis=1)
    ...
    KI_dict = {}
    for KI in KIs:
        df = pd.read_csv(os.path.join(CORE_MS1.path, f'{KI}.csv'))
        KI_dict[KI] = df
