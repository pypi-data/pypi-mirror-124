...
class ProcessQubo(ProcessParams):
    ...
    @staticmethod
    def __check_symmetric_ndarray(data, rtol=1e-06, atol=1e-09):
        """
        See:
    ...
    def check_objective(self, data):
        ...
        if isinstance(data, np.ndarray):
            assert self.__check_symmetric_ndarray(data=data), \
    ...
    def __check_dwave_size(self, data):
        ...
        max_fully_connected = [self.DWAVE_MAX_CLIQUES[k] for k in self.DWAVE_MAX_CLIQUES if k in self.device.name]
