...
from multiprocessing_stress_tester.debugger import stresstest
...
def submit(cpp_files, safety, testcase, timelimit):
    ...
    print(f"Starting stresstest ({np.round(time.perf_counter() - start, 3)} sec)")
    # executing stress-test 
    dbg_res = stresstest(safety, testcase, timelimit)
    print(f"Stresstest completed ({np.round(time.perf_counter() - start, 3)} sec)")
    ...
    print(f"Removing temporary files ({np.round(time.perf_counter() - start, 3)} sec)")
    ...
    print(f"Removed temporary files ({np.round(time.perf_counter() - start, 3)} sec)")
