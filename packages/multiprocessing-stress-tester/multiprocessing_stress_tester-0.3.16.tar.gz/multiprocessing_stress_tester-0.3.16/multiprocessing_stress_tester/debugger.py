...
def stresstest(safe, tcs, tl):
    ...
    clear_files(['log.txt'])
    print(f"Starting compiling... (from stresstest: {round(time.perf_counter() - start, 3)} sec)"
    compilation = compile_files(safe)
    if compilation != 0:
        return compilation
    print(f"Compilation completed (from stresstest: {round(time.perf_counter() - start, 3)} sec)"
    execution = execute(tcs, tl)
    if execution != 0:
        execution += 4
    print(f"Execution completed (from stresstest: {round(time.perf_counter() - start, 3)} sec)"
    return execution
