...
from nbplusplus.data_model.code_cell import cells
...
def dynamic_or_static_only_test(test_f, dynamic_enabled, static_enabled):
    ...
    def dynamic_test_f(*args, **kwargs):
        orig_dynamic_enabled = nbpp().mut_settings.dynamic_slicing_enabled
        orig_static_enabled = nbpp().mut_settings.dynamic_slicing_enabled
        try:
            nbpp().mut_settings.dynamic_slicing_enabled = dynamic_enabled
            nbpp().mut_settings.static_slicing_enabled = static_enabled
            test_f(*args, **kwargs)
        finally:
            nbpp().mut_settings.dynamic_slicing_enabled = orig_dynamic_enabled
            nbpp().mut_settings.static_slicing_enabled = orig_static_enabled
...
def test_nested_symbol_usage_with_variable_subscript():
    for static_slicing_enabled in [True, False]:
        if not static_slicing_enabled and not nbpp().mut_settings.dynamic_slicing_enabled:
            continue
        orig_enabled = nbpp().mut_settings.static_slicing_enabled
        try:
            nbpp().mut_settings.static_slicing_enabled = static_slicing_enabled
            run_cell('x = 1')
            run_cell('lst = [1, 2, 3, 4, 5]')
            run_cell('lst[x] = 3')
            run_cell('logging.info(lst[1])')
            ...
            nbpp().mut_settings.static_slicing_enabled = orig_enabled
...
def test_dynamic_only_increment():
    nbpp().mut_settings.static_slicing_enabled = False
    run_cell('x = 0')
    run_cell('x += 1')
