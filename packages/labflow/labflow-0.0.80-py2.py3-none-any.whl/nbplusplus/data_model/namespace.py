...
class Namespace(Scope):
    ...
    def __init__(self, obj: Any, *args, **kwargs) -> None:
        ...
        if obj is not None and not isinstance(obj, int) and id(obj) in nbpp().namespaces:  # pragma: no cover
    ...
    def is_garbage(self) -> bool:
        return self._tombstone or self.obj_id not in nbpp().aliases or self.obj_id not in nbpp().namespaces

    @property
    def is_subscript(self) -> bool:
        dsym = nbpp().get_first_full_symbol(self.obj_id)
        if dsym is None:
            return False
        else:
    ...
    def update_obj_ref(self, obj) -> None:
        self._tombstone = False
        nbpp().namespaces.pop(self.cached_obj_id, None)
        self.obj = obj
        self.cached_obj_id = id(obj)
        nbpp().namespaces[self.cached_obj_id] = self
    ...
    def transfer_symbols_to(self, new_ns: Namespace) -> None:
        for dsym in list(self.all_data_symbols_this_indentation(exclude_class=True, is_subscript=False)):
            try:
                inner_obj = nbpp().retrieve_namespace_attr_or_sub(new_ns.obj, dsym.name, is_subscript=False)
        ...
        for dsym in list(self.all_data_symbols_this_indentation(exclude_class=True, is_subscript=True)):
            try:
                inner_obj = nbpp().retrieve_namespace_attr_or_sub(new_ns.obj, dsym.name, is_subscript=True)
