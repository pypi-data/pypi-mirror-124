...
import nbplusplus.tracing.mutation_event as me
from nbplusplus.analysis.symbol_edges import get_symbol_edges
from nbplusplus.analysis.symbol_ref import SymbolRef
from nbplusplus.analysis.utils import stmt_contains_lval
from nbplusplus.data_model.data_symbol import DataSymbol
...
from nbplusplus.tracing.symbol_resolver import resolve_rval_symbols
from nbplusplus.tracing.utils import match_container_obj_or_namespace_with_literal_nodes
...
class TraceStatement:
    ...
    def get_post_call_scope(self):
        ...
        if isinstance(self.stmt_node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            func_name = None
        func_sym = nbpp().statement_to_func_cell.get(id(self.stmt_node), None)
        if func_sym is None:
        ...
        try:
            ...
            for resolved in symbol_ref.gen_resolved_symbols(
                ...
                if resolved.is_reactive and not blocking_seen:
                    nbpp().updated_deep_reactive_symbols.add(resolved.dsym)
                    reactive_seen = True
                if reactive_seen and not blocking_seen:
                    nbpp().updated_reactive_symbols.add(resolved.dsym)
                if blocking_seen and resolved.dsym not in nbpp().updated_symbols:
                    nbpp().blocked_reactive_timestamps_by_symbol[resolved.dsym] = nbpp().cell_counter()
        except TypeError:
            return
    ...
    def _handle_assign_target_for_deps(
        ...
        if maybe_fixup_literal_namespace:
            namespace_for_upsert = nbpp().namespaces.get(id(obj), None)
    ...
    def _handle_starred_store_target(self, target: ast.Starred, inner_deps: List[Optional[DataSymbol]]):
        try:
            ...
            return
        ns = nbpp().namespaces.get(id(obj), None)
        if ns is None:
    ...
    def _handle_store_target_tuple_unpack_from_namespace(
        ...
        for (i, inner_dep), (_, inner_target) in match_container_obj_or_namespace_with_literal_nodes(rhs_namespace, target):
            ...
            if isinstance(inner_target, (ast.List, ast.Tuple)):
                inner_namespace = nbpp().namespaces.get(inner_dep.obj_id, None)
    ...
    def _handle_store_target(self, target: ast.AST, value: ast.AST, skip_namespace_check: bool = False):
        if isinstance(target, (ast.List, ast.Tuple)):
            rhs_namespace = (
                ...
                else nbpp().namespaces.get(id(tracer().saved_assign_rhs_obj), None)
            )
    ...
    def _make_lval_data_symbols_old(self):
        ...
        for target, dep_node in symbol_edges:
            ...
            if is_class_def:
                ...
                nbpp().namespaces[id(class_ref)] = self.class_scope
            try:
                scope, name, obj, is_subscript, excluded_deps = tracer().resolve_store_data_for_target(target, self.frame)
                scope.upsert_data_symbol_for_name(
    ...
    def _handle_specific_mutation_type(
        ...
        namespace_scope = nbpp().namespaces.get(mutated_obj_id, None)
        mutated_sym = nbpp().get_first_full_symbol(mutated_obj_id)
    ...
    def handle_dependencies(self):
        if not nbpp().dependency_tracking_enabled:
            return
        for mutated_obj_id, mutation_event, mutation_arg_dsyms, mutation_arg_objs in tracer().mutations:
            ...
            Timestamp.update_usage_info(nbpp().aliases[mutated_obj_id])
            for mutated_sym in nbpp().aliases[mutated_obj_id]:
