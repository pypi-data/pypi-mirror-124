...
from nbplusplus.analysis.symbol_ref import get_attrsub_symbol_chain, LiveSymbolRef, SymbolRef, Atom
from nbplusplus.analysis.mixins import SaveOffAttributesMixin, SkipUnboundArgsMixin, VisitListsMixin
...
class ComputeLiveSymbolRefs(SaveOffAttributesMixin, SkipUnboundArgsMixin, VisitListsMixin, ast.NodeVisitor):
    ...
    def visit_Assign_impl(self, targets, value, aug_assign_target=None) -> None:
        ...
        if (
            nbpp().mut_settings.flow_order == FlowOrder.ANY_ORDER
    ...
    def visit_Name(self, node: ast.Name) -> None:
        ...
        if self._in_kill_context:
            ...
            if id(node) in nbpp().reactive_node_ids:
        ...
        for resolved in live_symbol_ref.gen_resolved_symbols(
            ...
            if update_liveness_time_versions:
                ...
                if ts_to_use.is_initialized:
                    nbpp().add_static_data_dep(liveness_time, ts_to_use)
