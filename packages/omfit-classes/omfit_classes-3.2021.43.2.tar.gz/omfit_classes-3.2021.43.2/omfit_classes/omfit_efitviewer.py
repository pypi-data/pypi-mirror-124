...
def load_cx_ods(gfile=None, device=None, shot=None, t=None, efitid=None, index=0, no_empty=None):
    """
    ...
    if gfile is None:
        load_cmd = "setup_hardware_overlay_cx({}, {}, {}, efitid={}, default_load=False, no_empty={})".format(
            repr(device), repr(shot), repr(t), repr(efitid), no_empty
        )
        ods = out[ods_tag] = setup_hardware_overlay_cx(device, shot, t, efitid=efitid, default_load=False, quiet=True, no_empty=no_empty)
    else:
        ods = out[ods_tag] = setup_hardware_overlay_cx(device, geqdsk=gfile, default_load=False, quiet=True, no_empty=no_empty)
        load_cmd = "setup_hardware_overlay_cx({}, geqdsk={}, default_load=False, no_empty={})".format(repr(device), gfile_str, no_empty)
