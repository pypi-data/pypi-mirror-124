...
class HelperVoronoiTarget(FunctionTarget):
    ...
    def compute(self, x):
        ...
        tmp_f = ops.index_update(tmp_f, ops.index[self.x_id], -1)
...
class NLPSolver:
    ...
    def barrier_method(self):
        ...
        x_size: int = x.shape[0]
        self.option.barrier.t0 = max(
            2e2, min(self.option.barrier.t0 * np.sqrt(x_size), 5e3)
        )
        t = self.option.barrier.t0
        i = 0
        while (
            i < self.option.max_iter.barrier and x_size / t >= self.option.prec.barrier
        ):
            i += 1
    ...
    def newton_method(self, x, t):
        ...
        while (
            ...
            and crit
            >= self.option.prec.newton * max(1.0, 1e4 * np.power(2, -iter_barrier))
            and not backline_exceeded
        ):
            i += 1
            # condition inequality constraints
            if self.minimize_option.lin_equal_constr.enabled:
                A = ops.index_update(A, ops.index[:x_size, :x_size], t * func_hes)
                b = ops.index_update(b, ops.index[:x_size], -t * func_grad)
    ...
    def pre_build_newton_linear_constraint_matrix(self):
        ...
        newton_constraint_mat = ops.index_update(
            newton_constraint_mat,
            ops.index[x_size:newton_mat_size, 0:x_size],
            self.minimize_option.lin_equal_constr.mat,
        )
        newton_constraint_mat = ops.index_update(
            newton_constraint_mat,
            ops.index[0:x_size, x_size:newton_mat_size],
    ...
    def compute_log_barrier_function(self, x):
        ...
        if "custom" == self.minimize_option.inequal_constr.use_template:
            ...
            hessian += (
                1 / np.power(func_val, 2) * func_grad * func_grad.T
                - 1 / func_val * func_hes
            )
