...
from .geom import *
import interpolation.deepsvg.svglib.geom as geom
import re
import torch
...
class SVGPath:
    ...
    def from_xml(x: minidom.Element):
        ...
        print(x.hasAttribute("fill"),'~~~~svgPathfill')
        if fill == True:
            fill = x.getAttribute("fill")
        # fill = x.hasAttribute("fill") if not x.getAttribute("fill") == "none" else False
        ...
        for command in path_commands:
            if isinstance(command, SVGCommandMove):
                ...
                if isinstance(command, SVGCommandClose):
                    # print('not in move,in close',command) #Z
                    if allow_empty or svg_path.path_commands:  # SVGPath contains at least one command
                        svg_path.closed = True
                        if not svg_path.path_commands:
                            svg_path.path_commands.append(empty_command)
                        #连接起点与终点
                        if svg_path.path_commands[0].start_pos!=svg_path.path_commands[-1].end_pos:
                            svg_path.path_commands.append(SVGCommandLine(
                                                                     svg_path.path_commands[-1].end_pos,svg_path.path_commands[0].start_pos))
                        svg_paths.append(svg_path)
                    svg_path = None
                else:
    ...
    def save_svg(self, viewbox=Bbox(24), *args, **kwargs):
        from .svg import SVG
        return SVG([self.to_group()], viewbox=viewbox).save_svg(*args, **kwargs)
    ...
    def duplicate_extremities(self):
        ...
        # print(self.path_commands,'in duplicate_extremities')
        return self
