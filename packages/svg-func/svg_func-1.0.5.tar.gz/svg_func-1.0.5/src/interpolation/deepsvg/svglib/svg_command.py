...
class SVGCommand:
    ...
    def from_str(cmd_str: str, args_str: List[Num], pos=None, initial_pos=None, prev_command: SVGCommand = None):
        ...
        # print('command from_str',cmd_str,args_str) M [0.0, 34.64] L [8.91, 21.89, 17.83, 9.14] Z []
        ...
        for _ in range(nb_args // expected_nb_args):
            ...
            if cmd is SVGCmdEnum.LINE_TO or cmd is SVGCmdEnum.LINE_TO_VERTICAL or cmd is SVGCmdEnum.LINE_TO_HORIZONTAL:
                ...
                Q1 = args[0]
                Q2 = args[1]
                C1 = Point((Q0.x + 2 / 3 * (Q1.x - Q0.x)), (Q0.y + 2 / 3 * (Q1.y - Q0.y)))
                C2 = Point((Q2.x + 2 / 3 * (Q1.x - Q2.x)), (Q2.y + 2 / 3 * (Q1.y - Q2.y)))
                print(C1, C2, args[1])
                cmd_parsed = SVGCommandBezier(pos, C1, C2, args[1])
