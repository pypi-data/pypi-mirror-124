...
class TestBacktester:
    ...
    def test_successful_setting_of_strategy(self, backtester):
        ...
        backtester.set_strategy(StrategySubclass)
        assert backtester._strat_class == StrategySubclass
        assert isinstance(backtester.strategy, bt.BacktesterStrategy)
        assert backtester.strategy.parameters.fast_period == 50
        assert backtester.strategy.parameters.slow_period == 200

        backtester.set_strategy(StrategySubclass, parameters={"period": 1000})
        assert backtester._strat_class == StrategySubclass
        assert isinstance(backtester.strategy, bt.BacktesterStrategy)
        assert backtester.strategy.parameters.period == 1000

    def test__backtester_end_to_end__buy_at_start_missing_data(self):
        ...
        assert list(backtester.exchange.asset_equity["XBTUSD"].values())[-1] == 141824.5778771789
        assert list(backtester.exchange.asset_equity["ADAUSD"].values())[-1] == 5113.602638087386

    def test__backtester_end_to_end__ema_cross_9_50_case(self):
        ...
        expected_equity = [
            100000.0, 101783.83805909472, 103041.86672817028, 108097.27823167763, 111520.74698946749,
        ...
        assert backtester.exchange.equity.tolist() == expected_equity
        assert backtester.exchange.position_size.tolist() == expected_position_size
    ...
    def test_visualization(self):
        ...
        # Invalid plots type
        assert backtester.visualize(plots=123) is None

        # Total visualization is only for equity and drawdown
        assert backtester.visualize(plots=["fast_ma"]) is None

        # No symbol for candles plot
        assert backtester.visualize(plots="candles") is None

        # Wrong input symbol
        assert backtester.visualize("XBTUSDT", plots={"fast_ma": {}}) is None

        assert isinstance(backtester.visualize(), go.Figure)
        assert isinstance(backtester.visualize(plots="equity"), go.Figure)

        assert isinstance(backtester.visualize(
            "XBTUSD", plots={"fast_ma": {}}), go.Figure)

        assert isinstance(backtester.visualize(
            "XBTUSD", plots=["fast_ma", "equity", "drawdown"]),
            go.Figure)

        assert isinstance(backtester.visualize(
            "XBTUSD", plots=["fast_ma", "slow_ma"]), go.Figure)

        assert isinstance(backtester.visualize(
            "XBTUSD", plots=["fast_ma", "slow_ma", "candles"]), go.Figure)
