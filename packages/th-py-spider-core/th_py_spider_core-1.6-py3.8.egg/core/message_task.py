from utils.tsutils import getQueue
import traceback;
import json;
class TxyunMessage:
    ...
    def listent(self):
        while True:
            try:
                myQueue = getQueue(self.qname,self.secretId,self.secretKey)
                ...
                try:
                    ...
                    if "lx" not in messageBody.keys() or "data" not in messageBody.keys():
                        ...
                        flag = self.run(message_type= messageBody["lx"],message_body = messageBody["data"]);
                        if flag == True:
                            myQueue.delete_message(recv_msg.receiptHandle)
                except Exception as e:
                    traceback.print_exc();
