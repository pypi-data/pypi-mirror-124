...
class Definition:
	...
	def get_default_value(self, field_name=None):
		if not self.has_default_value():
			if field_name is None:
				field_name = self.name
			raise Config.exception_factory.create_internal_error(field_name, 'No default value configured')
...
class Index:
	...
	def get_version_by_display_name(display_name):
		# In the case of overlapping display names, choose the latest.
		# Overlap may occur because more than one version may be tied to a
		# release.
		...
		raise Config.exception_factory.create_internal_error(
				version,
				'No such version with display name: {0}'.format(display_name)
			)
	...
	def _mutate_model(definition_name, base_model, versions, target_version):
		# Init Mutation
		###############
		mutated_model = {}
	...
	def _add_inherited_fields(inherited_definition_name, version, model):
		parent_definition = Index.get(inherited_definition_name, version)
		inherited_fields = parent_definition.list_fields()
		for field_name in inherited_fields:
			model[field_name] = inherited_fields[field_name]

	def _extend_model(data_format, version):
		model = {}
		for inherited_definition_name in data_format.inherits:
			Index._add_inherited_fields(inherited_definition_name, version, model)
		for field_name in data_format.model:
			field = data_format.model[field_name]
			for action in data_format.field_init_actions:
		...
		data_format.model = model

	def _extend_mutations(mutations, data_format, version):
		# TODO: Should traverse inheritance heirarchy, not just parent
		for inherited_definition_name in data_format.inherits:
			inherited_definition = Index.get(inherited_definition_name, version)
			mutations.extend(inherited_definition.get_mutations())
	...
	def register_definition(
		...
		if Utils.Empty.isempty(base_version) is True:
			base_version = 'base'
		if data_type is DataType.Object:
			Index._extend_model(data_format, base_version)
		...
		for version in Index._versions[initial_version_index:]:
			...
			if version not in versions:
				...
				latest_def_version = Definition(
						...
						base_version=latest_def_version.base_version,
						final_version=latest_def_version.final_version,
						version=version,
			...
			if Utils.Empty.isempty(final_version) is False:
				if Index.compare_versions(version, final_version) == 1:
					break
			mutated_data_format = latest_def_version.get_data_format().copy()
			mutations = []
			if data_type == DataType.Object:
				Index._extend_model(mutated_data_format, version)
				mutated_data_format.model = Index._mutate_model(name, mutated_data_format.model, versions, version)
				Index._extend_mutations(mutations, mutated_data_format, version)
			mutations.extend(versions[version]['mutations'])
			version_conversion = None
