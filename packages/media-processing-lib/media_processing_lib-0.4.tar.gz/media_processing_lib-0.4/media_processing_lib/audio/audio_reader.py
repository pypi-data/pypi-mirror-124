...
def tryReadAudio(path:str, audioLib:str="librosa", count:int=5, **kwargs):
	...
	if forceWav and isVideo:
		...
		Debug.log(1, "[mpl::audio] Converting %s video to wav. Got path: %s" % (path, tmpPath))
		path = tmpPath
	...
	while True:
		try:
			...
			Debug.log(1, "[mpl::audio] Read audio %s. Shape: %s. Sample rate: %2.3f" % \
			...
			Debug.log(1, "[mpl::audio] Path: %s. Exception: %s" % (path, str(e)))
			i += 1
