...
def find_overlap(obj_a: t.Any, obj_b: t.Any) -> set:
    try:
        ...
        raise InternalException("Error when trying to find overlap between objects.")


def check_objects_number(objects_collection: t.Iterable) -> None:
    if set(objects_collection) == {}:
        raise InternalException("Any CpG overlap between myNorm and manifest files.")

def overlap_samples_between_poi_and_mynorm(poi: DataFrame, mynorm: DataFrame) -> t.Tuple[DataFrame, DataFrame]:
    if not set(poi.index) == set(mynorm.columns):
        intersection = set.intersection(
            set(poi.index), set(mynorm.columns)
        )
