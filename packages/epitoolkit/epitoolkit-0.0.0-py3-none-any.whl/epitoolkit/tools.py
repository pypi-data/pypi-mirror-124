...
from .utils import (extract_probes_from_region, check_if_porbes_in_mynorm, extract_category_from_manifest)
from .validators import (corect_col_names, 
                        find_overlap, 
...
class Visualize:
    def __init__(
        ...
        self.poi, self.mynorm = overlap_samples_between_poi_and_mynorm(self.poi, self.mynorm)
    ...
    def __upgrade_figure(self, fig: go.Figure, category_order: t.Union[t.List[str], None] = None,
                        title: str = "",
    ...
    def __export_figure(self, fig: go.Figure, 
                        static: bool = False, 
                        export: t.Union[str, Path, None] = None) -> None:
    ...
    def plot_CpG(self, cpg: str, title: str = "",
                static: bool = False,
                show_legend: bool = False,
        ...
        fig = self.__upgrade_figure(fig,  title=title,
    ...
    def plot_Range(self, chr: t.Union[str, int, None] = None, start: t.Union[int, None] = None, end: t.Union[int, None] = None, 
                        cpgs: t.Collection = [],
        ...
        if chr and start and end:
            available_probes = extract_probes_from_region(self.manifest, self.probes, chr, start, end)

        elif cpgs:
            available_probes = check_if_porbes_in_mynorm(self.probes, cpgs)
        
        else:
            raise InternalException("User must provide chr AND start and END arguments or collection of cpgs to visualize.")

        data = self.mynorm.loc[available_probes, :].T
        data = pd.concat((data, self.poi["POI"]), axis=1)
        data = pd.melt(data, "POI")

        fig = px.box(data, x="variable", y="value", color="POI", width=width, height=height, points=show_all_points)
        fig = self.__upgrade_figure(fig, category_order=None, 
                                    title=title, 
                                    show_legend=show_legend, 
                                    x_axis_label=x_axis_label, 
                                    y_axis_label=y_axis_label, 
...
class EnrichemntAnalysis():
    def __init__(self, manifest: t.Union[str, DataFrame], mynorm: t.Union[str, DataFrame], skiprows: int = 7, **kwargs):
    ...
    def enrichemntAnalysis(self, categories_to_analyse: t.Collection, cpgs: t.Collection) -> None:
        ...
        for category in categories_to_analyse:
            ...
            data = extract_category_from_manifest(self.manifest, category, self.mynorm.index)

            bg = data.value_counts(normalize=True, dropna=False) * 100
            input = data.loc[cpgs].value_counts(normalize=True, dropna=False) * 100
            ...
            fig.update_layout(barmode='group', title=category, xaxis_title="", yaxis_title="Frequency [%]")
            fig.show()
