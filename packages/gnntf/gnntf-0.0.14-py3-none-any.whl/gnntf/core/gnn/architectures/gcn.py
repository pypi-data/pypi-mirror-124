...
class NGCFLayer(Layer):
    def __build__(self, gcn, outputs: int, activation=tf.nn.leaky_relu, bias: bool=True,
        ...
        self.adjacency = gcn.get_adjacency(self.node_dropout, normalized="bipartite")
        return (gcn.top_shape()[0], outputs)
    ...
    def __forward__(self, gcn, features: tf.Tensor):
        ...
        output = self.activation(tf.matmul(tf.multiply(features, aggregated_features), self.W1)) \
            + self.activation(tf.matmul(aggregated_features, self.W2))
        return tf.math.l2_normalize(gcn.dropout(output, self.dropout), axis=0)
...
class NGCF(GNN):
    ...
    def __init__(self,
        ...
        layers.append(self.add(NGCFLayer(num_classes, regularize=0.02, dropout=dropout)))
