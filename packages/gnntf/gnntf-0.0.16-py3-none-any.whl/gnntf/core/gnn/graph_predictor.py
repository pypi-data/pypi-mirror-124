...
class negative_sampling:
    def __init__(self, positive_edges, graph, samples=1, negative_nodes=None, pool=None):
        ...
        self.graph = graph
        self.pool = pool
        self._negative_pool = {u: [v for v in negative_nodes if u!=v and not graph.has_edge(u, v) and not graph.has_edge(v, u)] for u, _ in positive_edges}
        if pool is not None:
            self._negative_pool = {u: random.sample(negatives, pool) for u, negatives in self._negative_pool.items()}

    def _resample(self):
        edges = np.empty((2*len(self.positive_edges),2), dtype=int)
        i = 0
        for u, v in self.positive_edges:
            edges[i, 0], edges[i, 1] = u, v
            i += 1
            negative_nodes = self._negative_pool[u]
