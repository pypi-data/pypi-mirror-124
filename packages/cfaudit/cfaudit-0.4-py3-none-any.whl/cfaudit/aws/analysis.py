...
def analyze_security_groups(session: boto3.session.Session, whitelist: list = []) -> dict:
    ...
    Parameters:
        - aws_session:
            Type: boto3.Session
            Description: Client object from cfaudit.aws.authentication.create_session() modules
    ...
    response['Metadata']['StartedAt'] = cfaudit.generic.helpers.get_current_time()

    whitelist += cfaudit.aws.whitelist.list_all_safe_ips(session)
    all_security_groups = session.client('ec2').describe_security_groups()['SecurityGroups']
    all_attached_groups = cfaudit.aws.attached.list_all_attached_secgroups(session)
    ...
    response['Metadata']['FinishedAt'] = cfaudit.generic.helpers.get_current_time()
    response['Metadata']['CloudProvider']['Name'] = CLOUD_PROVIDER
    response['Metadata']['CloudProvider']['Auth'] = (
        cfaudit.aws.authentication.get_session_info(session)
    )
    return response
