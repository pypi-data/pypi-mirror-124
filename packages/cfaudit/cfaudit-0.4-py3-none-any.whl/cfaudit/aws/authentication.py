...
def create_session(
    ...
    if not session.get_credentials():
        raise cfaudit.aws.exceptions.cfauditAWSCreateSessionError(
            "cfaudit could not authenticate to AWS. "
...
def get_boto_clients(session: boto3.session.Session) -> dict:
    """
    Receives the session, and return all cfaudit used Boto3 Clients
    """
    return {
