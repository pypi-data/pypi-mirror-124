...
def print_human(analysis):
    ...
    colorama.init()
    HEADER = cfaudit.generic.output.generate_header_message(
        "cfaudit Analysis"
    )

    FIRST_SECTION = cfaudit.generic.output.generate_section_message(
    ...
    if unused_groups_list:
        ...
        UNUSED_SECGROUP_NOTIFICATIONS.append(
            cfaudit.generic.output.generate_warning_message(
                f"{len(unused_groups_list)} security groups found not being used"
            )
        )
    else:
        UNUSED_SECGROUP_NOTIFICATIONS.append(
            cfaudit.generic.output.generate_info_message(
                "All security groups are attached and being used"
            )
        )

    SECOND_SECTION = cfaudit.generic.output.generate_section_message(
    ...
    if unsafe_groups_list:
        ...
        if alert_rules:
            UNSAFE_RULES_NOTIFICATIONS.append(
                cfaudit.generic.output.generate_alert_message(
    ...
    start_time = cfaudit.generic.helpers.generate_human_time(
        cfaudit.generic.helpers.convert_string_datetime(analysis["Metadata"]["StartedAt"])
    )
    end_time = cfaudit.generic.helpers.generate_human_time(
        cfaudit.generic.helpers.convert_string_datetime(analysis["Metadata"]["FinishedAt"])
    )
