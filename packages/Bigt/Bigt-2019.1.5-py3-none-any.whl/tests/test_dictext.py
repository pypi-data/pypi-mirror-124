...
def test_weights():
    a = {"a":[{'a': '21', 'b': 3, 'd': '20'}, {'a': '1', 'b': 3, 'd': '2'}]}
    b = {"a":[{'a': '1', 'b': 3, 'd': '20', "y": 123}]}
    result, not_present, changed = DictExt(b).issubset(a, weights={"a": {"a": 2, "y": {"v": 4}}})
    assert not result
    assert not_present == {'a': [{'y': 123}]}
    assert changed == {"a":[{'d': {"old": "20", "new": "2"}}]}
