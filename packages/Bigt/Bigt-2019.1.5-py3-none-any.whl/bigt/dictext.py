...
class _list(_default):
    ...
    def _coefficient(self, not_present, changed, weights={}):
        ...
        """
        def if_present(k):
            return weights[k] if type(weights[k]) == int else 1
        n, c = 0, 0
        for k, _ in (not_present or {}).items():
            n += 1 * (if_present(k) if k in weights else 1)
        for k, _ in (changed or {}).items():
            c += 1 * (if_present(k) if k in weights else 1)
        return (n * 1.000001) + c
