...
def test_weights():
    a = {"a":[{'z': {'a': '21', 'b': 3, 'd': '20'}}, {'z': {'a': '1', 'b': 3, 'd': '2'}}]}
    b = {"a":[{'z': {'a': '1', 'b': 3, 'd': '20', "y": 123}}]}
    result, not_present, changed = DictExt(b).issubset(a, weights={"a": {'z': {"a": 2, "y": {"v": 4}}}})
    assert not result
    assert not_present == {'a': [{'z': {'y': 123}}]}
    assert changed == {"a":[{'z': {'d': {"old": "20", "new": "2"}}}]}
