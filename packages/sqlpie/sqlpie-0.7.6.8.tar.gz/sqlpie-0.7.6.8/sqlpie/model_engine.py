...
from sqlpie.exceptions import UnknownSourceError, FailedAddingEdge


class ModelEngine:

  def __init__(self, model, vars_payload={}):
    environment_conf_file = open("./config/environment.yml", "r")
    self.environment_conf = yaml.load(environment_conf_file, Loader=yaml.FullLoader)['environment']
  ...
  def source(self, source_name, table_name):
    ...
    source_type = None   
    if source_name in list(Project.sources()['sources'].keys()):
      source_schema = self.sources_conf['sources'][source_name]['schema_name']
      source_type = 'source'
    if self.dynamic_env:
      if self.dynamic_env_diff:
        if source_table in self.dynamic_env_diff:
          ...
          source_schema = self.dynamic_env_prod_index[source_name]
      else:
        model = self.dynamic_env_model
        prep_model = self.dynamic_env_prep_model
    else:
    ...
    if edge not in self.edges:
      ...
      try:
        ...
        raise FailedAddingEdge(FailedAddingEdge.message(source_table, destination_table))
    return self.get_source_table_return_name(source_type, source_schema, source_name, table_name)
  
  def get_source_table_return_name(self, source_type, source_schema, source_name, table_name):
    if source_type == 'source':
      if 'database' in self.sources_conf['sources'][source_name].keys():
        db = self.sources_conf['sources'][source_name]['database']
      else:
        db = self.sources_conf['default_database']
    elif self.dynamic_env:
      if source_type == 'dynamic_env_prod_pointer':
        db = self.environment_conf['production']['database']
      else:
        db = self.environment_conf['staging']['database']
    else:
      db = self.environment_conf['production']['database']
    return f"{db}.{source_schema}.{table_name}"
