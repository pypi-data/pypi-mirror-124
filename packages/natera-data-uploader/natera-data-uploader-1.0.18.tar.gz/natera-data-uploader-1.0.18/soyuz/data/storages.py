...
class ExodusStorage(Storage):
    ...
    def upload_file(self, data_file, remote_folder, types, properties):
        ...
        data = {'name': data_file.get_name(),
                ...
                'tags': []}
        response = requests.post(os.path.join(self.exodus_url, "2.0/data/files/new"), json=data)
        ...
        with open(data_file.get_full_path(), 'rb') as file_object:
            ...
            for i, chunk in enumerate(read_in_chunks(file_object, self.chunk_size)):
                data = {'file': {
                        ...
                        'uploadId': uploadId,
                        'part': i+1}
                response = requests.post(os.path.join(self.exodus_url, "2.0/data/files/part-upload"), json=data)
                response.raise_for_status()
                data = response.json()
        ...
        response = requests.post(os.path.join(self.exodus_url, "2.0/data/files/finish-upload"), json=data)
        response.raise_for_status()
        data = response.json()
        file_id = data['link']
