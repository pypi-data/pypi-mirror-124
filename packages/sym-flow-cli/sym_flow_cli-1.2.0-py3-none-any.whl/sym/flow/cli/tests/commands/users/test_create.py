        ...
        with click_setup() as runner:
            result = runner.invoke(
                ...
                [
                    ...
                    "foo_bar@symops.io",
                    "-s",
                    "okta",  # factory does not create an okta service
                ],
            )
            assert "No service is registered for type okta" in result.output
            assert (
                "You can create the service with `symflow services create --service-type okta`"
                in result.output
            )
            mock_apply.assert_not_called()
