...
def to_service_list(ctx: click.Context, param, value) -> Dict[ServiceType, List[Service]]:
    """Parses the space sparated --identities list into services to set up identities for
    ...
    registered_services = api.get_services()
    ...
    for service_type_str in value:
        ...
        if not service_type:
            existing_types = set([s.service_type for s in registered_services])
...
def users_create(
    ...
    \b
    Example:
        `symflow users create user@symops.io -i aws_sso -i slack`
    """
    api = SymAPI(url=options.api_url)
    users = api.get_users()
